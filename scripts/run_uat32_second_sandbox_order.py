"""Run the one approved UAT3.2 Hyperliquid testnet lifecycle probe."""
...
REPORT_PATH = Path("docs/uat3_2_second_sandbox_order_attempt.md")
SUMMARY_PATH = Path("docs/uat3_2_second_sandbox_order_attempt_summary.json")
...
def _decision(summary: dict[str, Any]) -> str:
    if summary["open_order_remains"] or summary["unknown_state"] or summary["unexpected_fill"]:
        return "UAT3.3 is blocked"
    if not summary.get("account_api_wallet_readiness", {}).get("allowed", False):
        return "UAT3.3 is blocked"
    return "UAT3.3 additional sandbox lifecycle testing may be scoped"


def _markdown_report(summary: dict[str, Any]) -> str:
    status = "verified" if summary["order_attempt_count"] == 1 else "blocked"
    next_decision = _decision(summary)
    ...
    readiness = summary.get("account_api_wallet_readiness") or {}
    approval_text = REQUIRED_UAT32_ACTUAL_SUBMISSION_APPROVAL_TEXT
    return f"""# UAT3.2 Second Sandbox Order Attempt
...
Status: `{'verified' if readiness.get('allowed') else 'blocked'}`

| Field | Value |
| --- | --- |
| checked | `{str(readiness.get('checked', False)).lower()}` |
| account role | `{readiness.get('account_role')}` |
| signer role | `{readiness.get('signer_role')}` |
| signer is target account | `{str(readiness.get('signer_is_target_account', False)).lower()}` |
| API wallet authorized for account | `{str(readiness.get('api_wallet_authorized_for_account', False)).lower()}` |
| sandbox account equity available | `{str(readiness.get('sandbox_account_equity_available', False)).lower()}` |
| sandbox account equity sufficient | `{str(readiness.get('sandbox_account_equity_sufficient', False)).lower()}` |
| live-fed drawdown verified | `{str(readiness.get('sandbox_drawdown_live_fed_verified', False)).lower()}` |
| drawdown not stale | `{str(readiness.get('sandbox_drawdown_not_stale', False)).lower()}` |
| account address | `{readiness.get('account_address_abbrev')}` |
| signer address | `{readiness.get('signer_address_abbrev')}` |
...
async def _main() -> int:
    ...
    parser.add_argument("--execute-approved-uat32", action="store_true")
    parser.add_argument("--env-file", default=".env")
    args = parser.parse_args()

    if not args.execute_approved_uat32:
        raise SystemExit("Refusing to run without --execute-approved-uat32")

    approval_result = validate_uat32_actual_submission_approval_text(
        REQUIRED_UAT32_ACTUAL_SUBMISSION_APPROVAL_TEXT
    )
    ...
    service = UAT32SecondSandboxOrderAttemptService(
        ...
        json.dumps(
            {
                ...
                "account_api_wallet_readiness": summary.get("account_api_wallet_readiness", {}).get("allowed"),
