...
The static local evidence dashboard in `apps/dashboard/` visualizes the generated SV1.13 review JSON plus component `batch_report.json` files in human-readable panels, charts, and ledgers. It also includes Experiments, UAT Chart Cockpit, and UAT2 Shadow Run tabs. The UAT Chart Cockpit tab loads committed UAT2 and UAT3.4 summary JSON and UAT4.1 presents it as an exchange-style workstation with compact top bar, observation-only market rail, central chart cockpit, right order-book/market/signal/risk rail, and bottom blotter tabs. It shows the UAT watchlist, market-data coverage, static chart snapshots, EMA5/EMA10/SMA20/RSI/MACD labels, shadow/routed lifecycle markers, active ETH sandbox route card, unified equity-source visibility, routed-order ledger filters, and shadow-signal overlay. The UAT2 tab loads `docs/uat2_shadow_strategy_top20_observation_summary.json`, shows the 45-record signal matrix, would-open records, no-trade reason breakdowns, ETH evidence candidate status, timing assumptions, not-live-account shadow drawdown, boundary flags, the blocked UAT3 readiness checklist, and the UAT3.0/UAT3.0.1/UAT3.0.2/UAT3.0.3 sandbox-design/readiness status panel. Run it from the repo root with `.venv/bin/python -m http.server 8765`, then open `http://127.0.0.1:8765/apps/dashboard/index.html`. It is a visualization surface only: it does not generate evidence packs, import candles, approve paper/live trading, call private/signed/order endpoints, create approvals, add order controls, submit orders, or change Money Flow rules.
