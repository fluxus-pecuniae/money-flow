...
PT0.0.3 extends the Historical Replay cockpit with `1D` replay support, a target-start readiness audit for `2025-01-01T00:00:00Z`, and a dashboard data-horizon panel backed by `docs/pt0_0_3_historical_strategy_replay_summary.json`. The current committed local data does not reach Jan 2025; `1D` is explicitly aggregated from `4h` historical replay candles and does not create a new production Money Flow 1D sleeve. UAT3.1-PT0.0.3 still creates no production order artifacts, no order controls, no live behavior, no Money Flow rule change, no evidence pack, and no private/signed/order endpoint calls.
...
SV2.0.2 resolves the canonical evidence blocker. It imports normalized Hyperliquid public mainnet candles through the hardened strategy-validation candle importer, writes canonical campaign configs under `configs/strategy_validation/campaigns/sv2_0_2/`, and generates ignored canonical evidence packs for Money Flow v1.2 across `15m`, `1h`, `4h`, and `1d`. Supported canonical evidence symbols are BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, and AVAX; SHIB is explicitly deferred as `kSHIB` because unit semantics are not clean enough for canonical evidence. Dynamic equity uses 10,000 USDC per independent scenario, open positions are force-closed at dataset end, testnet data is not strategy truth, no orders were submitted, and SOR-EV1 may proceed from this baseline. See [SV2.0.2 canonical evidence packs](docs/sv2_0_2_canonical_sv2_evidence_packs.md).
...
The static local evidence dashboard in `apps/dashboard/` visualizes the generated SV1.13 review JSON plus component `batch_report.json` files in human-readable panels, charts, and ledgers. It also includes Experiments, Historical Replay, UAT Chart Cockpit, and UAT2 Shadow Run panels, though the visible top-level navigation is Strategy, Historical Replay, Evidence, and Experiments. The Historical Replay tab loads `docs/pt0_0_3_historical_strategy_replay_summary.json` with fallback to PT0.0.2 plus SV2.0 readiness from `docs/sv2_0_historical_data_refresh_summary.json`; it renders BTC/ETH/SOL historical replay candles when replay data exists, exposes the expanded SV2.0 universe and 1D readiness, shows Money Flow v1.2/`sleeve_1d`, and keeps unsupported/missing replay data visible instead of silently substituting another chart. Chart arrows and trade table rows select the linked trade in a polished inspector with entry/exit reasons, indicator tiles, costs, PnL, and equity movement. Its replay-strategy dropdown switches between `OG replay / strategy`, research-only `MACD removed`, and research-only `Only close on 5/20 cross` across available historical replay rows. Historical replay uses historical public candle replay data as strategy truth; Hyperliquid testnet prices are not strategy truth. The UAT Chart Cockpit tab loads committed UAT2, UAT3.4, UAT4.2, and PT0 summary JSON. PT0 replaces the custom chart shell with the official local TradingView Lightweight Charts standalone bundle and renders Hyperliquid testnet public candlesticks, volume, EMA5/EMA10/SMA20 overlays, RSI/MACD labels, green/red paper/sandbox markers, top-20 paper eligibility, sandbox balance-poll policy, internal 10,000 USDC paper-equity state, and routed-order / paper-trade blotter rows. Browser live charting polls Hyperliquid testnet public `allMids` and `candleSnapshot` every 15 seconds and falls back to committed local JSON if public polling is unavailable. PT0.0.1 stabilizes this chart by using an explicit bounded chart height, reusing the existing chart and series handles across refreshes, removing the autosize feedback loop, limiting `fitContent()` to new symbol/timeframe initialization, and adding `?disableLivePolling=true` / `?livePolling=false` fallback flags. The UAT2 tab loads `docs/uat2_shadow_strategy_top20_observation_summary.json`, shows the 45-record signal matrix, would-open records, no-trade reason breakdowns, ETH evidence candidate status, timing assumptions, not-live-account shadow drawdown, boundary flags, the blocked UAT3 readiness checklist, and the UAT3.0/UAT3.0.1/UAT3.0.2/UAT3.0.3 sandbox-design/readiness status panel. Run it from the repo root with `.venv/bin/python -m http.server 8765`, then open `http://127.0.0.1:8765/apps/dashboard/index.html`. It is a visualization and PT0 paper/sandbox monitoring surface only: it does not generate evidence packs, import candles, approve live trading, call live endpoints, call private order endpoints, create approvals, add order controls, submit orders, or change Money Flow rules.
