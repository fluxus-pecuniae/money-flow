...
SV2.1 adds a broad 1D-only public Hyperliquid period evidence refresh at [SV2.1 Broad Hyperliquid 1D Period Evidence](docs/sv2_1_broad_hyperliquid_1d_period_evidence.md). The run used public `meta` and `candleSnapshot` only, targeted 183 active public metadata symbols, imported available 1D candles back to 2024-01-01 where available, and generated ignored period packs for `2024`, `2025`, `YTD`, and `ALL`: 646 baseline packs total. Generated raw candles/configs stay under `/tmp/money-flow-sv21-broad-1d/`, and generated evidence packs stay under ignored `reports/strategy_validation/`. The follow-up Historical Replay builder writes 5116 ignored selected chart/trade JSON files for the SV2.1 baseline plus 1912 ignored evidence-only conservative candidate pack directories for `avoid_low_rolling_range_50`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. The dashboard `Historical Replay` tab uses a Period selector for 2024 / 2025 / YTD / ALL and lazy-loads those selected replay files. This is founder-review research evidence only, not a production rule change, parameter optimization, paper/live approval, or order/submission path.
