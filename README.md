...
MF-ORIG-EV2 adds [multi-timeframe Original Money Flow evidence packs](docs/mf_orig_ev2_multitimeframe_evidence_packs.md) plus a compact [MF-ORIG-EV2 summary JSON](docs/mf_orig_ev2_multitimeframe_evidence_summary.json). It generates 144 ignored evidence-pack directories and 36 ignored dashboard chart-data files from canonical SV2.0.2 DB-imported candle truth, covering the four MF-ORIG hypotheses across BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, and both next-candle fill assumptions. The 1d row is source-primary, 4h/1h are fractal adaptations, and 15m is a stress-test adaptation. Historical Replay and the Evidence Run Ledger now load MF-ORIG-EV2 strategies when local chart-data JSON exists. Production Money Flow v1.2 remains unchanged; no MF-ORIG hypothesis is approved; no orders were submitted; live trading is not approved; dashboard date filters remain display-only and not canonical evidence.
