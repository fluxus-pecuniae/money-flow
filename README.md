...
- Paused timeframes: `15m` remains paused and kept as diagnostic/legacy context only.
...
- Next recommended action: after founder review, start the `pt_rt1_6_week2_active` run from the Paper Trading control server or documented command.
...
- Current phase truth: `PT-RT1.6` prepares the founder-selected Week 2 paper slate and dashboard defaults without starting the runtime; `GOAL-STRAT1` found zero founder production-testing review candidates; `GOAL-STRAT2` candidates remain research-only and are not active Week 2 lanes.
...
- SV2.0.2 canonical historical evidence is the current multi-timeframe baseline.
- SV2.1 broad 1D evidence is separate founder-review research and does not supersede SV2.0.2 unless a later phase says so.
- PT-RT is forward observation, not historical evidence regeneration.
- Candidate, MF-ORIG, and wildcard lanes are synthetic-only and cannot send testnet orders.
- Testnet fills/prices do not update strategy PnL and are not strategy truth.
- Dashboard display filters are browser recalculations from loaded rows; arbitrary-date canonical evidence requires backend Strategy Validation.
...
Static review without runtime controls is still available:
...
Current Paper Trading status: PT-RT1.5.3 fixes the fixed-25-USDC Hyperliquid testnet size/precision path exposed by PT-RT1.5.2. The bounded PT-RT1.5.3 smoke fetched Hyperliquid testnet public metadata, used BTC `asset_id=3` and `szDecimals=5`, formatted `raw_quantity=0.0003371657844161974442833541252` to `formatted_quantity=0.00033`, submitted one labeled `testnet_transport_smoke_not_strategy_signal` order, received accepted/open, canceled it, and reconciled to no open order. No synthetic trade was created and synthetic PnL was not updated. Active timeframes remain 1h / 4h / 1d, 15m is paused, startup-valid confirmations remain blocked until a fresh false-to-true post-start signal, and signed testnet transport remains fixed 25 USDC for fresh `money_flow_v1_2_baseline` opens only. Public mainnet candles remain strategy truth, candidate lanes remain synthetic-only, no strategy is production-approved, and live trading is not approved.
...
Current Obsidian entrypoints are the canonical command center, current phase note, decision log, coordination note, project memory, and the relevant track maps. OB2.0 refreshes the brain after EV-AUDIT1 so a founder or agent can distinguish Money Flow v1.2, Original Money Flow / MF-ORIG, SOR repair variants, STRAT-EV discovery, canonical evidence methodology, dashboard visualization, UAT sandbox plumbing, and the PT-RT1 paper-observation substrate. The key current maps are [Strategy Family Map](money-flow/00%20Maps/Strategy%20Family%20Map.md), [Evidence and Backtesting Map](money-flow/00%20Maps/Evidence%20and%20Backtesting%20Map.md), [Data Source and Market Data Map](money-flow/00%20Maps/Data%20Source%20and%20Market%20Data%20Map.md), [Dashboard and UI Map](money-flow/00%20Maps/Dashboard%20and%20UI%20Map.md), [Paper Observation Roadmap](money-flow/00%20Maps/Paper%20Observation%20Roadmap.md), [Strategy Status Register](money-flow/10%20Strategy/Strategy%20Status%20Register.md), and [EV-AUDIT1 Summary](money-flow/20%20Evidence/EV-AUDIT1%20Summary.md). Do not create duplicate command centers or competing current-phase notes.

SV2.0.2 is the current canonical evidence baseline: DB-imported Hyperliquid public-mainnet candles, BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, 36 packs, and 72 rows. EV-AUDIT1 promotes no clean production strategy candidate. Evidence is good enough for visual review and hypothesis filtering only, not production-rule change, strategy paper-runtime authorization, or live trading. The Gerald Peters original Money Flow PDF is stored in `money-flow/90 Reference/` for future source-exact MF-ORIG reconciliation. Paper trading is approved only for Hyperliquid testnet/sandbox; live trading and real-capital trading are not approved.

PT0.0.3 extends the Historical Replay cockpit with `1D` replay support, a target-start readiness audit for `2025-01-01T00:00:00Z`, and a dashboard data-horizon panel backed by `docs/pt0_0_3_historical_strategy_replay_summary.json`. SV2.0/SV2.0.2 supersede the earlier PT-only limitation: `1d` is now a real Money Flow v1.2 sleeve, and the regenerated public-mainnet canonical evidence/chart data reaches Jan 2025 for supported 4h and 1d rows where Hyperliquid public history is available. UAT3.1-PT0.0.3 still creates no production order artifacts, no order controls, no live behavior, no Money Flow rule change, no evidence pack, and no private/signed/order endpoint calls.

SV2.0 adds `sleeve_1d` as a real Money Flow sleeve and updates the strategy version to `money_flow_v1_2`. The 1D settings are an initial non-optimized baseline: 50 bars, RSI 46-72, overbought 78, trim 84, max EMA5 extension 3.0%, and MACD required. The refreshed evidence universe is BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, AVAX, and SHIB; SHIB is explicitly resolved through Hyperliquid public mainnet metadata as `kSHIB`. Public `candleSnapshot` data reaches Jan 2025 for 4h and 1D rows, while 15m and 1h rows are limited by Hyperliquid public recent-candle availability and carry `hyperliquid_public_5000_candle_limit`. See [SV2.0 historical readiness](docs/sv2_0_historical_data_refresh_1d_and_expanded_universe_readiness.md), [SV2.0 evidence rebuild](docs/sv2_0_money_flow_1d_sleeve_expanded_universe_evidence_rebuild.md), and `docs/sv2_0_historical_data_refresh_summary.json`.

SV2.0.1 hotfixes canonical evidence truth before SOR-EV1. The compact SV2 rows now normalize Hyperliquid close slots to exact timeframe boundaries, split staged data from DB-imported data, label compact rows as non-canonical, force-close/MTM-account final open positions, count entry fees at open, keep internal `1d` while displaying `1D`, and reject missing EMA/RSI/MACD fields instead of treating them as zero. Runtime sleeve allocations are now 0.25 each. See [SV2.0.1 canonical evidence truth hotfix](docs/sv2_0_1_canonical_evidence_truth_hotfix.md).
...
SOR-EV2.1 adds the [Evidence Lab / Variant Review dashboard](docs/sor_ev2_1_evidence_lab_ui.md). The dashboard loads the committed SOR-EV1/SOR-EV2 summaries plus later SOR-EV3 and MF-ORIG-EV1.1 summaries, labels canonical SV2.0.2 DB-imported evidence as the baseline, and shows variant matrix, control pockets, worst trades, late-entry, large adverse-candle, RSI/MACD rejection, and Original Money Flow reconstruction panels. The Variant Summary Matrix uses founder-review labels so promising, mixed, deferred, no-op, diagnostic-only, and hard-rejected outcomes are separated instead of flattening every non-candidate into rejected. MF-ORIG-EV1.1 appears as corrected replay/report JSON visualization, not a new canonical evidence-pack run. It is UI/visualization only: variants and MF-ORIG hypotheses remain evidence-only, no production Money Flow rules changed, no orders were submitted, live trading is not approved, testnet execution remains separate from strategy evidence, and dashboard date filters are display-only rather than canonical evidence.
...
SOR-EV3 adds the founder-selected [avoid sideways / low-volatility drilldown](docs/sor_ev3_avoid_sideways_low_volatility.md). It uses canonical SV2.0.2 pack paths only, passes baseline parity for all 72 canonical scenarios, and true-forward replays ATR-percentile, flat-trend, rolling-range compression, MACD-flat chop, and conservative combined blockers with blocked-entry attribution and control-pocket impact. No variant was promoted; production Money Flow rules remain unchanged. Evidence Lab loads the SOR-EV3 summary in a focused founder-candidate section and now distinguishes strict candidate promotion from founder-review labels: `avoid_low_rolling_range_20` is `promising_control_pocket_risk`, `avoid_low_rolling_range_50` is `promising_high_pnl_control_risk`, and hard rejections remain separate. The local SV2.0.2 dashboard chart-data generator can now append full Historical Replay rows for `avoid_low_rolling_range_20` and `avoid_low_rolling_range_50` across the 9 supported symbols, 4 timeframes, and both fill assumptions; those replay rows are research-only and do not approve a production rule. Dashboard date filters remain display-only and not canonical evidence.

MF-ORIG-EV1.1 hotpatches the evidence-only [Original Money Flow reconstruction](docs/mf_orig_ev1_original_money_flow_reconstruction.md) plus [source specification and gap matrix](docs/mf_orig_ev1_original_money_flow_spec_and_gap_matrix.md). MF-ORIG-EV1 reconstructed the prompt-provided Gerald Peters Money Flow source hierarchy separately from production Money Flow v1.2, treating `1d` as the primary original timeframe and modeling Stage 1-4, 5 EMA / 20 SMA triggers, RSI profit warnings, MACD-as-TSI substitute confirmation/warnings, prior support/pivot stop proxies, and 1% risk-based sizing. MF-ORIG-EV1.1 quarantines the pre-hotpatch PnL/drawdown conclusions, regenerates the Markdown/JSON with event-ledger accounting, single-counted entry fees and trim PnL, peak-to-trough drawdown, and truthful baseline-positive 1d control-pocket labeling. Evidence Lab now auto-loads `docs/mf_orig_ev1_original_money_flow_reconstruction_summary.json` so the founder can review the latest corrected MF-ORIG run visually. The source PDF is now present in `money-flow/90 Reference/`; future MF-ORIG work can reconcile exact source text without changing existing evidence numbers. Production Money Flow v1.2 remains unchanged; no original hypothesis is approved for production, paper runtime, or live trading.

MF-ORIG-EV2 adds [multi-timeframe Original Money Flow evidence packs](docs/mf_orig_ev2_multitimeframe_evidence_packs.md) plus a compact [MF-ORIG-EV2 summary JSON](docs/mf_orig_ev2_multitimeframe_evidence_summary.json). It now generates 288 ignored evidence-pack directories and 612 ignored dashboard chart-data files from canonical SV2.0.2 DB-imported candle truth, covering the four source-faithful 1% risk-sizing MF-ORIG hypotheses plus four founder-requested full-equity/notional counterparts across BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, and both next-candle fill assumptions. The 1d row is source-primary, 4h/1h are fractal adaptations, and 15m is a stress-test adaptation. Historical Replay and the Evidence Run Ledger now load all eight MF-ORIG-EV2 replay strategies when local chart-data JSON exists. Production Money Flow v1.2 remains unchanged; no MF-ORIG hypothesis is approved; no orders were submitted; live trading is not approved; dashboard date filters remain display-only and not canonical evidence.

EV-AUDIT1 adds a full [hypothesis, data integrity, and paper-readiness audit](docs/ev_audit1_full_hypothesis_data_and_paper_readiness_review.md) plus a compact [audit summary JSON](docs/ev_audit1_full_hypothesis_data_and_paper_readiness_review_summary.json). It inventories Money Flow v1.2, SOR-EV1/SOR-EV2/SOR-EV3, MF-ORIG-EV1.1/MF-ORIG-EV2, and pending STRAT-EV1 plan-only status; scores data and methodology; explains biggest winners, biggest losers, losing streaks, regime attribution, control-pocket damage, and paper-observation readiness. The audit promotes no clean strategy candidate. It says current evidence is useful for visual review and hypothesis filtering only, not a production-rule change, strategy paper-runtime authorization, or live trading. PT-RT1 is the recommended future phase for real-time public market data plus paper observation under separate scope and gates.

PT-RT1 adds the [real-time paper-observation and testnet-plumbing substrate](docs/pt_rt1_real_time_paper_observation_and_testnet_plumbing.md). The strategy-truth lane is public Hyperliquid mainnet market data only, evaluates fully closed candles, computes real-time indicators, and maintains independent synthetic 10,000 USDC ledgers. PT-RT1.1A expands the lab before the 24-hour run to exactly 10 lanes: Money Flow v1.2 baseline, two SOR rolling-range candidate lanes, four MF-ORIG full-equity reference lanes, and three wildcard expert observation hypotheses. PT-RT1.1B adds the [Hyperliquid public-mainnet connector/runtime readiness](docs/pt_rt1_1b_hyperliquid_live_market_data_and_runtime_readiness.md). PT-RT1.1C wrote ignored local artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`; PT-RT1.4.1 later verified that this older process still created 15m opens after the active Week 1 cutover, so that directory is now pre-cutover burn-in and excluded from active scoring. PT-RT1.2 adds [runtime state and 20 USDC testnet transport gates](docs/pt_rt1_2_runtime_state_and_testnet_probe_transport.md). PT-RT1.3 adds [candle-truth data-health semantics](docs/pt_rt1_3_candle_truth_data_health.md): missing, stale, thin, or nonpositive Hyperliquid public mids are warning-only when clean fully closed `candleSnapshot` rows are available; candle/indicator readiness is the strategy gate. PT-RT1.4 adds the [Paper Trading command-center cleanup](docs/pt_rt1_4_paper_trading_command_center_cleanup.md): active Week 1 review uses `1h`, `4h`, and `1d`; `15m` is paused as legacy/noise-reduction data and excluded from active scoring; lane comparison is timeframe-scoped; and testnet labels separate audit-only shapes from disabled order transport. PT-RT1.4.1 adds the daily founder review pack at [PT-RT Week 1 Daily Summary](docs/pt_rt_week1_day_summary.md) plus compact JSON, and starts the active runtime under ignored `reports/paper_runtime/pt_rt1_4_1_active_week/`; the first artifact cycle had 0 15m rows and Week 1 paper observation may continue. PT-RT1.5 resets Week 1 to candle-close-only baseline-linked testnet lifecycle rows. PT-RT1.5.1 adds [signed testnet transport, warm-start gating, and open-position MTM](docs/pt_rt1_5_1_signed_testnet_transport_warm_start_and_mtm.md): startup-valid confirmations are recorded but blocked until fresh post-start signals, the dashboard defaults to ignored `reports/paper_runtime/pt_rt1_5_1_smoke/`, open-position MTM uses public mainnet mids or the latest closed candle, and signed testnet transport is allowed only for fresh Money Flow v1.2 baseline opens with fixed 25 USDC notional. PT-RT1.5.2 proved the signed testnet path could reach Hyperliquid testnet but exposed an `Order has invalid size` reject. PT-RT1.5.3 adds the [size/precision hotfix](docs/pt_rt1_5_3_hyperliquid_testnet_size_precision_hotfix.md), resolves order size formatting from Hyperliquid testnet public metadata, and verified one accepted/open -> canceled -> reconciled 25 USDC transport smoke. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping the process and machine awake/networked. The Paper Trading dashboard can be served with `scripts/run_dashboard_control_server.py` to expose a local-only Start Run / Stop Run panel that launches allowlisted PT-RT1.5.x runs through Mac `caffeinate`, with fresh-signal gating, signed baseline-only testnet transport gates, and compact decision logging. Testnet fills/prices never update strategy paper PnL. PT-RT1/PT-RT1.1/PT-RT1.2/PT-RT1.3/PT-RT1.4/PT-RT1.4.1/PT-RT1.5/PT-RT1.5.1/PT-RT1.5.2/PT-RT1.5.3 are not production approval, not live approval, and not strategy paper-runtime authorization for real capital.

To open the dashboard with the local Start Run control:

```bash
.venv/bin/python scripts/run_dashboard_control_server.py --host 127.0.0.1 --port 8767
```

Then open `http://127.0.0.1:8767/apps/dashboard/index.html`.
...
The report includes data range, instrument/source, component/timeframe, assumptions, fill timing, simulated trade summaries, no-trade/invalid reason counts, net/gross PnL, fees, slippage cost, win/loss rates, profit factor, closed-trade drawdown, mark-to-market drawdown, average trade duration, best/worst trade, return on initial capital, component/timeframe grouping, and limitations. Default Strategy Validation sizing remains `constant_initial_capital_notional_per_trade`: each opened trade sizes from `initial_capital * position_notional_pct`, while realized equity affects PnL/drawdown metrics but not the next trade size. SV1.13.2 also supports `dynamic_equity_pct`, where each new trade sizes from current realized equity after prior closed-trade net PnL. SV1.14 adds trade-anatomy and market-structure diagnostics over existing evidence packs and candles; the diagnostics explain current entry/exit logic, ETH `sleeve_1h` behavior, 15m/4h weakness, and recent swing high/low context without changing Money Flow rules or adding filters. SV1.15 adds controlled research-only hypothesis experiments over the dynamic-equity evidence, comparing one-change overlays and attribution against baseline while leaving production Money Flow unchanged. SV1.15.1 classifies those results by methodology: completed-trade overlays are diagnostic estimates rather than true forward replays, recent-low invalidation is a lookahead diagnostic upper bound, and lower-RSI entry admission required rejected-signal replay data before truthful testing. SV1.16 adds that Strategy Validation-only replay substrate: evaluated candles can carry production-rule decision/rejection context, RSI zone, EMA/MACD state, price-extension facts, market regime, and recent swing high/low diagnostics, and the replay runner can test a narrow lower-RSI trend-intact variant chronologically with position occupancy and `dynamic_equity_pct` sizing. SV1.16.1 clarifies that `production_rule_*_in_replay_state` fields are Money Flow production-rule evaluations under the active replay state, while legacy `baseline_*` fields are compatibility aliases rather than independent baseline path truth after variant divergence. It also separates production-rule rejection counts from variant no-trade counts and variant-admitted-from-rejection counts. SV1.17 uses the replay substrate for lower-RSI plus market-structure true replay experiments across the full BTC/ETH/SOL x 15m/1h/4h public campaign suite. Each variant is compared against the matching same-symbol/same-component baseline as an independent dynamic-equity scenario; some variants improve losing baselines, but ETH `sleeve_1h` baseline remains the strongest above-starting-equity pocket. SV1.18 closes the current evidence cycle and freezes only Hyperliquid ETH `sleeve_1h` baseline current rules as a UAT observation candidate; UAT is plumbing and behavior validation, not performance validation. These are research results only and authorize no production rule change. Use `same_candle_close_research_only` only as an explicitly optimistic research assumption; `next_candle_open` and `next_candle_close` are available for less optimistic fill timing checks. This is a research/backtest tool only. It is not paper trading, live execution, routing automation, smart routing, best-binding selection, fanout, target reselection, route-executor behavior, or proof of future outcomes.
...
Evidence packs are written under `reports/strategy_validation/<campaign_name>/<run_id>/` by default and include `campaign_config.json`, `manifest.json`, `batch_report.json`, `batch_report.md`, and a pack `README.md`. SV1.4.1 makes this write path collision-safe: the default `--collision-policy unique_suffix` never silently overwrites an existing pack, so a duplicate same-second run writes a suffixed directory such as `<run_id>-001`; `--collision-policy fail_if_exists` is also available for explicit fail-fast workflows. The manifest records the assumptions hash, symbols, components, named windows, fill timings, fees/slippage, window convention, blocked-run counts/reasons, requested run id, final run id, final evidence-pack path, collision policy, collision occurrence/suffix truth, and report paths. Generated evidence packs are ignored by Git and review bundles unless explicitly copied elsewhere for review. This is repeatable research workflow only: it does not change Money Flow rules, optimize parameters, recommend a variant, paper trade, live trade, route, submit, create live artifacts, or call exchange adapters.
...
SV1.5 prepares canonical campaign evidence generation against real persisted candle data. Campaign config `window_convention` metadata is validated against the platform convention and cannot be used to imply a different boundary rule; validation still uses candle closes in `(start_at, end_at]`. SV1.5.1 tightens that validation so contradictory text such as `[start_at, end_at]`, `inclusive start`, or `start included` is rejected instead of treated as harmless display metadata. Audit output can now be rendered as founder-readable Markdown:
...
SV1.9.1 further hardens candle import truth before real evidence. Timezone-naive timestamps are rejected by default with `candle_import_naive_timestamp` / `timezone_required_for_candle_import`; timezone-explicit source data is preferred for canonical evidence. A non-default `--assume-naive-utc` override exists only for explicitly accepted fixture/exploratory research and records `timestamp_assumption=assume_naive_utc`, `naive_timestamp_override_used=true`, source label, input path/name/hash, row counts, imported environment/venue/symbol/timeframe, and warning reason codes in the import summary. Because candle rows do not yet link to a persisted import-batch provenance table, override-derived imports must not be treated as clean canonical evidence without founder/operator acceptance.
...
The local SV1.12.5 run imported all 9 Hyperliquid public campaign files into the intended migrated `money_flow` DB: `25848` candles inserted, `0` updated, `0` unchanged, and no evidence packs generated. SV1.12.5.1 closes the repo/import state by rechecking the intended DB, operator-verified non-trading identity, expected symbol/timeframe candle counts, and no-evidence-pack boundary before SV1.13. See [SV1.12.5 Public Campaign Import Result](docs/strategy_validation_sv1_12_5_public_campaign_import_result.md).

SV1.13 generates the first real Hyperliquid public campaign evidence packs from those imported candles. The evidence review expands `money_flow_hyperliquid_public_ytd_recent.json` into three component-scoped evidence configs because the public campaign has component-specific timeframe windows. It generated local, ignored evidence packs under `reports/strategy_validation/` for `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`, with founder-readable summary at [SV1.13 Hyperliquid Public Evidence Review](docs/strategy_validation_sv1_13_hyperliquid_public_evidence_review.md). SV1.13.1 adds [Hyperliquid Evidence Interpretation](docs/strategy_validation_sv1_13_1_hyperliquid_evidence_interpretation.md), which clarifies that grouped aggregate totals are sums across research runs rather than one account/scenario PnL, shows scenario-level fill/cost/drawdown evidence, documents constant initial-capital notional sizing, and makes ETH `sleeve_1h` concentration explicit. SV1.13.2 adds [Dynamic Equity Evidence](docs/strategy_validation_sv1_13_2_dynamic_equity_evidence.md): `dynamic_equity_pct` sizes each new validation trade from current realized equity after prior closed-trade net PnL while preserving the legacy constant-notional mode as the default. ETH `sleeve_1h` remained above starting equity across the tested dynamic fill/cost assumptions, while 15m and 4h dynamic scenarios ended below starting equity in this public campaign. SV1.14 adds [Trade Anatomy And Market-Structure Diagnostics](docs/strategy_validation_sv1_14_trade_anatomy_and_market_structure.md), which explains current Money Flow readiness/entry/exit logic, confirms entries below the RSI sleeve floor are not allowed, reports ETH 1h / 15m / 4h trade anatomy, and records descriptive recent swing high/low context plus later-test hypotheses without changing rules. SV1.15 adds [Controlled Hypothesis Experiments](docs/strategy_validation_sv1_15_hypothesis_experiments.md), which compares research-only resistance, support-context, invalidation, 15m regime, and 4h extension overlays plus RSI/entry-style attribution against the dynamic-equity baseline. SV1.16/SV1.16.1 add [Rejected-Signal Replay Instrumentation](docs/strategy_validation_sv1_16_rejected_signal_replay.md), which captures per-candle rejected entry context, runs a narrow lower-RSI trend-intact true replay example, and clarifies replay-state production-rule semantics plus variant counter separation without changing production rules. SV1.17 expands true replay experiments across the full BTC/ETH/SOL x 15m/1h/4h public campaign suite and keeps ETH `sleeve_1h` baseline as the strongest above-starting-equity pocket. SV1.18 adds [Evidence Credibility Closeout And UAT Candidate Freeze](docs/strategy_validation_sv1_18_evidence_closeout_and_uat_candidate_freeze.md), which closes the current evidence cycle, freezes only Hyperliquid ETH `sleeve_1h` current baseline rules as a UAT observation candidate, and defines UAT as plumbing and behavior validation rather than performance validation. UAT0 adds [Safety / Security / Runtime Hardening](docs/uat0_safety_security_runtime_hardening.md), UAT0.1 adds [API Auth/Authz And Runtime Lockout](docs/uat0_1_api_auth_runtime_lockout.md), UAT0.2 adds [Adapter Runtime Policy And Redaction](docs/uat0_2_adapter_runtime_policy_and_redaction.md), UAT0.3 adds [Top-20 Universe And Drawdown Readiness](docs/uat0_3_top20_universe_and_drawdown_readiness.md), UAT1 adds [Public Read-Only Connectivity And Top-20 Universe](docs/uat1_public_read_only_connectivity_and_top20_universe.md), UAT1.1 adds [Shadow Signal Audit And Drawdown Readiness](docs/uat1_1_shadow_signal_audit_and_drawdown_readiness.md), UAT2 adds [Shadow Strategy Top-20 Observation](docs/uat2_shadow_strategy_top20_observation.md), UAT2.1 adds [Dashboard Visualization And Approval Readiness](docs/uat2_1_dashboard_visualization_and_approval_readiness.md), UAT3.0 adds [Sandbox Order Design And Approval/Lifecycle Readiness](docs/uat3_0_sandbox_order_design_and_readiness.md), UAT3.0.1 adds [Sandbox Runtime / Approval / Risk Readiness](docs/uat3_0_1_sandbox_runtime_approval_risk_readiness.md), UAT3.0.2 adds [Sandbox Gate Integration Dry-Run](docs/uat3_0_2_sandbox_gate_integration_dry_run.md), UAT3.0.3 adds [Sandbox Gate Wiring And Label Enforcement](docs/uat3_0_3_sandbox_gate_wiring_and_label_enforcement.md), UAT3.0.4 adds [Sandbox Private Read-Only Drawdown](docs/uat3_0_4_sandbox_private_read_only_drawdown.md), UAT3.0.5 adds [Sandbox Private Read-Only Drawdown Verification](docs/uat3_0_5_sandbox_private_read_only_drawdown_verification.md), UAT3.0.6 adds [Sandbox Submit Path Dry-Run Wiring](docs/uat3_0_6_sandbox_submit_path_dry_run_wiring.md), UAT3.1 adds [First Sandbox Order Attempt](docs/uat3_1_first_sandbox_order_attempt.md), UAT3.2 adds [Second Sandbox Order Attempt](docs/uat3_2_second_sandbox_order_attempt.md), UAT3.3 adds [Hyperliquid Account Targeting Precision And Order Attempt](docs/uat3_3_hyperliquid_account_targeting_precision_and_order_attempt.md), UAT3.4 adds [Sandbox Routing Pipeline And Order Ledger](docs/uat3_4_sandbox_routing_pipeline_and_order_ledger.md), UAT4.0 adds [Live UAT Dashboard / Chart Cockpit](docs/uat4_0_live_uat_dashboard_chart_cockpit.md), UAT4.1 adds [Exchange-Style Dashboard Redesign](docs/uat4_1_exchange_style_dashboard_redesign.md), UAT4.2 adds [Live Market Dashboard + Paper-Equity Runtime Monitor](docs/uat4_2_live_market_dashboard_and_paper_equity_monitor.md), PT0 adds [TradingView Charts + Top-20 Paper/Sandbox Runtime Foundation](docs/pt0_tradingview_charts_and_top20_paper_sandbox_runtime.md), PT0.0.1 adds [TradingView Chart Stability Hotfix](docs/pt0_0_1_tradingview_chart_stability_hotfix.md), and PT0.0.2 adds [Historical Strategy Replay Cockpit](docs/pt0_0_2_historical_strategy_replay_cockpit.md). Historical PT0 sandbox/testnet paper-plumbing scope remains audit context and is not production strategy approval. PT0.0.2 uses historical/mainnet public candle replay data as strategy truth and keeps Hyperliquid testnet prices limited to sandbox execution plumbing. Live trading, real-capital trading, live exchange API keys, production auto-submit, SOR/fanout/CBBO/target reselection, cross-venue routing, Money Flow rule changes, strategy optimization, and evidence packs remain not approved. The frozen ETH candidate remains the evidence candidate; top-20 paper/sandbox support is plumbing/runtime eligibility, not proof of future outcomes or strategy profitability. Aster/Binance remain later comparative candidates; OKX/Coinbase/Kraken remain blocked by source-policy gaps.

PT0.0.3 adds [Historical Data Horizon + 1D Replay Support](docs/pt0_0_3_historical_data_horizon_and_1d_readiness.md). The dashboard now prefers `docs/pt0_0_3_historical_strategy_replay_summary.json`, supports 15m/1h/4h/1D historical replay selection, shows Jan 2025 target-start readiness, reports the actual available data horizon, and labels 1D as deterministic aggregation from 4h historical replay candles rather than a new production Money Flow 1D sleeve.

SV2.0 adds [Historical Data Refresh + 1D + Expanded Universe Readiness](docs/sv2_0_historical_data_refresh_1d_and_expanded_universe_readiness.md) and [Money Flow 1D Sleeve + Expanded Universe Evidence Rebuild](docs/sv2_0_money_flow_1d_sleeve_expanded_universe_evidence_rebuild.md). Money Flow v1.2 now includes `sleeve_1d` as a real strategy sleeve alongside `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`; existing 15m/1h/4h settings remain unchanged. The SV2.0 public-mainnet refresh resolves BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX/SHIB through Hyperliquid metadata, maps SHIB explicitly to `kSHIB`, fetches public `candleSnapshot` rows for supported symbols across 15m/1h/4h/1D, and records the Jan 2025 target horizon plus Hyperliquid public 5000-candle limits. It submits no orders, calls no private/signed/order endpoints, uses no API keys, uses no testnet prices as strategy truth, and does not optimize parameters.

The static local evidence dashboard in `apps/dashboard/` visualizes regenerated SV2.0.2 canonical evidence packs plus matching Historical Replay chart/trade JSON in human-readable panels, charts, and ledgers. The visible top-level navigation is Strategy, Historical Replay, Evidence, and Evidence Lab; the invalid legacy Experiments surface is no longer exposed. Evidence Lab loads `docs/sor_ev1_money_flow_trade_loss_anatomy_and_variants_summary.json` and `docs/sor_ev2_true_forward_stop_and_rejected_signal_replay_summary.json` to show methodology-labeled SOR variants, rejection/candidate status, control-pocket impact, worst trades, late-entry classifications, large adverse-candle context, RSI/MACD rejection analysis, and the SOR-EV2.2 baseline-versus-variant overlay. The overlay renders baseline SV2.0.2 entry/exit markers, linkable SOR-EV2 adverse-candle/stop-context markers, worst-trade focus, control-pocket view, and explicit unavailable states when exact variant marker data is missing. The Evidence tab now has `Replay strategy`, `Fill assumption`, and `Period` selectors for the Run Ledger: it can keep the canonical evidence-pack batch-report ledger, switch to generated SV2.0.2 Historical Replay strategy rows, or inspect the current SV2.1 founder-approved 1D period evidence rows across 2024, 2025, YTD, and ALL when ignored local packs are present. Generated replay rows include a `Result` label comparing PnL and drawdown to the matching Money Flow v1.2 canonical row: `improved_pnl_drawdown`, `improved_pnl_not_drawdown`, `improved_drawdown_not_pnl`, `same_result`, or `no bueno`. The Historical Replay tab loads SV2.0.2 dashboard chart data from ignored `reports/strategy_validation/sv2_0_2_dashboard_chart_data/20260512T064916Z/` and SV2.1 founder-approved 1D selected chart/trade JSON from ignored `reports/strategy_validation/sv2_1_broad_1d_dashboard_chart_data/20260516T091500Z/` when present, with fallback to PT0.0.3/PT0.0.2 summary JSON. The current SV2.1 founder-approved rebuild targets BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, AVAX, TRX, ADA, ZEC, LINK, XMR, TON, LTC, UNI, DOT, ASTER, AAVE, POL, FIL, and TRUMP, and now covers all 10 PT-RT1 lanes; PEPE/kPEPE, OKB, and SHIB/kSHIB are hidden/deferred because unit/support semantics are not clean for founder review. Chart arrows and trade table rows select the linked trade in a polished inspector with entry/exit reasons, indicator tiles, costs, PnL, and equity movement; arrow descriptions are disabled by default for cleaner chart review. Historical replay uses historical public candle replay data as strategy truth; Hyperliquid testnet prices are not strategy truth. The hidden legacy UAT Chart Cockpit loads committed UAT2, UAT3.4, UAT4.2, and PT0 summary JSON. PT0 replaces the custom chart shell with the official local TradingView Lightweight Charts standalone bundle and renders Hyperliquid testnet public candlesticks, volume, EMA5/EMA10/SMA20 overlays, RSI/MACD labels, green/red paper/sandbox markers, top-20 paper eligibility, sandbox balance-poll policy, internal 10,000 USDC paper-equity state, and routed-order / paper-trade blotter rows. Browser live charting polls Hyperliquid testnet public `allMids` and `candleSnapshot` every 15 seconds and falls back to committed local JSON if public polling is unavailable. PT0.0.1 stabilizes this chart by using an explicit bounded chart height, reusing the existing chart and series handles across refreshes, removing the autosize feedback loop, limiting `fitContent()` to new symbol/timeframe initialization, and adding `?disableLivePolling=true` / `?livePolling=false` fallback flags. The hidden legacy UAT2 panel loads `docs/uat2_shadow_strategy_top20_observation_summary.json`, shows the 45-record signal matrix, would-open records, no-trade reason breakdowns, ETH evidence candidate status, timing assumptions, not-live-account shadow drawdown, boundary flags, the blocked UAT3 readiness checklist, and the UAT3.0/UAT3.0.1/UAT3.0.2/UAT3.0.3 sandbox-design/readiness status panel. Run it from the repo root with `.venv/bin/python -m http.server 8765`, then open `http://127.0.0.1:8765/apps/dashboard/index.html`. It is a visualization and PT0 paper/sandbox monitoring surface only: it does not generate evidence packs, import candles, approve live trading, call live endpoints, call private order endpoints, create approvals, add order controls, submit orders, or change Money Flow rules.
