...
PT-RT1 adds the [real-time paper-observation and testnet-plumbing substrate](docs/pt_rt1_real_time_paper_observation_and_testnet_plumbing.md). The strategy-truth lane is public Hyperliquid mainnet market data only, evaluates fully closed candles, computes real-time indicators, and maintains independent synthetic 10,000 USDC ledgers. PT-RT1.1A expands the lab before the 24-hour run to exactly 10 lanes: Money Flow v1.2 baseline, two SOR rolling-range candidate lanes, four MF-ORIG full-equity reference lanes, and three wildcard expert observation hypotheses. It also adds founder-requested scanner symbols, requested/resolved symbol handling, TRON->TRX and PEPE->kPEPE aliases, blocked-symbol reason codes, and dashboard lane/wildcard diagnostics. The testnet plumbing lane is separate, disabled/kill-switched by default, requires exact approval, and never updates strategy paper PnL. The dashboard now has a Paper Observation tab, plus dry-run, testnet-probe, and 60-day observation runbooks. PT-RT1.1 checked for the first probes-disabled 24-hour runtime artifact set and is blocked because `reports/paper_runtime/pt_rt1_1_24h_dry_run/` is absent; PT-RT1.1B may start that runtime collection after PT-RT1.1A readiness review, while PT-RT1.2 cannot proceed until a real dry run is executed and summarized. PT-RT1/PT-RT1.1/PT-RT1.1A are not production approval, not live approval, and not strategy paper-runtime authorization.
