...
PT-RT1 adds the [real-time paper-observation and testnet-plumbing substrate](docs/pt_rt1_real_time_paper_observation_and_testnet_plumbing.md). The strategy-truth lane is public Hyperliquid mainnet market data only, evaluates fully closed candles, computes real-time indicators, and maintains independent synthetic 10,000 USDC ledgers. PT-RT1.1A expands the lab before the 24-hour run to exactly 10 lanes: Money Flow v1.2 baseline, two SOR rolling-range candidate lanes, four MF-ORIG full-equity reference lanes, and three wildcard expert observation hypotheses. It also adds founder-requested scanner symbols, requested/resolved symbol handling, TRON->TRX and PEPE->kPEPE aliases, blocked-symbol reason codes, and dashboard lane/wildcard diagnostics. PT-RT1.1B adds the [Hyperliquid public-mainnet connector/runtime readiness](docs/pt_rt1_1b_hyperliquid_live_market_data_and_runtime_readiness.md), smoke-verifies public `meta` / `allMids` connectivity, resolves the expanded watchlist, records bounded paper decisions, and adds dashboard connection status. PT-RT1.1C wrote ignored local artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`; the current local artifact set contains about 479k paper decision rows, 0 trade rows, and latest summary timestamp `2026-05-15T22:22:12Z`. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping the process and machine awake/networked. The runtime now defaults to `--decision-log-mode compact`, which writes actionable `paper_opened`/`paper_closed` decisions, data-unavailable rows, and first-seen non-actionable audit context while suppressing repeated identical non-actionable rows across cycles; `full_audit` remains an explicit CLI mode for short diagnostic runs. The Paper Observation dashboard can now be served with `scripts/run_dashboard_control_server.py` to expose a local-only Start Run / Stop Run panel that launches allowlisted 5-minute, 1-hour, 6-hour, or 24-hour PT-RT1 runs through Mac `caffeinate` while forcing `--enable-testnet-probes`, `--founder-approved-testnet-probes-20usdc`, `--testnet-probe-notional-usdc 20`, `--public-mainnet-only`, and compact decision logging. Those testnet probes are currently audit/order-shape rows only: the PT-RT1 runtime records 20 USDC probe intent shapes and explicitly records `order_endpoint_called=false` / `signed_order_endpoint_called=false`; no signed transport submission is performed by this runtime. Static `http.server` review still works, but its Start/Stop controls remain unavailable because no local control API is present. The Paper Observation dashboard also browser-polls Hyperliquid public mainnet `allMids` every 1 second for a compact symbol / mid price / health watchlist; health becomes `unhealthy` when the latest market-data tick is missing or stale for more than 2 minutes. The selected pair uses public `candleSnapshot` for a live TradingView chart, and the adjacent Signal Generation panel lists recorded synthetic `paper_opened` intended-entry decisions. The dashboard surfaces decision-log mode, size, written rows, and suppressed rows from runtime summaries. The testnet plumbing lane is separate, exact-approval gated, capped at 20 USDC per signal in the dashboard-started runtime, and never updates strategy paper PnL. The dashboard now has a Paper Observation tab, plus dry-run, testnet-probe, and 60-day observation runbooks. PT-RT1.1D evaluates artifacts after completion. PT-RT1/PT-RT1.1/PT-RT1.1A/PT-RT1.1B/PT-RT1.1C are not production approval, not live approval, and not strategy paper-runtime authorization.
