...
PT-RT1 adds the [real-time paper-observation and testnet-plumbing substrate](docs/pt_rt1_real_time_paper_observation_and_testnet_plumbing.md). The strategy-truth lane is public Hyperliquid mainnet market data only, evaluates fully closed candles, computes real-time indicators, and maintains independent synthetic 10,000 USDC ledgers for the baseline and three evidence-only candidate/reference lanes. The testnet plumbing lane is separate, disabled/kill-switched by default, requires exact approval, and never updates strategy paper PnL. The dashboard now has a Paper Observation tab, plus dry-run, testnet-probe, and 60-day observation runbooks. PT-RT1 is not production approval, not live approval, and not strategy paper-runtime authorization.
