...
MF-ORIG-EV1 adds an evidence-only [Original Money Flow reconstruction](docs/mf_orig_ev1_original_money_flow_reconstruction.md) plus a [source specification and gap matrix](docs/mf_orig_ev1_original_money_flow_spec_and_gap_matrix.md). It reconstructs the prompt-provided Gerald Peters Money Flow source hierarchy separately from production Money Flow v1.2, treating `1d` as the primary original timeframe and modeling Stage 1-4, 5 EMA / 20 SMA triggers, RSI profit warnings, MACD-as-TSI substitute confirmation/warnings, prior support/pivot stop proxies, and 1% risk-based sizing. The source PDF was not present in the repo or common local document paths during this pass, so the report explicitly labels the source limitation. Production Money Flow v1.2 remains unchanged; no original hypothesis is approved for production, paper runtime, or live trading.
