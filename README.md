...
SOR-EV2.1 adds the [Evidence Lab / Variant Review dashboard](docs/sor_ev2_1_evidence_lab_ui.md). The dashboard loads the committed SOR-EV1/SOR-EV2 summaries, labels canonical SV2.0.2 DB-imported evidence as the baseline, and shows variant matrix, control pockets, worst trades, late-entry, large adverse-candle, and RSI/MACD rejection panels. The Variant Summary Matrix uses founder-review labels so promising, mixed, deferred, no-op, diagnostic-only, and hard-rejected outcomes are separated instead of flattening every non-candidate into rejected. It is UI/visualization only: variants remain evidence-only, no production Money Flow rules changed, no orders were submitted, live trading is not approved, testnet execution remains separate from strategy evidence, and dashboard date filters are display-only rather than canonical evidence.
