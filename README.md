...
Current Obsidian entrypoints are the canonical command center, current phase note, decision log, coordination note, project memory, and the relevant track map. After OB1.0, Strategy Validation is represented as its own track. UAT0 completed a safety/security/runtime audit, UAT0.1 closes the P0 API auth/authz baseline for sensitive `/api/v1` routes plus an inspectable fail-safe runtime policy, UAT0.2 closes the adapter-level runtime-policy baseline with private/signed/order guards before transport plus representative bearer/API-key/secret/password/DB URL redaction tests, and UAT0.3 adds fixture-only top-20 observation-universe resolution, Hyperliquid public read-only info-type allowlisting, and a runtime drawdown monitor model. UAT1 public read-only connectivity is complete: it verified explicit no-key public Hyperliquid endpoint behavior, fetched a no-key public top-volume source, and resolved observation-only Hyperliquid top-20 candidates without private/signed/order endpoints or strategy execution. UAT1.1 adds model/report-only shadow signal audit, operator-visible shadow drawdown, UAT1 universe snapshot loading, and representative structured API-error/log redaction verification. UAT2 completes a bounded no-order public-read-only Money Flow shadow observation across the UAT1 top-20 universe and emits only shadow audit/report artifacts. UAT2.1 adds dashboard visualization and an informational founder-readiness panel for UAT3 scoping review. UAT3.0 through UAT3.0.6 progressively define and dry-run the sandbox-order gate chain. UAT3.1 is complete: exact founder/operator approval was verified, one Hyperliquid testnet ETH post-only limit order attempt under 10 USDC notional was made, Hyperliquid rejected it with a sanitized user/API-wallet-not-found response, no cancel was required, and reconciliation found no open order. UAT3.2 is complete as a blocked fixed-key preflight. UAT3.3 fixed account targeting/precision and later verified a successful Hyperliquid testnet ETH post-only lifecycle follow-up. UAT3.4 operationalizes that success into a fixed-target sandbox routing pipeline and routed-order ledger. UAT4.0 through UAT4.2 add read-only dashboard/chart, public market monitor, and internal paper-equity visibility. PT0 integrates TradingView Lightweight Charts and the top-20 paper/sandbox runtime foundation; PT0.0.1 fixes the P0 chart stability issue; PT0.0.2 adds a Historical Replay cockpit using historical public candle replay data, not Hyperliquid testnet prices, as Money Flow strategy truth; PT0.0.3 adds aggregated 1D replay/data-horizon visibility. SV2.0 now supersedes that dashboard-only 1D state by adding `sleeve_1d` as a real Money Flow v1.2 sleeve, preserving existing 15m/1h/4h settings, resolving BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX/SHIB through Hyperliquid public mainnet metadata, fetching public mainnet `candleSnapshot` rows for 15m/1h/4h/1D where available, and writing compact readiness/evidence summaries without using testnet data as strategy truth. UAT3.5 remains blocked because testnet metadata still lacks precision validation for some UAT observation-universe symbols. UAT3.1-SV2.0 created no production `OrderIntent`, `PreparedVenueOrder`, `SubmittedOrder`, executable approval, live behavior, broad top-20 live order submission, routing expansion, order endpoint call, live endpoint use, private/signed/order dashboard call, exchange API-key use, dashboard order control, or unapproved repeated order. Paper trading is approved only for Hyperliquid testnet/sandbox; live trading and real-capital trading are not approved. The key maps are [Current State Dashboard](money-flow/00%20Maps/Current%20State%20Dashboard.md), [Strategy Validation Map](money-flow/00%20Maps/Strategy%20Validation%20Map.md), and [UAT Roadmap](money-flow/00%20Maps/UAT%20Roadmap.md). Do not create duplicate command centers or competing current-phase notes.
