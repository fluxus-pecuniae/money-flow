...
Current Paper Trading status: PT-RT1.5 resets the active Week 1 runtime scope to `pt_rt1_5_week1_active`, keeps 15m paused, evaluates strategy signals only after fully closed 1h / 4h / 1d candles, and allows Hyperliquid testnet order lifecycle plumbing only from `money_flow_v1_2_baseline` synthetic opens with a fixed 25 USDC notional. Public mainnet candles remain strategy truth, candidate lanes remain synthetic-only, testnet fills never update synthetic PnL, no strategy is production-approved, and live trading is not approved.
