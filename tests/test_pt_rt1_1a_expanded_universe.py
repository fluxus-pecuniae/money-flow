...
def test_pt_rt1_1a_existing_summary_is_dashboard_ready() -> None:
    ...
    assert summary["latest_readiness_phase"] == "PT-RT1.5.1"
    assert summary["dashboard_status"]["public_mainnet_connection_status_visible"] is True
    assert summary["next_phase"]["decision"] == "Run pt_rt1_5_1_smoke for one hour, then review warm-start, MTM, and signed-testnet lifecycle behavior before Week 1 observation"
