...
def test_obsidian_brain_workflow_exists() -> None:
    ...
    assert re.search(r"Current implemented phase: `SV1\.12\.2`", current_phase)
    assert "SV1.12.2" in command_center
...
def test_obsidian_current_state_notes_do_not_have_stale_current_truth() -> None:
    ...
    for path in current_state_paths:
        ...
        assert "SV1.12.2" in contents, f"{path} does not reflect current SV1.12.2 work"
