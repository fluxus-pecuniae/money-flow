...
def runtime_dir(root: Path, scope: str = "pt_rt1_6_week2_active") -> Path:
    return root / "reports" / "paper_runtime" / scope


def base_summary() -> dict:
    return {
        "status": "week2_founder_selected_slate_cycle_verified",
        "active_review_scope": "pt_rt1_6_week2_active",
...
def test_obs_os1_generates_daily_review_and_outputs(tmp_path: Path) -> None:
    ...
    write_jsonl(
        ...
        [{"exit_time": "2026-06-08T11:30:00Z", "lane_id": "money_flow_v1_2_baseline", "symbol": "ETH", "net_pnl": "20"}],
    )
    write_jsonl(
        scope_dir / "testnet_order_lifecycle.jsonl",
        [
            {
                "time": "2026-06-08T11:02:00Z",
                "trigger_lane": "money_flow_v1_2_baseline",
                "status": "reconciled",
    ...
    assert review["week2_truth"]["active_timeframes"] == ["1h", "4h", "1d"]
    assert review["week2_truth"]["disabled_timeframes"] == ["15m"]
    ...
    json_path, md_path = obs.write_review(review, tmp_path / "reports" / "paper_reviews" / "pt_rt1_6_week2_active")
