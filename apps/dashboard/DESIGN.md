...
- DASH-PT3 layout: the header carries always-visible critical-state pills (RUNTIME ACTIVE/IDLE driven by the local control-server status the dashboard already polls; LIVE DISABLED · NOT APPROVED, static red). The Paper Trading body leads with the full-width Cockpit / Global Filters bar, then the terminal grid (wider Watchlist left rail ~212-252px; dominant center chart; wider Runtime Control right rail ~300-344px sized so its content never truncates), then the tabbed blotter, the full-height Daily Review / Anomaly Flags card (no nested scroll), the full-width Testnet Order Transport footer, and finally the dense status strip (#paper-observation-health-banner) as the bottom reference band with all lane chips and safety labels intact. Hidden symbols (e.g. OKB) are excluded from the watchlist display only; backend resolver policy unchanged. The chart and its markers remain untouched.
