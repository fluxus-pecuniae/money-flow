...
The visible top-level navigation is `Experiments`, `Historical Replay`, `Evidence`, and `Strategy`. `Historical Replay` is the default view. The former `UAT Chart Cockpit` and `UAT2 Shadow Run` surfaces remain as hidden legacy panels for regression coverage and historical context, but they are no longer exposed as primary tabs.
