...
The `UAT Chart Cockpit` tab loads `docs/uat2_shadow_strategy_top20_observation_summary.json` and `docs/uat3_4_sandbox_routing_pipeline_and_order_ledger_summary.json` by default when served from the repo root. It shows the UAT watchlist, market-data coverage, a static local chart snapshot, EMA5 / EMA10 / SMA20 / RSI / MACD labels, green shadow/sandbox-entry markers, red sandbox-cancel markers, the active fixed-target ETH sandbox route card, unified-mode equity-source visibility, routed-order ledger filters, and shadow-signal overlays. UAT4.0 uses committed local JSON summaries only; live public read-only refresh is deferred to a later phase.
