...
The visible top-level navigation is `Strategy`, `Historical Replay`, `Evidence`, and `Evidence Lab`. `Historical Replay` is the default view. The invalid legacy `Experiments` surface is not exposed as a tab. The former `UAT Chart Cockpit` and `UAT2 Shadow Run` surfaces remain as hidden legacy panels for regression coverage and historical context, but they are no longer exposed as primary tabs.

The `Evidence` tab has a `Replay strategy` selector for the Run Ledger. The default option keeps the canonical evidence-pack batch-report ledger. When regenerated SV2.0.2 / MF-ORIG-EV2 chart-data JSON is present, the same selector can switch the Run Ledger to generated Historical Replay strategy rows, including `SV2.0.2 canonical Money Flow v1.2`, `SOR-EV3 avoid low rolling range 20`, `SOR-EV3 avoid low rolling range 50`, and the four MF-ORIG-EV2 Original Money Flow hypotheses. Generated replay rows include a `Result` column comparing PnL and drawdown to the matching Money Flow v1.2 canonical row for the same symbol/timeframe/fill: green `improved_pnl_drawdown`, amber `improved_pnl_not_drawdown`, amber `improved_drawdown_not_pnl`, neutral `same_result`, or red `no bueno`. This is a dashboard review switch only; date filters are display-only and do not regenerate canonical evidence packs or approve any variant.
