...
The `UAT2 Shadow Run` tab loads `docs/uat2_shadow_strategy_top20_observation_summary.json` by default when served from the repo root. It shows UAT2 summary cards, a filterable 45-record signal matrix, would-open records, no-trade reason breakdowns, the ETH `sleeve_1h` evidence-candidate card, `next_candle_open` / `next_candle_close` timing status, the `same_candle_close_research_only` research-only boundary, shadow drawdown labeled not-live-account, no-artifact boundary flags, the blocked UAT3 readiness checklist, and the UAT3.0/UAT3.0.1/UAT3.0.2 sandbox-design/readiness panel. It is a visualization and founder-review surface only; it adds no approval action and cannot enable orders.

The UAT3.0/UAT3.0.1/UAT3.0.2 panel is informational. It shows that the future initial sandbox subset is Hyperliquid ETH USDC perpetual `sleeve_1h`, actual sandbox order submission is not approved, founder/operator approval is required for any later UAT3.1 submission, sandbox runtime policy and fixture validators exist, unified dry-run preflight exists, runtime full-blocker propagation and numeric edge-case validation are implemented, sandbox account drawdown is fixture-only / missing live sandbox feed, artifact-label persistence enforcement is missing, and submit/risk/approval wiring is still required. It has no active order submission button.
