...
The `Historical Replay` tab loads `docs/pt0_0_3_historical_strategy_replay_summary.json` first and falls back to `docs/pt0_0_2_historical_strategy_replay_summary.json`. It uses historical public candle replay data as Money Flow strategy truth for BTC/ETH/SOL across 15m/1h/4h/1D, not Hyperliquid testnet prices. It renders historical candlesticks through TradingView Lightweight Charts, overlays EMA5/EMA10/SMA20 on the price pane, shows RSI 14 and MACD in separate TradingView panes inside the same chart/crosshair timeline as the candles, places green entry-fill and red exit-fill markers, shows a trade inspector with RSI/MACD/cost/PnL/equity details, displays a dynamic 10,000 USDC paper-equity panel, compares BTC/ETH/SOL replay scenarios, shows a Jan 2025 data-horizon panel, and keeps the UAT3.4 sandbox execution ledger in a separate section. The replay-strategy dropdown supports `OG replay / strategy` and `MACD removed`; the MACD-removed option is research-only and does not change production Money Flow rules. The `1D` view is explicitly aggregated from `4h` historical replay candles and does not create a production Money Flow 1D sleeve. It calls no private/signed/order endpoints, uses no API keys, submits no orders, and changes no Money Flow rules.
