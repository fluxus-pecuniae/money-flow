...
| Indicator overlays | implemented | EMA5, EMA10, and SMA20 render on the price chart. RSI 14 and MACD now render inside the same TradingView chart on the same time scale as the candles, using bounded vertical indicator bands; RSI/MACD values also remain available in the trade inspector/export. MACD values are inspectable even when the MACD-removed replay variant does not use MACD as a gate. |
...
| Advanced RSI/MACD interactions | deferred | RSI and MACD are time-aligned with candles in the same chart; click-to-select directly from indicator bands can be added later. |
