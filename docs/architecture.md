...
- block a concurrent explicit submit for the same child intent with `submission_in_progress` before adapter submission, or return existing submitted-order truth when a `SubmittedOrder` already exists
