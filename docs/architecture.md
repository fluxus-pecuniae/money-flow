...
- `apps.api`: also exposes routing assessment, target-choice creation/inspection, explicit target-choice conversion endpoints, existing child-intent preview/readiness endpoints for converted routed child-intent inspection, and the narrow Phase 7.2/7.3/7.4 approval-consuming action endpoints for recommendation acceptance, target-choice conversion, and preview/readiness inspection.
