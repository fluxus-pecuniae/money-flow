...
- Strategy layer: Money Flow v1.2, including `sleeve_15m`, `sleeve_1h`, `sleeve_4h`, and `sleeve_1d`.
- Historical evidence layer: Strategy Validation packs, with SV2.0.2 as the canonical multi-timeframe baseline.
- Research variant layer: SOR/MF-ORIG/STRAT evidence and overlays, none production-approved.
- Forward observation layer: PT-RT synthetic paper ledgers from public mainnet candles.
- Testnet plumbing lane: Hyperliquid testnet lifecycle checks, separate from strategy truth and synthetic PnL.
- Execution/routing substrate: existing gated, lineage-bound platform services; not current strategy approval or live trading authority.
- Dashboard layer: founder visualization and local runtime control, not evidence generation or order entry.

## Strategy / Evidence / Runtime Separation

- `Evidence` and backend Strategy Validation reports answer what historical tests produced.
- `Historical Replay` renders candles, indicators, markers, and trades for visual inspection.
- `The Lab` renders SOR/MF-ORIG research variants and methodology warnings.
- `Paper Trading` observes current public-mainnet market conditions with synthetic ledgers.
- Dashboard date filters are display-only recalculations and never canonical evidence-pack regeneration.

## Safety Boundaries

- No live trading is approved.
- No strategy is production-approved.
- Public mainnet data is strategy truth for PT-RT.
- Hyperliquid testnet is plumbing/lifecycle validation only.
- Testnet fills never update synthetic PnL.
- Candidate, MF-ORIG, and wildcard lanes cannot send testnet orders.
- No SOR/fanout/CBBO/target-reselection expansion is part of the current docs refresh.
...
UAT0 completes a safety/security/runtime readiness audit at the architecture boundary. It adds no exchange connectivity and makes no exchange calls. UAT0.1 adds scoped bearer authentication/authorization for sensitive `/api/v1` control-plane routes and an inspectable `RuntimeSafetyPolicy` with fail-safe defaults: paper trading, live trading, exchange order submission, and private exchange endpoints are disabled unless a later explicit phase enables and verifies them. UAT0.2 adds adapter-level runtime-policy guards before private/signed/order transport, classifies public read-only adapter methods, defines a Hyperliquid future-UAT1 read-only allowlist artifact, and strengthens representative redaction tests. UAT0.3 adds fixture-only top-20 observation-universe resolution policy, Hyperliquid public read-only info-type classification for `meta`, `metaAndAssetCtxs`, `allMids`, `l2Book`, `candleSnapshot`, and `fundingHistory`, plus a fixture-tested runtime drawdown monitor model. UAT1 verifies actual public-read-only Hyperliquid endpoint behavior and public no-key top-volume source ingestion under explicit `--uat1-public-read-only --allow-public-read-only-network` gating. It resolves observation-only Hyperliquid supported assets from the public top-20 source and creates a compact report/summary, but it still does not call private/signed/order endpoints, use API keys, submit orders, add paper/live behavior, add routing behavior, run Money Flow live, create strategy/execution artifacts, generate evidence packs, or change Money Flow rules. UAT1.1 adds model/report-only shadow signal audit records, UAT2 timing assumption representation, no-live-artifact boundary checks, UAT1 universe snapshot loading, operator-visible shadow drawdown state, and representative API-error / structured-log redaction verification. UAT2 adds an explicit bounded no-order shadow runner using the UAT1 observation universe and public Hyperliquid `candleSnapshot` data. It evaluates current baseline Money Flow rule logic without creating `StrategyDecision`, `SignalEvent`, `OrderIntent`, `PreparedVenueOrder`, `ExecutionReadinessAssessment`, `SubmittedOrder`, approval, routing, paper, or live artifacts. UAT2 shadow timing compares `next_candle_open` and `next_candle_close`; `same_candle_close_research_only` remains research-only. UAT2.1 adds static dashboard visualization for the UAT2 summary JSON, including signal matrix filters, would-open/no-trade review, ETH candidate status, timing/drawdown cards, boundary flags, and an informational UAT3 blocked readiness panel. UAT3.0 through UAT3.0.6 define and dry-run the sandbox/testnet gate chain without creating production artifacts. UAT3.1 makes one approved ETH testnet attempt that is safely rejected by venue account truth; UAT3.2 blocks before transport on fixed-key readiness; UAT3.3 fixes Hyperliquid account-targeting and tick/lot precision, then records a later successful founder-approved ETH testnet follow-up with normal user account mode, `vaultAddress` omitted, accepted/open order, successful cancel, and no-open-order reconciliation. UAT3.4 adds a production-like fixed-target sandbox route and routed-order ledger through `services/uat/sandbox.py`, `services/uat/sandbox_order.py`, and `scripts/run_uat34_sandbox_routing_pipeline.py`: the active route is Hyperliquid testnet ETH only, standard perp clearinghouse equity is selected for the current account, unified/portfolio spot-clearinghouse USDC fallback remains implemented/tested, route validation rejects non-ETH/top-20/fanout/SOR/target-reselection/route-executor behavior, and the dashboard shows routed-order ledger status without order controls. The approved UAT3.4 run made exactly one ETH post-only testnet attempt under the 20 USDC cap, received accepted/open, canceled successfully, and reconciled to no open orders. UAT4.0 adds a read-only static UAT Chart Cockpit dashboard tab sourced from committed local UAT2 and UAT3.4 JSON summaries. UAT4.1 rebuilds that cockpit into an exchange-style static workstation with compact top bar, observation-only left market rail, central chart cockpit, right order-book/market/signal/risk rail, bottom blotter tabs, and a canonical dashboard design system at `apps/dashboard/DESIGN.md`. UAT4.2 adds `services/uat/live_monitor.py`, a local refresh summary, public-read-only market monitor data, deterministic indicator snapshots, paper-observation scanner records, a 60-second sandbox private-read-only balance polling policy, and an internal 10,000 USDC paper-equity ledger. PT0 is implemented as the Hyperliquid testnet/sandbox paper runtime foundation: TradingView Lightweight Charts, top-20 paper/sandbox universe eligibility, current-equity sizing policy, paper scanner records, and default-disabled risk-gated sandbox route candidates are wired without live endpoint use. UAT3.5 remains blocked by incomplete UAT-universe precision validation for testnet-unsupported observation symbols. UAT3.1-PT0 create no production `OrderIntent`, `PreparedVenueOrder`, `SubmittedOrder`, executable approval, live real-capital behavior, live broad top-20 submission, smart routing/SOR/fanout/CBBO/target reselection, Money Flow rule change, evidence pack, live endpoint use, private order dashboard call, exchange API-key use, or unapproved repeated order.

PT0 supersedes the prior PT0 roadmap state as historical sandbox/testnet plumbing scope. It allowed Hyperliquid testnet/sandbox paper-plumbing experiments under metadata, precision, risk, lease, label, and no-live gates; it did not approve a production strategy or live trading. PT0 adds `services/uat/pt0_runtime.py`, `scripts/refresh_pt0_runtime_summary.py`, the committed PT0 summary JSON, and a local official TradingView Lightweight Charts bundle. PT0.0.2 adds `services/strategy_validation/historical_replay.py`, `scripts/refresh_pt002_historical_replay_summary.py`, `docs/pt0_0_2_historical_strategy_replay_summary.json`, and a dashboard Historical Replay tab so BTC/ETH/SOL historical public candle replay data, not Hyperliquid testnet prices, is strategy truth for visual Money Flow entry/exit validation. PT0.0.3 adds `scripts/refresh_pt003_historical_replay_summary.py`, `docs/pt0_0_3_historical_strategy_replay_summary.json`, and `docs/pt0_0_3_historical_data_horizon_and_1d_readiness.md`; it extends replay coverage to 1D by deterministic 4h aggregation and reports Jan 2025 target-start readiness without creating a production Money Flow 1D sleeve in that phase. SV2.0 now adds `sleeve_1d` as a real Money Flow v1.2 sleeve, adds `services/strategy_validation/sv2.py`, `scripts/run_sv20_historical_refresh.py`, `configs/strategy_validation/campaigns/money_flow_sv2_0_hyperliquid_public_2025_expanded.json`, and compact SV2.0 readiness/evidence summaries for BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX/SHIB across 15m/1h/4h/1D using Hyperliquid public mainnet `candleSnapshot`. SV2.0.1 hotfixes that architecture boundary: internal timeframe is `1d` with dashboard label `1D`, public close slots are normalized to exact timeframe boundaries, staged public data cannot masquerade as DB-imported canonical data, compact rows are non-canonical, final open positions are force-closed/MTM-accounted, missing indicators invalidate evaluation rather than defaulting to zero, and sleeve allocations are 0.25 each. SV2.0.2 adds `scripts/run_sv202_canonical_import_and_evidence.py`, imports normalized Hyperliquid public mainnet candles through the hardened importer into the intended `money_flow` DB, writes 36 per-symbol/per-timeframe canonical SV2.0.2 campaign configs under `configs/strategy_validation/campaigns/sv2_0_2/`, generates ignored canonical evidence packs for Money Flow v1.2 across 15m/1h/4h/1d, records fully closed effective end boundaries, and uses each supported pair/timeframe's full available DB-imported window rather than a common clipped window. Status truth is recorded in `docs/sv2_0_2_canonical_sv2_evidence_packs.md`. PT0 keeps sandbox order routing default-disabled by `PT0_SANDBOX_ORDER_ROUTING_ENABLED=false`; live trading, real-capital trading, live exchange API keys, production auto-submit, SOR/fanout/CBBO/target reselection, cross-venue routing, parameter optimization, and live endpoint use remain not approved.
...
- `apps.dashboard`: static local founder/operator review surface for Strategy Validation evidence packs, Strategy Validation experiment/replay summaries, UAT2 shadow observation summaries, UAT3.4 routed sandbox order ledger summaries, and UAT4.2 live-monitor summary JSON. It loads ignored local evidence-review JSON and component batch reports from `reports/strategy_validation*`, committed replay/summary JSON under `docs/`, or accepts manual JSON file selection in the browser. UAT2.1 adds a UAT2 Shadow Run tab for read-only visualization of `docs/uat2_shadow_strategy_top20_observation_summary.json`; UAT3.0 adds an informational sandbox-order design/readiness panel to that view; UAT3.0.1 updates it with fixture/readiness validator status; UAT3.0.2 adds unified dry-run preflight, full runtime blocker propagation, numeric edge-case validation, and artifact-label persistence blocker status; UAT3.0.3 updates it with boundary-label enforcement and dry-run executable gate wiring status; UAT3.4 adds routed-order ledger visibility from `docs/uat3_4_sandbox_routing_pipeline_and_order_ledger_summary.json`; UAT4.2 adds public-read-only market monitor, indicator, paper-equity, and sandbox balance-poll visibility from `docs/uat4_2_live_market_dashboard_and_paper_equity_monitor_summary.json`. The dashboard creates no evidence packs, imports, approvals, exchange calls, routing/execution artifacts, order intents, submitted orders, order controls, or strategy-rule changes.
- `apps.dashboard.pt0`: PT0 extends the dashboard with the official local TradingView Lightweight Charts bundle and `docs/pt0_tradingview_charts_and_top20_paper_sandbox_runtime_summary.json`. It shows Hyperliquid testnet public candlesticks, volume, EMA overlays, green/red paper/sandbox markers, top-20 paper eligibility, internal 10,000 USDC paper-equity state, PT0 runtime limits, and routed-order / paper-trade rows. It still creates no evidence packs, imports, approvals, exchange calls, order controls, submitted orders, live behavior, or strategy-rule changes.
- `apps.dashboard.pt0_historical_replay`: PT0.0.2 adds the Historical Replay cockpit backed by `docs/pt0_0_2_historical_strategy_replay_summary.json`. PT0.0.3 makes the dashboard prefer `docs/pt0_0_3_historical_strategy_replay_summary.json`, adds 1D timeframe selection, and shows target-start/source/coverage/aggregation warnings in a data-horizon panel. It renders BTC/ETH/SOL x 15m/1h/4h/1D historical candles, EMA overlays, historical entry/exit markers, trade inspector, dynamic 10,000 USDC equity panel, comparison table, and a separate sandbox execution ledger. Hyperliquid testnet market data is not used as strategy truth in this view.
