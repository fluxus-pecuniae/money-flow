...
- `apps.dashboard`: static local founder/operator review surface for Strategy Validation evidence packs. It loads ignored local evidence-review JSON and component batch reports from `reports/strategy_validation*`, or accepts manual JSON file selection in the browser, and creates no evidence packs, imports, approvals, exchange calls, routing/execution artifacts, or strategy-rule changes.
