...
SV2.0.2 is the canonical evidence baseline: DB-imported Hyperliquid public-mainnet candles, BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, 36 packs, 72 rows, dynamic equity, 10000 USDC per independent scenario, and next_candle_open / next_candle_close fills. SV2.1 adds a separate broad 1D-only public Hyperliquid period refresh for founder review: 183 active public metadata symbols, available 1D candles back to 2024-01-01 where possible, and 646 ignored baseline packs across 2024, 2025, YTD, and ALL. It also adds ignored Historical Replay chart/trade JSON for those 1D period sets and evidence-only conservative candidate packs for `avoid_low_rolling_range_50`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. SV2.1 does not supersede the SV2.0.2 multi-timeframe canonical baseline unless a later phase explicitly adopts it. Dashboard Historical Replay, Evidence Lab overlays, and date filters are display-only and are not canonical evidence-pack regeneration. PT-RT1 is implemented as a real-time public-market paper-observation substrate, but it is not production approval, live approval, or strategy paper-runtime authorization. PT-RT1.1A expands the paper-observation lab to 10 synthetic lanes, a founder-requested scanner universe, requested/resolved symbol handling, blocked-symbol reason codes, and wildcard diagnostics. PT-RT1.1B adds the public-mainnet connector/runtime command, smoke-verifies Hyperliquid public `meta` and `allMids`, resolves the expanded watchlist, records bounded paper decisions, and prepares PT-RT1.1C. PT-RT1.1C starts the actual 24-hour probes-disabled collection under ignored `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`; PT-RT1.1D evaluates artifacts after completion.
...
SV2.1 broadens the 1D research evidence surface without changing strategy rules. It fetches Hyperliquid public mainnet `meta` and `candleSnapshot` only, resolves the active public metadata universe, seeds research-only non-trading/non-strategy-eligible identity rows, imports normalized timezone-explicit 1D candles into the intended local strategy-validation DB, and generates ignored Money Flow v1.2 period packs for `2024`, `2025`, `YTD`, and `ALL` where each symbol has candles. The first run targeted 183 active symbols, used effective end `2026-05-14T00:00:00Z`, and generated 646 baseline packs. The follow-up dashboard builder generated 5116 ignored selected Historical Replay chart/trade JSON files and 1912 ignored evidence-only candidate pack directories. Period rows blocked because no candles exist in the selected period remain blocked rather than fabricated; MF-ORIG candidate rows with incomplete indicator context are skipped rather than defaulted. SV2.1 is 1D-only founder-review research evidence, not optimization, not a variant promotion, not paper/live approval, and not exchange execution.
