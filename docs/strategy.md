...
UAT0 completes a safety/security/runtime audit without changing Money Flow strategy rules. The frozen evidence candidate remains Hyperliquid ETH USDC perpetual `sleeve_1h` current baseline, but future UAT observation is not ETH-only: it should use a top-20 high-volume supported-asset universe to validate platform behavior, symbol mapping, no-trade reasoning, risk visibility, and operator explainability. Top-20 inclusion is not strategy approval. Future UAT2 shadow reports must compare `next_candle_open` and `next_candle_close`; `same_candle_close_research_only` remains research-only. UAT1 is blocked until API auth/authz, UAT mode gating, live endpoint lockout, secret/error/log hygiene, and market identity prerequisites are closed.
