...
- Next recommended action: after founder review, start `pt_rt1_6_week2_active` before drawing any strategy conclusion from forward observation.
...
- Active Week 2 scoring: `1h`, `4h`, `1d`.
- Paused: `15m`, because of noise reduction.
...
- Candidate, MF-ORIG, and wildcard lanes are synthetic-only and cannot send testnet orders unless the lane is the baseline control.
- Only fresh post-start Money Flow v1.2 baseline opens may trigger fixed 25 USDC Hyperliquid testnet transport under gates.
...
SV2.0.2 is the canonical historical baseline: DB-imported Hyperliquid public-mainnet candles, BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, 36 packs, 72 rows, dynamic equity, 10000 USDC per independent scenario, and next_candle_open / next_candle_close fills. SV2.1 adds a separate founder-approved 1D-only public Hyperliquid period refresh for founder review: BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, AVAX, TRX, ADA, ZEC, LINK, XMR, TON, LTC, UNI, DOT, ASTER, AAVE, POL, FIL, and TRUMP; available 1D candles back to 2024-01-01 where possible; and 90 ignored baseline packs across 2024, 2025, YTD, and ALL. PEPE/kPEPE and OKB are excluded by resolver policy, and ASTER/TRUMP lack 2024 packs because public 1D candles do not cover that period. SV2.1 also adds 1,800 ignored Historical Replay chart/trade JSON files for those 1D period sets and 810 evidence-only candidate/reference/wildcard packs covering all 10 PT-RT1 paper-observation lanes. SV2.2 adds a latest public-mainnet replay refresh through 2026-06-08 across the founder 23-symbol universe, `1h`/`4h`/`1d`, and the three Week 2 selected strategies: `money_flow_v1_2_baseline`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. SV2.1 and SV2.2 do not supersede the SV2.0.2 multi-timeframe canonical baseline unless a later phase explicitly adopts them. Dashboard Historical Replay, Evidence Lab overlays, and date filters are display/research review surfaces and are not production approval or canonical evidence replacement. PT-RT1.5.3 is the current forward-observation hotfix: active timeframes are `1h`, `4h`, and `1d`; `15m` is paused for Week 1; processed signal keys, open synthetic positions, realized equity by lane, and last processed close state persist locally; repeated same-candle opens become held/blocked decisions; missing/stale public mids are warning-only when clean fully closed candles are available; candle/indicator readiness creates true `data_unavailable`; fixed 25 USDC Hyperliquid testnet transport is baseline-only, fresh-post-start only, sized from testnet public metadata / `szDecimals`, and separate from synthetic PnL; and the explicit PT-RT1.5.3 transport smoke reached accepted/open, canceled, and reconciled.
...
PT0.0.3 extends historical replay data visibility without changing strategy rules. It adds 1D replay selection and Jan 2025 target-start readiness truth for BTC/ETH/SOL across 15m/1h/4h/1D. The current committed historical source does not reach Jan 2025; actual earliest available timestamps are reported per symbol/timeframe. The 1D view is deterministic aggregation from 4h historical replay candles and is labeled as `not_a_new_1d_money_flow_sleeve`. PT0.0.3 submits no orders, calls no order endpoints, uses no testnet prices as strategy truth, changes no Money Flow rules, and does not optimize parameters.

SV2.0 makes 1D a real Money Flow sleeve. Money Flow v1.2 consists of `sleeve_15m`, `sleeve_1h`, `sleeve_4h`, and `sleeve_1d`; existing 15m/1h/4h settings remain unchanged. The initial 1D baseline uses 50 bars, RSI 46-72, overbought 78, trim 84, max EMA5 extension 3.0%, and MACD confirmation required. These 1D settings are not optimized. SV2.0 also expands Strategy Validation evidence readiness to BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX/SHIB using Hyperliquid public mainnet metadata and `candleSnapshot`; SHIB is explicitly resolved as venue symbol `kSHIB`. Testnet market data is not strategy truth, no orders are submitted, no private/signed/order endpoints are called, and no live trading or real-capital trading is approved.

SV2.0.1 fixes evidence truth for the SV2 baseline. Hyperliquid public close timestamps are normalized to exact canonical `(start_at, end_at]` close slots, staged public-candle rows no longer report `imported=true`, compact replay rows are explicitly labeled non-canonical, final open positions are force-closed/MTM-accounted in compact rows, entry fees are counted at open, missing EMA/RSI/MACD fields invalidate evaluation instead of becoming zero, and runtime sleeve allocations are now 0.25 each for 15m/1h/4h/1d.

SV2.0.2 resolves the canonical SV2 evidence blocker and was regenerated with fully closed timeframe end boundaries. Normalized Hyperliquid public mainnet candles are imported through the hardened canonical importer into the intended `money_flow` strategy-validation DB, 36 per-symbol/per-timeframe canonical campaign configs are generated for `15m`, `1h`, `4h`, and `1d`, and existing evidence-pack machinery emits ignored canonical packs for Money Flow v1.2. Each supported symbol/timeframe backtests over its own full available DB-imported window rather than the shortest common multi-symbol window. Supported canonical evidence symbols are BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, and AVAX; SHIB remains represented but excluded/deferred as `kSHIB` because unit semantics are not clean enough for canonical evidence. Dynamic equity remains 10,000 USDC per independent scenario, open positions are force-closed at dataset end, compact rows remain non-canonical, testnet data is not strategy truth, no orders are submitted, and SOR-EV1 may proceed from the regenerated SV2.0.2 baseline.

SV2.1 broadens the 1D research evidence surface without changing strategy rules. It fetches Hyperliquid public mainnet `meta` and `candleSnapshot` only, resolves the founder-approved PT-RT1 requested/resolved universe, seeds research-only non-trading/non-strategy-eligible identity rows, imports normalized timezone-explicit 1D candles into the intended local strategy-validation DB, and generates ignored Money Flow v1.2 period packs for `2024`, `2025`, `YTD`, and `ALL` where each symbol has candles. The current run targets 23 resolved founder-approved symbols, uses effective end `2026-05-14T00:00:00Z`, and generated 90 baseline packs. The follow-up dashboard builder generated 1,800 ignored selected Historical Replay chart/trade JSON files and 810 ignored evidence-only candidate/reference/wildcard pack directories across all 10 PT-RT1 paper-observation lanes. Period rows blocked because no candles exist in the selected period remain blocked rather than fabricated. SV2.1 is 1D-only founder-review research evidence, not optimization, not a variant promotion, not paper/live approval, and not exchange execution.

SOR-EV1 and SOR-EV2 use that canonical SV2.0.2 baseline for evidence-only variant research. SOR-EV1 performs loss anatomy and diagnostic overlays; SOR-EV2 performs true-forward replay for stop/exit and rejected-signal variants. Both promote no production variant and change no Money Flow rules.

SOR-EV3 tests the founder-selected `avoid_sideways_low_volatility` family as focused true-forward replay only. It defines objective past/current-candle features for ATR percentile compression, flat SMA20/EMA10 slope, rolling-range compression, MACD flat chop, and a conservative combined blocker; baseline parity passes for all 72 canonical SV2.0.2 scenarios. All SOR-EV3 variants remain rejected/not promoted, and blocked open signals are reported separately from matched canonical baseline trades with PnL attribution. Production Money Flow rules, 15m/1h/4h/1d sleeve parameters, order behavior, live trading, and SOR/fanout/CBBO/cross-venue routing remain unchanged.

MF-ORIG-EV1.1 hotpatches the original Money Flow Trading System reconstruction as Strategy Validation-only research. MF-ORIG-EV1 originally used the prompt-provided Gerald Peters source summary, built a formal source-rule/gap matrix, and compared source-faithful 1d-first hypotheses with current Money Flow v1.2 canonical SV2.0.2 evidence. The PDF is now present in `money-flow/90 Reference/` for future source-exact reconciliation. MF-ORIG-EV1.1 quarantines pre-hotpatch PnL/drawdown conclusions and regenerates the reports with event-ledger accounting, single-counted entry fees and trim PnL, peak-to-trough drawdown, and baseline-positive 1d control-pocket filtering. The reconstruction still models Stage 1-4, 5 EMA / 20 SMA source triggers, RSI profit-warning/trim context, MACD as the TSI substitute, prior support/pivot stop proxies, and 1% risk-budget sizing from current realized equity. The current production Money Flow v1.2 implementation is unchanged, and original hypotheses are not production-approved, paper-approved, or live-approved.

MF-ORIG-EV2 extends that corrected reconstruction across all four timeframes and all 9 canonical SV2.0.2 supported symbols. It generates evidence-only packs for `mf_orig_stage2_5_20_crossover`, `mf_orig_stage2_breakout_resistance`, `mf_orig_stage2_pullback_reclaim`, and `mf_orig_stage_filter_only` plus founder-requested `_full_equity` counterparts across BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, `15m`/`1h`/`4h`/`1d`, and both next-candle fill assumptions. The source-faithful rows keep 1% risk-budget sizing; the full-equity rows size entries from current realized equity for direct comparison with full-equity replay expectations. The `1d` rows remain source-primary, `4h`/`1h` are fractal adaptations, and `15m` is a stress-test adaptation. MF-ORIG-EV2 preserves event-ledger accounting, 10,000 USDC independent-scenario starting equity, force-close handling, and peak-to-trough drawdown. The resulting dashboard replay lanes and Evidence Run Ledger comparisons are evidence-only review surfaces, not production approval or canonical date-filter regeneration.

EV-AUDIT1 is the current audit layer over those evidence tracks. It does not create another hypothesis or rerun evidence; it inventories current Money Flow v1.2, SOR-EV1/SOR-EV2/SOR-EV3, MF-ORIG-EV1.1/MF-ORIG-EV2, and pending strategy-discovery status, then scores data integrity, methodology confidence, candidate confidence, and founder decision readiness. The audit verdict is that no clean strategy candidate is promoted; current evidence is good enough for visual review and hypothesis filtering only. PT-RT1 implements the separately scoped real-time public-market paper-observation substrate using trusted public market data, PT-RT1.1A expands readiness to 10 observation lanes and requested/resolved scanner visibility, PT-RT1.1B connects and smoke-verifies public-mainnet runtime readiness, but no 24-hour run has been verified yet and no strategy approval or live-trading decision follows.
...
PT-RT1.1A is readiness only. It does not start the 24-hour runtime collection, does not enable testnet probes, does not submit testnet or live orders, and does not regenerate canonical historical evidence packs.

PT-RT1.1B adds the Hyperliquid public-mainnet connector and runtime command:
...
The dashboard has a local-only PT-RT1 runtime control helper:
...
PT-RT1.2 fixes the runtime-state behavior found during local review. Each output directory's `state.json` now persists processed signal keys, open synthetic positions, realized equity by lane, and last processed candle close by lane/symbol/timeframe. Repeated same-candle `paper_opened` attempts become held/blocked rows with `duplicate_signal_ignored` instead of new opens. Closed synthetic positions append rows to `trades.jsonl`. `summary.json` separates unavailable public market-data rows from lane-expanded `data_unavailable` decisions so one unavailable candle row is not mistaken for 10 separate market failures.
...
Historical 20 USDC Hyperliquid testnet transport was explicitly separate from dashboard-started audit/order-shape mode. PT-RT1.5.3 supersedes that current policy with fixed 25 USDC baseline-only testnet plumbing gated by fresh post-start Money Flow v1.2 baseline opens or one explicit transport smoke, exact approval, local signing configuration, and testnet metadata / `szDecimals` size preflight. Testnet fills/prices never update synthetic paper PnL.
...
PT-RT1 does not approve production Money Flow rule changes, strategy paper-runtime promotion, live trading, live orders, API-key use from the strategy lane, SOR/fanout/CBBO behavior, or testnet data as strategy truth. The current PT-RT1.5.3 testnet transport policy is fixed 25 USDC, Money Flow v1.2 baseline-only, fresh-post-start only except for one explicit transport smoke, metadata-size-preflighted, and separate from synthetic PnL.
