...
SV2.0.2 is the canonical evidence baseline: DB-imported Hyperliquid public-mainnet candles, BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, 36 packs, 72 rows, dynamic equity, 10000 USDC per independent scenario, and next_candle_open / next_candle_close fills. Dashboard Historical Replay, Evidence Lab overlays, and date filters are display-only and are not canonical evidence-pack regeneration. PT-RT1 is implemented as a real-time public-market paper-observation substrate, but it is not production approval, live approval, or strategy paper-runtime authorization. PT-RT1.1 is currently blocked because no full 24-hour probes-disabled runtime artifacts exist yet. PT-RT1.1A expands the paper-observation lab to 10 synthetic lanes, a founder-requested scanner universe, requested/resolved symbol handling, blocked-symbol reason codes, and wildcard diagnostics. PT-RT1.1B adds the public-mainnet connector/runtime command, smoke-verifies Hyperliquid public `meta` and `allMids`, resolves the expanded watchlist, records bounded paper decisions, and prepares PT-RT1.1C for the actual 24-hour probes-disabled collection.
...
PT-RT1.1 dry-run validation is currently blocked. The expected full ignored artifact directory does not exist, so public mainnet data refresh over 24 hours, closed-candle gating over time, synthetic ledgers, duplicate-signal behavior, data-health gating, and dashboard runtime readability are not verified for a 24-hour run. PT-RT1.1C may start that probes-disabled collection after PT-RT1.1B readiness review. PT-RT1.2 testnet plumbing probes may not proceed until that probes-disabled dry run passes.
