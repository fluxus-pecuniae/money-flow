...
UAT0 completes a safety/security/runtime audit without changing Money Flow strategy rules. UAT0.1 adds scoped bearer authentication for sensitive `/api/v1` routes and an inspectable fail-safe runtime safety policy; it still does not connect to exchanges, submit orders, approve paper trading, approve live trading, add routing behavior, or change Money Flow rules. The frozen evidence candidate remains Hyperliquid ETH USDC perpetual `sleeve_1h` current baseline, but future UAT observation is not ETH-only: it should use a top-20 high-volume supported-asset universe to validate platform behavior, symbol mapping, no-trade reasoning, risk visibility, and operator explainability. Top-20 inclusion is not strategy approval. Future UAT2 shadow reports must compare `next_candle_open` and `next_candle_close`; `same_candle_close_research_only` remains research-only. UAT1 remains blocked until adapter-level runtime-policy enforcement verification, selected-venue sandbox/read-only endpoint policy, secret/error/log hygiene, runtime drawdown monitoring, and market identity prerequisites are closed.
