...
PT-RT1.1 dry-run validation is currently blocked. The expected ignored artifact directory `reports/paper_runtime/pt_rt1_1_24h_dry_run/` does not exist, so public mainnet data refresh, closed-candle gating, synthetic ledgers, duplicate-signal behavior, data-health gating, and dashboard runtime readability are not verified for a 24-hour run. PT-RT1.2 testnet plumbing probes may not proceed until that probes-disabled dry run passes.
