...
MF-ORIG-EV1 reconstructs the original Money Flow Trading System as Strategy Validation-only research. It uses the prompt-provided Gerald Peters source summary because the PDF was not present locally, builds a formal source-rule/gap matrix, and compares source-faithful 1d-first hypotheses with current Money Flow v1.2 canonical SV2.0.2 evidence. The reconstruction models Stage 1-4, 5 EMA / 20 SMA source triggers, RSI profit-warning/trim context, MACD as the TSI substitute, prior support/pivot stop proxies, and 1% risk-budget sizing from current realized equity. The current production Money Flow v1.2 implementation is unchanged, and original hypotheses are not production-approved, paper-approved, or live-approved.
