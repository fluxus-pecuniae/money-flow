...
- a concurrent explicit submit for the same child intent blocks with `submission_in_progress` before adapter submission, while a repeat after a `SubmittedOrder` exists returns existing submitted-order truth
...
Phase 6 closeout still does not make the platform a routing optimizer or automation layer. It closes an explicit single-target recommendation-backed path through gated submit and read-only post-submit/workflow inspection; Phase 6.10.1 hardens concurrent explicit submit and lineage truth only. Strategy, planning, routing assessment, route-readiness audit, recommendation, target choice, child-intent creation, readiness, explicit submission, and post-submit lifecycle boundaries remain separate.
