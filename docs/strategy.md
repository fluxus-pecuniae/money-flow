...
SV2.0.2 resolves the canonical SV2 evidence blocker. Normalized Hyperliquid public mainnet candles are imported through the hardened canonical importer into the intended `money_flow` strategy-validation DB, canonical campaign configs are generated for `15m`, `1h`, `4h`, and `1d`, and existing evidence-pack machinery emits ignored canonical packs for Money Flow v1.2. Supported canonical evidence symbols are BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, and AVAX; SHIB remains represented but excluded/deferred as `kSHIB` because unit semantics are not clean enough for canonical evidence. Dynamic equity remains 10,000 USDC per independent scenario, open positions are force-closed at dataset end, compact rows remain non-canonical, testnet data is not strategy truth, no orders are submitted, and SOR-EV1 may proceed from the SV2.0.2 baseline.
