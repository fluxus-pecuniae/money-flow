...
UAT0 completes a safety/security/runtime audit without changing Money Flow strategy rules. UAT0.1 adds scoped bearer authentication for sensitive `/api/v1` routes and an inspectable fail-safe runtime safety policy. UAT0.2 adds adapter runtime-policy guards before private/signed/order transport, public read-only classification, a Hyperliquid future-UAT1 read-only allowlist artifact, and representative redaction tests. UAT0.3 adds fixture-only top-20 observation-universe resolver policy, Hyperliquid public read-only info-type classification, and a fixture-tested runtime drawdown monitor model. UAT1 verifies actual public-read-only Hyperliquid endpoint behavior and a public no-key top-volume source under explicit public-read-only network gating, then resolves observation-only Hyperliquid supported assets. UAT1 does not run Money Flow live, approve paper trading, approve live trading, submit orders, call private/signed/order endpoints, use API keys, add routing behavior, or change Money Flow rules. UAT1.1 adds model/report-only shadow signal audit records, operator-visible shadow drawdown state, UAT1 universe snapshot loading, and representative structured API-error/log redaction verification without running UAT2 or creating strategy/execution artifacts. UAT2 runs the current baseline Money Flow rule logic as no-order shadow observation across the UAT1 Hyperliquid top-20 universe using only public read-only candle snapshots. It writes shadow audit/report records only and creates no `StrategyDecision`, `SignalEvent`, `OrderIntent`, `PreparedVenueOrder`, `ExecutionReadinessAssessment`, `SubmittedOrder`, approval, routing, paper, or live artifacts. UAT2.1 visualizes that UAT2 output in the static dashboard and adds an informational UAT3 blocked readiness panel without enabling approval or order actions. UAT3.0 defines the future sandbox-order design only: initial scope is Hyperliquid ETH `sleeve_1h`, tiny sandbox/testnet quantity, explicit approval gate, sandbox runtime policy, sandbox account drawdown feed, artifact labeling, submit-lease/lifecycle verification, and risk gates. UAT3.0.1 adds fixture-only validators for sandbox runtime policy, artifact labels, actual-submission approval scope, risk gates, sandbox drawdown feed, and submit-lease duplicate-prevention. UAT3.0.2 hardens those validators with full runtime-policy blocker propagation, non-positive numeric validation, and a unified dry-run sandbox gate preflight. UAT3.0.3 adds sandbox artifact label boundary helpers and a dry-run executable gate service that wires approval, risk, drawdown, and submit-lease checks without creating artifacts. UAT3.0.4 adds fail-closed private read-only sandbox account/drawdown policy, credential approval/boundary validation, endpoint category separation, redaction checks, and sandbox account drawdown feed modeling. UAT3.0.5 validates the exact private-read-only approval boundary, verifies local sandbox/testnet credential environment variables, performs one Hyperliquid testnet read-only account-state request, and records `sandbox_drawdown_feed_live_fed_verified` without sending an API key/private key or calling order/cancel/amend/retry endpoints. UAT3.1 actual sandbox order submission remains blocked. The frozen evidence candidate remains Hyperliquid ETH USDC perpetual `sleeve_1h` current baseline, but UAT observation is not ETH-only: it uses a top-20 high-volume supported-asset universe to validate platform behavior, symbol mapping, no-trade reasoning, risk visibility, and operator explainability. Top-20 inclusion is not strategy approval. UAT2 shadow reports compare `next_candle_open` and `next_candle_close`; `same_candle_close_research_only` remains research-only.

UAT3.0.5 does not change Money Flow strategy behavior. The exact private-read-only approval text is recorded, sandbox/testnet credential environment variables are verified locally, and one Hyperliquid testnet read-only account-state request produced `sandbox_drawdown_feed_live_fed_verified`. No API key/private key was sent, no order/cancel/amend/retry endpoint was called, and no strategy/execution artifact was created. Actual sandbox order submission, paper trading, live trading, private order endpoints, and order/cancel/amend/retry endpoints remain unapproved.
