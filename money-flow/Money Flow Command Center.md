...
Current focus: Strategy Validation guarded canonical candle import. The intended local `money_flow` DB is migrated/current but still has zero candles, research identity must be operator-verified and non-trading, timezone-explicit candle files must map one-to-one to canonical requirements and pass requirement-aware preflight before import, no evidence packs are generated in SV1.12.1, and paper trading is not approved.
