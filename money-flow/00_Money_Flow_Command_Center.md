...
- Testnet plumbing: fixed 25 USDC Hyperliquid testnet transport is allowed only for fresh post-start Money Flow v1.2 baseline opens when PT-RT1.5.2 gates and local signing config pass. The explicit PT-RT1.5.2 smoke reached Hyperliquid testnet and rejected on venue size validation without synthetic PnL impact.
- Production approval: no strategy is production-approved.
- Live trading: not approved; no real-capital trading is approved.
- Next recommended action: start `pt_rt1_5_2_week1_active`, then fix or verify the testnet size formatter before relying on accepted/open lifecycle coverage.

## Current Truth

| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1.5.2` signed testnet transport smoke + active Week 1 restart handoff on top of completed `PT-RT1.5.1`, `PT-RT1.5`, `PT-RT1.4.1`, `PT-RT1.4`, `PT-RT1.3`, `PT-RT1.2`, `PT-RT1.1C`, `PT-RT1.1B`, `PT-RT1.1A`, `PT-RT1`, `OB2.0`, and `EV-AUDIT1`; no strategy candidate promoted |
...
| Next recommended phase | Start the clean `pt_rt1_5_2_week1_active` session; keep `1h`/`4h`/`1d` active, keep `15m` paused/legacy, confirm startup-valid signals are blocked until fresh post-start transitions, and address the testnet venue size reject before depending on accepted/open lifecycle coverage |
...
| Paper trading | PT-RT1/PT-RT1.1A/PT-RT1.1B/PT-RT1.1C/PT-RT1.4/PT-RT1.4.1/PT-RT1.5/PT-RT1.5.1/PT-RT1.5.2 add synthetic paper-observation ledgers and runtime collection/review UI only. Active Week 1 paper review uses `1h`/`4h`/`1d`; `15m` is paused for noise reduction. PT-RT1.5.2 verifies signed testnet transport with one labeled smoke order that reached Hyperliquid testnet and rejected on size, keeps fresh-signal gating, and moves the preferred active scope to `pt_rt1_5_2_week1_active`. No strategy paper runtime is production-approved from EV-AUDIT1 or PT-RT1 evidence |
| Broader top-20 paper/sandbox | Historical PT0 broader top-20 Hyperliquid-supported paper/sandbox scope is risk-gated and metadata-gated context only. It is not production strategy approval and is separate from current PT-RT1.5.2 Paper Trading review |
...
PT-RT1 is implemented as a forward-observation substrate. The strategy-truth lane is Hyperliquid public mainnet market data only, with fully closed candle gating, indicator computation that does not default missing values to zero, and independent synthetic 10,000 USDC ledgers. PT-RT1.3 is the current runtime data-health layer: state persists processed signal keys, open synthetic positions, realized equity by lane, and last processed close by lane/symbol/timeframe; repeated same-candle `paper_opened` attempts become held/blocked rows; synthetic closes append `trades.jsonl`; and `summary.json` now treats stale/thin/missing public mids as warning-only when clean fully closed `candleSnapshot` rows are available. Candle/indicator readiness remains the true `data_unavailable` gate. PT-RT1.4 updates the founder-facing Paper Trading dashboard into the weekly command center. Active Week 1 paper review is scoped to `1h`, `4h`, and `1d`; `15m` is `disabled_for_week1_noise_reduction`, excluded from active lane comparison/all-active totals/new synthetic entries, and preserved as paused/legacy data. PT-RT1.4.1 verifies the active-week runtime cutover. PT-RT1.5 resets the active Week 1 Paper Trading scope to `pt_rt1_5_week1_active`, hides archived open/closed/signal rows by default, keeps market refresh active for watchlist/chart/unrealized context, and evaluates strategy signals only after fully closed `1h`/`4h`/`1d` candles plus grace delay. PT-RT1.5.1 enables signed Hyperliquid testnet transport only for fresh Money Flow v1.2 baseline open signals and adds warm-start false-to-true gating plus public-mainnet MTM. PT-RT1.5.2 verifies the signed transport path with one explicit `testnet_transport_smoke_not_strategy_signal` fixed 25 USDC smoke while preserving baseline-only fixed 25 USDC Hyperliquid testnet lifecycle rows for future fresh baseline opens. The smoke reached Hyperliquid testnet, was rejected by venue size validation, reconciled to no open order, created no synthetic trade, and did not update synthetic PnL. The clean active Week 1 runtime scope is now `reports/paper_runtime/pt_rt1_5_2_week1_active/`; operator start is still required. Candidate, MF-ORIG, and wildcard lanes remain synthetic-only. The current daily review pack remains `docs/pt_rt_week1_day_summary.md` until the new scope has fresh rows. Testnet fills never update synthetic PnL, public mainnet candles remain strategy truth, live trading is not approved, and no strategy is production-approved.
