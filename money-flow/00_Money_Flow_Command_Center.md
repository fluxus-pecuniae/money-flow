...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `MF-ORIG-EV1` Original Money Flow reconstruction evidence complete; production Money Flow v1.2 unchanged |
...
MF-ORIG-EV1 is complete as evidence-only research. It reconstructs the original Money Flow Trading System from the prompt-provided Gerald Peters September 5, 2019 source summary, because the PDF was not present in the repo or common local Downloads/Documents paths during the pass. The report formalizes the source hierarchy and gap matrix against current Money Flow v1.2, treats `1d` as the primary original timeframe, models Stage 1-4, 5 EMA / 20 SMA triggers, RSI profit warnings/trims, MACD as the TSI substitute, prior support/pivot stop proxies, and 1% risk-budget sizing. Canonical SV2.0.2 DB-imported evidence packs remain the comparison baseline. The reconstructed hypotheses showed pre-gate aggregate PnL/drawdown improvement but failed the candidate gate because control pockets were not preserved; they are not production-approved and do not authorize paper/live behavior.
