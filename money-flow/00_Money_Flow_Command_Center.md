...
| Execution status | UAT3.4 recorded one approved fixed-target Hyperliquid testnet ETH attempt: accepted/open, canceled successfully, reconciled to no open order; PT0 visualizes that ledger, shadow records, public-read-only monitor rows, TradingView Lightweight Charts, and internal paper-equity state in an exchange-style cockpit; live exchange order submission is not approved |
