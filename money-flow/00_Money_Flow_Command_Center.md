...
- Proposed next phase: founder/operator must supply explicit offline market-identity verification plus all 18 timezone-explicit candle files, rerun SV1.12.3 guarded import attempt to reach `canonical_import_complete`, then SV1.13 post-import canonical evidence review/evidence-pack generation can run only if data-readiness audits are clean. Paper-trading design remains deferred until founder/operator evidence review justifies it, and `Phase 8.1` remains deferred until explicitly scoped.
