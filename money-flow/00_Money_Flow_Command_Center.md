...
| Next recommended phase | Continue or restart the active Week 1 session with PT-RT1.5.3 present; keep `1h`/`4h`/`1d` active, keep `15m` paused/legacy, confirm startup-valid signals are blocked until fresh post-start transitions, and review the next fresh baseline-triggered lifecycle row. For strategy discovery, do not promote GOAL-STRAT1 near misses; add longer non-overlapping OOS/control-pocket data and execution-quality constraints before any next pass. |
