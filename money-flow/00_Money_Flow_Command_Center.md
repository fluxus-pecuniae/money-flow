...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1.3` candle-truth data-health semantics on top of completed `PT-RT1.2`, `PT-RT1.1C`, `PT-RT1.1B`, `PT-RT1.1A`, `PT-RT1`, `OB2.0`, and `EV-AUDIT1`; no strategy candidate promoted |
| Current major track | Forward paper observation substrate after SV2.0.2 canonical baseline, SOR, MF-ORIG, EV-AUDIT1, and OB2.0 |
| Next recommended phase | Run a fresh `PT-RT1.3` observation session and review candle-truth data-health rows, mid-warning rollups, duplicate-open blocking, synthetic trade rows, compact-log stats, and 20 USDC probe audit/transport-gate status before any signed testnet transport |
...
PT-RT1 is implemented as a forward-observation substrate. The strategy-truth lane is Hyperliquid public mainnet market data only, with fully closed candle gating, indicator computation that does not default missing values to zero, and independent synthetic 10,000 USDC ledgers. PT-RT1.3 is the current runtime data-health layer: state persists processed signal keys, open synthetic positions, realized equity by lane, and last processed close by lane/symbol/timeframe; repeated same-candle `paper_opened` attempts become held/blocked rows; synthetic closes append `trades.jsonl`; and `summary.json` now treats stale/thin/missing public mids as warning-only when clean fully closed `candleSnapshot` rows are available. Candle/indicator readiness remains the true `data_unavailable` gate. The Paper Observation dashboard now loads recent ignored PT-RT `decisions.jsonl` rows for Signal Generation so durable `paper_opened` rows remain visible even when the latest summary cycle has no new opens; it also separates audit/order-shape probe rows from signed testnet orders. TRUMP is deferred from fresh PT-RT scanner runs because it created excessive runtime noise. The Hyperliquid testnet plumbing lane remains separate, exact-approval gated, capped at 20 USDC in dashboard-started runs, post-only `Alo`, and never updates strategy paper PnL; signed testnet transport is an explicit PT-RT1.2 gated path requiring `--submit-testnet-probes`, exact transport approval, and a configured client. The normal dashboard path remains audit/order-shape only. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping that process and machine awake/networked. PT-RT1/PT-RT1.1/PT-RT1.2/PT-RT1.3 do not approve production rules, strategy paper-runtime promotion, live trading, live orders, private/signed/order endpoints from strategy truth, API-key use, SOR/fanout/CBBO, or testnet data as strategy truth.
