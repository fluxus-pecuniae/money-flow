...
- Current implemented phase: `SV1.16.1` replay context semantics and variant metric truth hotfix; SV1.16.1 clarifies production-rule-in-replay-state field semantics, variant divergence, and separated production-rule rejection / variant-admitted-from-rejection / variant no-trade counters while preserving production Money Flow rules.
- Phase 7 status: accepted complete.
- Proposed next phase: broader true replay experiments or manual founder/operator review of Hyperliquid-only constant-notional, dynamic-equity, trade-anatomy, methodology-hardened experiment, and SV1.16.1 replay-methodology evidence before any explicitly scoped paper-trading design phase. SV1.12.5 shows Aster and Binance have 18 additional complete native-trade-count candidate files but need separate non-trading identity verification/seed/import; OKX and Coinbase need a trade-count source or explicit canonical-contract decision; Kraken needs archive/vendor/operator data.
