...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `SOR-EV3` avoid_sideways_low_volatility drilldown complete; no SOR variant promoted |
| Current major track | Strategy Validation v2 evidence refresh |
| Next proposed phase | `SOR-EV4` only if the founder selects a narrower out-of-sample-style hypothesis; `PT0.1` supervised top-20 paper/sandbox runtime week remains future work using trusted market data |
