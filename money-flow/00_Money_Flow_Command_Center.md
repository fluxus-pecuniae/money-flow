...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1.4.1` active-week runtime cutover verification and daily founder review pack on top of completed `PT-RT1.4`, `PT-RT1.3`, `PT-RT1.2`, `PT-RT1.1C`, `PT-RT1.1B`, `PT-RT1.1A`, `PT-RT1`, `OB2.0`, and `EV-AUDIT1`; no strategy candidate promoted |
...
| Next recommended phase | Continue Week 1 paper observation from the restarted PT-RT1.4.1 active runtime focused on `1h`, `4h`, and `1d`; keep `15m` paused/legacy; use `docs/pt_rt_week1_day_summary.md` for daily founder review before any signed testnet transport |
...
| Paper trading | PT-RT1/PT-RT1.1A/PT-RT1.1B/PT-RT1.1C/PT-RT1.4/PT-RT1.4.1 add synthetic paper-observation ledgers and runtime collection/review UI only. Active Week 1 paper review uses `1h`/`4h`/`1d`; `15m` is paused for noise reduction. The retired pre-cutover runtime is excluded from active scoring and the restarted active runtime has 0 15m rows in its first artifact cycle. No strategy paper runtime is production-approved from EV-AUDIT1 or PT-RT1 evidence |
...
PT-RT1 is implemented as a forward-observation substrate. The strategy-truth lane is Hyperliquid public mainnet market data only, with fully closed candle gating, indicator computation that does not default missing values to zero, and independent synthetic 10,000 USDC ledgers. PT-RT1.3 is the current runtime data-health layer: state persists processed signal keys, open synthetic positions, realized equity by lane, and last processed close by lane/symbol/timeframe; repeated same-candle `paper_opened` attempts become held/blocked rows; synthetic closes append `trades.jsonl`; and `summary.json` now treats stale/thin/missing public mids as warning-only when clean fully closed `candleSnapshot` rows are available. Candle/indicator readiness remains the true `data_unavailable` gate. PT-RT1.4 updates the founder-facing Paper Trading dashboard into the weekly command center. Active Week 1 paper review is scoped to `1h`, `4h`, and `1d`; `15m` is `disabled_for_week1_noise_reduction`, excluded from active lane comparison/all-active totals/new synthetic entries, and preserved as paused/legacy data. PT-RT1.4.1 verifies the active-week runtime cutover: the old `pt_rt1_1c_24h_dry_run` process continued creating 15m entries after cutover, is now labeled `pre_pt_rt1_4_weekend_burn_in`, and is excluded from active scoring; the restarted active runtime under `reports/paper_runtime/pt_rt1_4_1_active_week/` produced zero 15m rows in its first artifact cycle. The current daily review pack is `docs/pt_rt_week1_day_summary.md` and reports `active_runtime_cutover_verified_after_restart` with `Week 1 paper observation may continue`. The page opens with a top health banner, timeframe-scoped Weekly Scoreboard, Timeframe Breakdown, live public candle chart, cleaned Open Synthetic Positions, cleaned Closed Synthetic Trades with summary cards, and a categorized Signal / Decision Stream. Testnet plumbing labels split audit-only shapes from actual testnet order transport; normal dashboard-started runs remain audit/order-shape only with testnet order transport disabled. TRUMP is deferred from fresh PT-RT scanner runs because it created excessive runtime noise. The Hyperliquid testnet plumbing lane remains separate, exact-approval gated, capped at 20 USDC in dashboard-started runs, post-only `Alo`, and never updates strategy paper PnL; signed testnet transport is an explicit PT-RT1.2 gated path requiring `--submit-testnet-probes`, exact transport approval, and a configured client. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping that process and machine awake/networked. PT-RT1/PT-RT1.1/PT-RT1.2/PT-RT1.3/PT-RT1.4/PT-RT1.4.1 do not approve production rules, strategy paper-runtime promotion, live trading, live orders, private/signed/order endpoints from strategy truth, API-key use, SOR/fanout/CBBO, or testnet data as strategy truth.
