...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1` real-time public-market paper observation + testnet plumbing probes implemented on top of completed `OB2.0` and `EV-AUDIT1`; `PT-RT1.1` dry-run validation is blocked because no 24-hour runtime artifacts exist; no strategy candidate promoted |
| Current major track | Forward paper observation substrate after SV2.0.2 canonical baseline, SOR, MF-ORIG, EV-AUDIT1, and OB2.0 |
| Next recommended phase | Run the PT-RT1 24-hour public-mainnet dry run with testnet probes disabled and retain ignored artifacts under `reports/paper_runtime/pt_rt1_1_24h_dry_run/`; PT-RT1.2 is blocked until that passes |
...
| Paper trading | PT-RT1 adds synthetic paper-observation ledgers only. No strategy paper runtime is production-approved from EV-AUDIT1 or PT-RT1 evidence |
...
PT-RT1 is implemented as a forward-observation substrate. The strategy-truth lane is Hyperliquid public mainnet market data only, with fully closed candle gating, indicator computation that does not default missing values to zero, duplicate synthetic-signal prevention, and independent synthetic 10,000 USDC ledgers for `money_flow_v1_2_baseline`, `avoid_low_rolling_range_50`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. The Hyperliquid testnet plumbing lane is separate, disabled and kill-switched by default, exact-approval gated, capped under 10 USDC, post-only `Alo`, cancel/reconcile required, and never updates strategy paper PnL. The dashboard now has a `Paper Observation` tab backed by the committed PT-RT1 summary JSON. PT-RT1.1 checked for the required 24-hour probes-disabled artifact set at `reports/paper_runtime/pt_rt1_1_24h_dry_run/` and found it missing, so public mainnet refresh, candle gating, ledgers, duplicate prevention, data-health gating, and dashboard runtime readability are not yet verified. The 60-day observation has not started. PT-RT1.2 is blocked until a real probes-disabled dry run passes. PT-RT1/PT-RT1.1 do not approve production rules, strategy paper-runtime promotion, live trading, live exchange orders, private/signed/order endpoints from strategy truth, API-key use, SOR/fanout/CBBO, or testnet data as strategy truth.
