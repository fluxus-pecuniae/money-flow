...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1` real-time public-market paper observation + testnet plumbing probes implemented on top of completed `OB2.0` and `EV-AUDIT1`; no strategy candidate promoted |
| Current major track | Forward paper observation substrate after SV2.0.2 canonical baseline, SOR, MF-ORIG, EV-AUDIT1, and OB2.0 |
| Next recommended phase | Run the PT-RT1 24-hour public-mainnet dry run with testnet probes disabled, then start the 60-day observation window only if stable |
...
PT-RT1 is implemented as a forward-observation substrate. The strategy-truth lane is Hyperliquid public mainnet market data only, with fully closed candle gating, indicator computation that does not default missing values to zero, duplicate synthetic-signal prevention, and independent synthetic 10,000 USDC ledgers for `money_flow_v1_2_baseline`, `avoid_low_rolling_range_50`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. The Hyperliquid testnet plumbing lane is separate, disabled and kill-switched by default, exact-approval gated, capped under 10 USDC, post-only `Alo`, cancel/reconcile required, and never updates strategy paper PnL. The dashboard now has a `Paper Observation` tab backed by the committed PT-RT1 summary JSON. The 60-day observation has not started. PT-RT1 does not approve production rules, strategy paper-runtime promotion, live trading, live exchange orders, private/signed/order endpoints from strategy truth, API-key use, SOR/fanout/CBBO, or testnet data as strategy truth.
