...
- Current implemented phase: `SV1.17` true replay experiment round 1; SV1.17 uses the SV1.16/SV1.16.1 replay substrate to compare a small set of Hyperliquid ETH `sleeve_1h` lower-RSI plus market-structure variants against baseline without changing production Money Flow rules.
- Phase 7 status: accepted complete.
- Proposed next phase: founder/operator review of Hyperliquid-only constant-notional, dynamic-equity, trade-anatomy, methodology-hardened experiment, SV1.16.1 replay-methodology, and SV1.17 true-replay-round evidence before any explicitly scoped paper-trading design phase. SV1.12.5 shows Aster and Binance have 18 additional complete native-trade-count candidate files but need separate non-trading identity verification/seed/import; OKX and Coinbase need a trade-count source or explicit canonical-contract decision; Kraken needs archive/vendor/operator data.
...
- SV1.15.1 status: hypothesis experiment methodology truth hotfix. Each SV1.15 variant now has a methodology label; completed-trade overlays are diagnostic estimates rather than true forward replays; recent-low invalidation is a lookahead diagnostic upper bound, not a candidate rule result; and lower-RSI admission remains deferred until rejected-signal replay instrumentation exists. No hypothesis is authorized, no Money Flow rules changed, and paper/live/routing/execution behavior remains deferred.
- SV1.16 status: rejected-signal replay instrumentation added. Per-candle replay context records production-rule action/reasons, accepted/rejected entry truth, RSI zones, EMA/MACD/extension state, regimes, and recent swing high/low diagnostics. The replay runner maintains position occupancy and dynamic-equity sizing, and the narrow `lower_rsi_floor_trend_intact_v1` ETH `sleeve_1h` example admitted replay-only entries but underperformed the current baseline sampled scenario. It is research-only, not a rule change, not paper/live authorization, and not routing/execution behavior.
- SV1.16.1 status: replay methodology truth hotfix. `production_rule_*_in_replay_state` fields now carry the preferred semantics, legacy `baseline_*` fields are compatibility aliases, variant divergence is explicit, and variant-admitted candles are counted separately from variant no-trade. Production Money Flow rules, paper/live trading, routing/execution behavior, and exchange calls remain unchanged/deferred.
- SV1.17 status: true replay experiment round one. ETH `sleeve_1h` baseline remained strongest in the sampled scenario; lower-RSI trend-intact v1/v2 deteriorated ending equity, support-confirmed admitted no trades, and EMA10-hold/no-resistance lowered drawdown but still ended below baseline. No variant is authorized.
