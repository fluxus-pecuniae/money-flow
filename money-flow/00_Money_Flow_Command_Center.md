...
- Current implemented phase: `SV1.17` true replay experiment round 1; SV1.17 uses the SV1.16/SV1.16.1 replay substrate to compare lower-RSI plus market-structure variants across the full Hyperliquid BTC/ETH/SOL x 15m/1h/4h public campaign suite without changing production Money Flow rules.
...
- SV1.17 status: true replay experiment round one. The full-suite run now covers BTC/ETH/SOL across `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`. Some variants improve losing baselines, but ETH `sleeve_1h` baseline remains the strongest above-starting-equity pocket; no variant is authorized.
