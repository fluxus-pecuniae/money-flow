...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `MF-ORIG-EV2` Original Money Flow multi-timeframe evidence packs + Historical Replay UI complete; production Money Flow v1.2 unchanged |
| Current major track | Strategy Validation v2 evidence refresh |
| Next proposed phase | `MF-ORIG-EV3` only if the founder wants deeper Original Money Flow refinement after dashboard review; `SOR-EV4` only if a narrower SOR hypothesis is selected; `PT0.1` supervised top-20 paper/sandbox runtime week remains future work using trusted market data |
...
MF-ORIG-EV2 is complete as an evidence-only multi-timeframe expansion. It preserves the MF-ORIG-EV1.1 event-ledger accounting and peak-to-trough drawdown hotpatch, then generates 144 ignored evidence-pack directories and 36 ignored dashboard chart-data files across four Original Money Flow hypotheses, BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX, 15m/1h/4h/1d, and both next-candle fill assumptions. Historical Replay and the Evidence Run Ledger now auto-load MF-ORIG-EV2 strategy rows when local chart-data JSON exists. `1d` remains source-primary, `4h`/`1h` are fractal adaptations, and `15m` is a stress-test adaptation. Candidate gates were re-run against Money Flow v1.2 canonical SV2.0.2 baseline rows; no original hypothesis is production-approved and no paper/live behavior is authorized. MF-ORIG-EV1.1 remains the accounting/drawdown hotpatch baseline and the source PDF limitation still stands because the PDF was not present locally.
