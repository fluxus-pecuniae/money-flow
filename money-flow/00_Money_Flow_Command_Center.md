...
SV2.1 broad 1D period evidence is complete as a separate research refresh. It used Hyperliquid public mainnet `meta` and `candleSnapshot` only, targeted 183 active public metadata symbols, imported available 1D candles back to 2024-01-01 where available, and generated 646 ignored period packs for 2024, 2025, YTD, and ALL. It broadens founder-review data for 1D only; it does not change Money Flow rules, supersede SV2.0.2 multi-timeframe evidence, approve a variant, approve paper/live, submit orders, or call private/signed/order endpoints.
