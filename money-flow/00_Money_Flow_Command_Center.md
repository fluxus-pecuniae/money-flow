...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `SOR-EV1` Money Flow loss anatomy and evidence-only variant diagnostics complete; SV2.0.2 canonical packs remain the baseline evidence source |
| Current major track | Strategy Validation v2 evidence refresh |
| Next proposed phase | `SOR-EV2` true-forward stop/rejected-signal replay may be scoped from SOR-EV1 diagnostics; `PT0.1` supervised top-20 paper/sandbox runtime week remains future work using trusted market data |
