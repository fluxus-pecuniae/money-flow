...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1.5` Week 1 reset + baseline testnet lifecycle + candle scheduler on top of completed `PT-RT1.4.1`, `PT-RT1.4`, `PT-RT1.3`, `PT-RT1.2`, `PT-RT1.1C`, `PT-RT1.1B`, `PT-RT1.1A`, `PT-RT1`, `OB2.0`, and `EV-AUDIT1`; no strategy candidate promoted |
...
| Next recommended phase | Start/restart Week 1 paper observation under `pt_rt1_5_week1_active`; keep `1h`/`4h`/`1d` active, keep `15m` paused/legacy, and review the first candle-close scheduler + baseline-only fixed 25 USDC Hyperliquid testnet lifecycle rows |
...
| Paper trading | PT-RT1/PT-RT1.1A/PT-RT1.1B/PT-RT1.1C/PT-RT1.4/PT-RT1.4.1/PT-RT1.5 add synthetic paper-observation ledgers and runtime collection/review UI only. Active Week 1 paper review uses `1h`/`4h`/`1d`; `15m` is paused for noise reduction. PT-RT1.5 resets active scope to `pt_rt1_5_week1_active`, archives old rows by default, evaluates strategy signals only after fully closed candles, and permits baseline-only fixed 25 USDC Hyperliquid testnet lifecycle rows. No strategy paper runtime is production-approved from EV-AUDIT1 or PT-RT1 evidence |
