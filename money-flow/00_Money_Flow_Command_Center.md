...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1.1C` 24-hour probes-disabled runtime collection artifacts plus compact decision logging default on top of completed `PT-RT1.1B`, `PT-RT1.1A`, `PT-RT1`, `OB2.0`, and `EV-AUDIT1`; evaluation is deferred to PT-RT1.1D after completion; no strategy candidate promoted |
| Current major track | Forward paper observation substrate after SV2.0.2 canonical baseline, SOR, MF-ORIG, EV-AUDIT1, and OB2.0 |
| Next recommended phase | Run `PT-RT1.1D` to evaluate ignored artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`, including compact-log suppression/size stats; PT-RT1.2 is blocked until that passes |
