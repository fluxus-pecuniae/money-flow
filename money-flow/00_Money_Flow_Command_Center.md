...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `OB2.0` Obsidian strategy brain refresh complete on top of completed `EV-AUDIT1`; no strategy candidate promoted |
| Current major track | Evidence governance / strategy brain after SV2.0.2 canonical baseline, SOR, MF-ORIG, and EV-AUDIT1 |
| Next recommended phase | `PT-RT1` real-time public market data + paper observation runtime may be scoped separately; it is not approved or implemented by OB2.0 |
...
| Paper trading | PT0 sandbox/testnet paper-plumbing approval exists for Hyperliquid testnet/sandbox only. PAPER TRADING IS APPROVED. No strategy paper runtime is approved from EV-AUDIT1 evidence |
