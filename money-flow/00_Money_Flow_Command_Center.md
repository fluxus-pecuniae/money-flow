...
- Local `HYPERLIQUID_UAT_SANDBOX_*` credential environment variables are missing, so no credentials were loaded, no API keys were used, and no private endpoints were called.
- Live Hyperliquid base URLs are blocked by sandbox/testnet validation; sandbox account-state payload parsing can produce a not-live-account drawdown feed when caller-supplied sandbox account truth is available.
- Live-fed sandbox drawdown remains blocked with `sandbox_drawdown_feed_missing`.
- UAT3.1 actual sandbox order submission remains blocked by missing actual-submission approval, live-fed sandbox account drawdown, real sandbox submit path wiring, executable approval/risk wiring to persistence/submit, and submit-lease integration verification.
- No order intent, prepared order, submitted order, executable approval, private endpoint call, order endpoint call, exchange API-key use, paper/live behavior, routing artifact, evidence pack, or Money Flow rule change was added.
