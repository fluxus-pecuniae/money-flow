...
| Current major track | Strategy Validation v2 evidence refresh |
| Next proposed phase | `SV2.x` DB-backed candle import + canonical evidence-pack generation may be scoped; SOR-EV1 / strategy-variant evidence remains blocked until canonical SV2 evidence exists or staged compact rows are explicitly accepted as noncanonical input; `PT0.1` supervised top-20 paper/sandbox runtime week remains future work using trusted market data |
...
| Production Money Flow rules | Money Flow v1.2 adds `sleeve_1d`; existing 15m/1h/4h settings remain unchanged |

SV2.0.1 is complete. It fixes canonical evidence truth without changing Money Flow rules: compact SV2 rows now count entry fees at open, force-close dataset-end open positions with explicit fields, normalize Hyperliquid `.999Z` public candle close timestamps to canonical `(start_at, end_at]` close slots, split fetched/normalized/staged data from DB-imported/canonical-evidence-ready data, set runtime sleeve allocations to 0.25 each, canonicalize internal `1d` while displaying `1D`, and treats missing EMA/RSI/MACD inputs as invalid rather than zero. Canonical SV2 evidence packs remain blocked with `canonical_sv2_evidence_packs_missing` because refreshed public candles are staged-only and not DB-imported through the hardened importer. SV2.0 is complete: Money Flow v1.2 includes `sleeve_1d` as a real first-class sleeve while preserving existing 15m/1h/4h settings. The initial 1D sleeve uses 50 bars, RSI 46-72, overbought 78, trim 84, max EMA5 extension 3.0%, and MACD required; it is not optimized. SV2.0 resolves BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX/SHIB through Hyperliquid public mainnet metadata, maps SHIB explicitly to `kSHIB`, fetches public mainnet `candleSnapshot` rows for 15m/1h/4h/1D where available, writes `docs/sv2_0_historical_data_refresh_summary.json`, and records compact dynamic-equity evidence rows. `4h` and `1D` reach the Jan 2025 target; `15m` and `1h` carry `hyperliquid_public_5000_candle_limit`. SV1.18, UAT0 through UAT4.2, PT0, PT0.0.1, PT0.0.2, and PT0.0.3 remain complete historical milestones. PT0.0.3's 1D replay was an aggregated dashboard view only; SV2.0 supersedes that limitation with the real `sleeve_1d`. UAT3.1-SV2.0.1 created no production `OrderIntent`, `PreparedVenueOrder`, `SubmittedOrder`, executable approval, live real-capital behavior, smart routing/SOR/fanout, order endpoint call, live endpoint use, private/signed/order dashboard call, exchange API-key use, dashboard order controls, or unapproved repeated order. UAT remains plumbing and behavior validation. Strategy evidence uses Hyperliquid public mainnet candles; Hyperliquid testnet remains sandbox execution plumbing only. Live trading is not approved. Real-capital trading is not approved. Live exchange order submission is not approved.
