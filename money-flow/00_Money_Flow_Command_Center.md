...
| Field | Current State |
| --- | --- |
| Current implemented milestone | `PT-RT1.4` Paper Trading command-center cleanup and active timeframe cutover on top of completed `PT-RT1.3` candle-truth data-health semantics, `PT-RT1.2`, `PT-RT1.1C`, `PT-RT1.1B`, `PT-RT1.1A`, `PT-RT1`, `OB2.0`, and `EV-AUDIT1`; no strategy candidate promoted |
...
| Next recommended phase | Run/review a fresh PT-RT active-week observation focused on `1h`, `4h`, and `1d`; keep `15m` paused/legacy; review candle-truth data health, duplicate-open blocking, synthetic trades, compact-log stats, and 20 USDC probe audit/transport-gate status before any signed testnet transport |
...
| Paper trading | PT-RT1/PT-RT1.1A/PT-RT1.1B/PT-RT1.1C/PT-RT1.4 add synthetic paper-observation ledgers and runtime collection/review UI only. Active Week 1 paper review uses `1h`/`4h`/`1d`; `15m` is paused for noise reduction. No strategy paper runtime is production-approved from EV-AUDIT1 or PT-RT1 evidence |
