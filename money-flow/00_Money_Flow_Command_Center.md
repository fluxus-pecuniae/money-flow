...
- Current implemented phase: `SV1.14` Money Flow trade-anatomy and market-structure diagnostics; SV1.13 generated component-scoped Hyperliquid public evidence packs, SV1.13.1 clarified interpretation truth, SV1.13.2 added per-scenario `dynamic_equity_pct` sizing, and SV1.14 explains ETH `sleeve_1h`, 15m, and 4h behavior without changing Money Flow rules.
- Phase 7 status: accepted complete.
- Proposed next phase: manual founder/operator review of the Hyperliquid-only evidence interpretation before any explicitly scoped paper-trading design phase. SV1.12.5 shows Aster and Binance have 18 additional complete native-trade-count candidate files but need separate non-trading identity verification/seed/import; OKX and Coinbase need a trade-count source or explicit canonical-contract decision; Kraken needs archive/vendor/operator data.
