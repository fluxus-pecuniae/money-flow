...
- MF-ORIG-EV1 proves current Money Flow v1.2 is Money Flow-inspired rather than source-faithful to the original hierarchy. The reconstructed original hypotheses showed pre-gate aggregate PnL/drawdown improvement in the first pass, but failed candidate gate due control-pocket preservation, so no original hypothesis is production-approved.
...
- MF-ORIG-EV1 did not change production Money Flow rules, approve an original strategy, submit orders, call private/signed/order endpoints, use testnet prices as strategy truth, use dashboard date filters as canonical evidence, or directly verify the PDF text because the PDF was not present locally.
