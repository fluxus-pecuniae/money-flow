...
| SV2.1 | Founder-approved Hyperliquid 1D period evidence / Historical Replay | Public Hyperliquid mainnet `meta`/`candleSnapshot` evidence refresh targets the founder-approved requested/resolved universe, generated 90 ignored baseline 1D period packs for 2024, 2025, YTD, and ALL, then generated 1800 selected Historical Replay chart/trade JSON files and 810 evidence-only candidate/reference/wildcard pack directories across all 10 PT-RT1 paper-observation lanes. The earlier broad active-metadata run was rejected for founder review and removed from local generated outputs. This is 1D founder-review evidence only and does not supersede SV2.0.2 multi-timeframe canonical evidence. |
...
| MF-ORIG-EV1 | Original Money Flow reconstruction / gap matrix | The original Money Flow Trading System was first reconstructed from the prompt-provided Gerald Peters source summary as a Strategy Validation-only research family. It documents source-vs-v1.2 drift, treats `1d` as primary source timeframe, tests 5 EMA / 20 SMA stage/crossover, breakout, pullback/reclaim, and stage-filter hypotheses with RSI warning trims, MACD-as-TSI substitute, structure-stop proxies, and 1% risk sizing. Production Money Flow v1.2 remains unchanged. The PDF is now present in `money-flow/90 Reference/` for future source-exact reconciliation. |
...
| MF-ORIG-EV2 | Multi-timeframe evidence packs / Historical Replay UI | Corrected Original Money Flow hypotheses are replayed across BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX x 15m/1h/4h/1d x next_candle_open/next_candle_close. The run now includes four source-faithful 1% risk-sizing hypotheses plus four full-equity/notional comparison counterparts, generating 288 ignored evidence-pack directories, 612 ignored dashboard chart-data files, and compact committed Markdown/JSON summaries. Historical Replay and Evidence Run Ledger can visualize all eight MF-ORIG-EV2 strategies. Production Money Flow v1.2 remains unchanged and no hypothesis is approved. |
| EV-AUDIT1 | Full hypothesis/data/paper-readiness audit | Audits Money Flow v1.2, SOR-EV1/SOR-EV2/SOR-EV3, MF-ORIG-EV1.1/MF-ORIG-EV2, and STRAT-EV1 plan-only status without adding variants or regenerating packs. It scores data/methodology/candidate confidence, explains winners/losers/loss streaks/regimes/control pockets, lists P0/P1/P2/P3 issues, and concludes no clean strategy candidate is promoted. Current evidence is useful for visual review and hypothesis filtering only; PT-RT1 was later scoped as Paper Trading forward observation, not approval. |
| PT-RT1 / PT-RT1.1A | Real-time public-market paper observation substrate | Implements a forward-observation substrate rather than another evidence pack: Hyperliquid public mainnet market data is strategy truth, fully closed candle gating and indicators feed independent synthetic 10,000 USDC ledgers for 10 observation lanes, the expanded scanner keeps requested/resolved/blocked symbol truth visible, and a separate Hyperliquid testnet plumbing-probe lane is disabled/kill-switched/exact-approval gated by default. It does not regenerate canonical evidence packs, change rules, approve paper/live, or use testnet fills as strategy PnL. |
...
- SV2.1 proves the project can broaden public-mainnet 1D founder-review evidence across the founder-approved requested/resolved universe and visualize those period sets in Historical Replay, including the three conservative evidence-only candidates. It does not supersede SV2.0.2 canonical multi-timeframe evidence, approve candidates, or prove profitability. PEPE/kPEPE and OKB remain excluded by resolver policy; ASTER/TRUMP have no 2024 public 1D period pack.
...
- MF-ORIG-EV2 proves the corrected original reconstruction can be replayed over the same canonical SV2.0.2 multi-symbol/multi-timeframe substrate and visualized in Historical Replay. The founder-requested full-equity rows provide direct full-equity comparison lanes beside the source-faithful 1% risk rows, but still do not approve a hypothesis; `1d` source-primary rows remain weak versus positive 1d controls while 1h/4h/15m fractal/stress-test rows are useful founder-review evidence only.
- EV-AUDIT1 proves the evidence estate can be audited coherently across baseline, SOR, MF-ORIG, and pending discovery tracks. It identifies useful review candidates but no clean promoted strategy candidate, and it classifies current evidence as adequate for visual review / hypothesis filtering only.
- PT-RT1 proves the repo now has the plumbing to begin controlled forward observation using public mainnet strategy-truth data, but it does not produce completed 24-hour or 60-day observation results yet and it does not make any backtest result canonical forward evidence.
...
- SV2.1 did not change Money Flow rules, optimize parameters, approve variants, approve paper/live, submit orders, call private/signed/order endpoints, use API keys, use testnet data as strategy truth, or commit generated evidence/chart artifacts.
...
- MF-ORIG-EV1.1 did not change production Money Flow rules, approve an original strategy, submit orders, call private/signed/order endpoints, use testnet prices as strategy truth, use dashboard date filters as canonical evidence, start MF-ORIG-EV2, or complete a direct source-exact PDF reconciliation.
- MF-ORIG-EV2 did not change production Money Flow rules, approve an original strategy, submit orders, call private/signed/order endpoints, use testnet prices as strategy truth, use dashboard date filters as canonical evidence, import/refetch candles, or complete a direct source-exact PDF reconciliation.
- EV-AUDIT1 did not add or approve any strategy, did not approve paper/live, did not regenerate evidence packs, did not use dashboard date filters as canonical evidence, and did not resolve live-market paper observation, order-book/funding/partial-fill, or direct MF-ORIG PDF verification gaps.
- PT-RT1 did not approve any strategy, did not approve production paper runtime or live trading, did not submit live orders, did not call private/signed/order endpoints from the strategy-truth lane, did not use API keys, did not use testnet prices/fills as strategy truth or PnL, did not regenerate historical evidence packs, and did not add SOR/fanout/CBBO/routing behavior.
