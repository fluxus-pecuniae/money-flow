...
- SV1.17 2026-05-08 true replay experiment round 1: Strategy Validation compares lower-RSI plus market-structure replay variants across the Hyperliquid BTC/ETH/SOL x 15m/1h/4h suite under `dynamic_equity_pct`; some variants improve losing baselines, ETH `sleeve_1h` baseline remains the strongest above-starting-equity pocket, and no variant is authorized.
- SV1.18 2026-05-09 evidence credibility closeout and UAT candidate freeze: current evidence does not prove edge, exactly one UAT observation candidate is frozen (`money_flow_hyperliquid_eth_1h_baseline_uat_candidate`), and UAT is defined as plumbing/behavior validation rather than performance validation.

## Current Next Shape

See [[40 Operations/Future Work Roadmap]].

Current implemented phase: `SV1.18` evidence credibility closeout and UAT candidate freeze; SV1.13 generated component-scoped evidence packs, SV1.13.2 added per-scenario dynamic-equity sizing, SV1.14 diagnosed trade anatomy/market structure, SV1.15 compared isolated research overlays plus attribution against baseline, SV1.15.1 made the methodology limitations explicit, SV1.16 adds per-candle true replay context plus a lower-RSI research replay example, SV1.16.1 hardens replay-state field/counter semantics, SV1.17 tests lower-RSI plus market-structure variants across BTC/ETH/SOL and 15m/1h/4h without authorizing rule changes, and SV1.18 freezes only Hyperliquid ETH `sleeve_1h` baseline current rules as a UAT observation candidate.

The next proposed work is `UAT0` safety/security/runtime hardening for plumbing and behavior validation only. Broader replay can later expand symbols/components/fill-cost sensitivity and exact stop/invalidation replay if founder review asks for it. January 2026 remains archival/vendor-data-required. Aster/Binance identity verification/import, OKX/Coinbase trade-count sourcing, and Kraken archive/vendor/operator data are broader venue-comparison follow-ups. Paper-trading design remains deferred until founder/operator evidence review explicitly scopes it. Phase 8.1 remains deferred until explicitly scoped.
