...
| UAT3.0.5 | complete | UAT3.0.5 validates the exact private-read-only approval text and sandbox/testnet credential boundary; local credential env vars are missing, so no credentials were loaded, no API keys were used, no private endpoints were called, and live-fed sandbox drawdown remains blocked. |
| UAT3.1 | blocked | Actual sandbox order submission requires explicit approval plus live-fed sandbox drawdown, real sandbox submit path wiring, executable approval-scope gate wiring, risk gate wiring, and submit-lease integration verification. |
