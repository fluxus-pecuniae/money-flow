...
- Phase observed in repo memory: `SV1.16.1` replay methodology-truth hotfix.
- Latest implemented scope: Hyperliquid-first public evidence review, dynamic-equity interpretation, trade-anatomy diagnostics, controlled research-only hypothesis experiments, SV1.15.1 methodology-truth hardening, SV1.16 per-candle true replay substrate, and SV1.16.1 replay context/counter semantics. BTC/ETH/SOL Hyperliquid research identity is operator-verified by `Tercirafael`, non-trading, and non-strategy-eligible; the intended migrated `money_flow` DB contains `25848` imported public campaign candles; evidence review generated component-scoped packs for `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`; SV1.16/SV1.16.1 record production-rule decision/rejection context, RSI zones, EMA/MACD state, regimes, market-structure diagnostics, replay divergence truth, and separated rejection/admission/no-trade counters, then run a narrow lower-RSI ETH `sleeve_1h` true replay that underperformed the current baseline sampled scenario without changing production Money Flow rules. Aster/Binance remain later comparative candidates, OKX/Coinbase are blocked by missing public trade count, and Kraken is blocked by incomplete public REST coverage.
...
- Proposed next phase: broader true replay experiments or manual founder/operator review of the Hyperliquid-only constant-notional, dynamic-equity, trade-anatomy, methodology-hardened experiment, and SV1.16.1 replay-methodology evidence before any explicitly scoped paper-trading design phase. Aster/Binance identity verification/import and OKX/Coinbase/Kraken source blockers are broader venue-comparison follow-ups; paper/live trading remains deferred.
