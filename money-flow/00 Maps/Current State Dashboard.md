...
- Phase observed in repo memory: `SV1.17` true replay experiment round 1.
- Latest implemented scope: Hyperliquid-first public evidence review, dynamic-equity interpretation, trade-anatomy diagnostics, controlled research-only hypothesis experiments, SV1.15.1 methodology-truth hardening, SV1.16 per-candle true replay substrate, SV1.16.1 replay context/counter semantics, and SV1.17 ETH `sleeve_1h` lower-RSI plus market-structure replay experiments. BTC/ETH/SOL Hyperliquid research identity is operator-verified by `Tercirafael`, non-trading, and non-strategy-eligible; the intended migrated `money_flow` DB contains `25848` imported public campaign candles; evidence review generated component-scoped packs for `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`; SV1.17 found that no tested ETH `sleeve_1h` lower-RSI plus market-structure variant beat the current baseline sampled scenario. Support-confirmed admitted no replay-only trades, EMA10-hold/no-resistance reduced drawdown but still ended below baseline, and no production Money Flow rules changed. Aster/Binance remain later comparative candidates, OKX/Coinbase are blocked by missing public trade count, and Kraken is blocked by incomplete public REST coverage.
...
- Proposed next phase: manual founder/operator review of the Hyperliquid-only constant-notional, dynamic-equity, trade-anatomy, methodology-hardened experiment, SV1.16.1 replay-methodology, and SV1.17 true replay experiment evidence before any explicitly scoped paper-trading design phase. Broader replay may later expand symbols/components/fill-cost sensitivity and exact stop/invalidation replay. Aster/Binance identity verification/import and OKX/Coinbase/Kraken source blockers are broader venue-comparison follow-ups; paper/live trading remains deferred.
