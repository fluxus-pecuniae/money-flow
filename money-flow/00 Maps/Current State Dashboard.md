...
- Latest implemented scope: operator-verified market-identity write guard and requirement-aware candle-import preflight hardening before guarded imports.
...
- SV1.11.1 makes non-dry-run identity writes require explicit operator verification and adds requirement-aware preflight so files must prove exact canonical close-slot coverage before import; it writes no candles and generates no evidence packs.
