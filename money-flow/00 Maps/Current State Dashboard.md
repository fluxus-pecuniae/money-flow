...
Money Flow is a controlled trading-system substrate plus Strategy Validation research platform; PT-RT1.5.3 is the current Paper Trading forward-observation hotfix, with active `1h`/`4h`/`1d`, paused `15m`, warm-start fresh-signal gating, open-position MTM, and baseline-only fixed 25 USDC Hyperliquid testnet plumbing. The PT-RT1.5.3 smoke used testnet metadata / `szDecimals`, reached accepted/open, canceled, and reconciled without synthetic PnL impact. SV2.0.2 canonical evidence remains the historical baseline, SOR/MF-ORIG/STRAT tracks are separated, no clean production strategy candidate exists, and no live approval follows.
...
SV2.0.2 canonical evidence remains the current baseline for historical comparison. PT-RT1.1C local runtime artifacts under ignored `reports/paper_runtime/pt_rt1_1c_24h_dry_run/` are now pre-cutover burn-in context. Fresh Week 1 review should continue or restart with the PT-RT1.5.3 size/precision hotfix present.
...
| PT-RT1.5.3 | testnet_size_precision_hotfix_verified | Current Paper Trading should continue or restart with the PT-RT1.5.3 hotfix present; active timeframes are `1h`/`4h`/`1d`, `15m` is paused, startup-valid entries are blocked until fresh post-start transitions, baseline-only fixed 25 USDC Hyperliquid testnet transport is gated, and candidate/MF-ORIG/wildcard lanes are synthetic-only. The fixed-25-USDC smoke reached accepted/open, canceled, and reconciled without synthetic PnL impact. |
...
- continue or restart the active Week 1 runtime with PT-RT1.5.3 present, then review the next fresh baseline-triggered lifecycle row before making stronger Week 1 forward-observation claims.
...
| PT-RT1.5.3 | size/precision hotfix verified with accepted/open -> canceled -> reconciled fixed-25-USDC testnet smoke; active runtime should continue/restart with hotfix present; not approved for production/live |
