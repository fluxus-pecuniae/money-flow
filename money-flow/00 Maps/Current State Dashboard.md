...
- Phase observed in repo memory: `SV1.18` evidence credibility closeout and UAT candidate freeze.
- Latest implemented scope: Hyperliquid-first public evidence review, dynamic-equity interpretation, trade-anatomy diagnostics, controlled research-only hypothesis experiments, SV1.15.1 methodology-truth hardening, SV1.16 per-candle true replay substrate, SV1.16.1 replay context/counter semantics, SV1.17 full-suite lower-RSI plus market-structure replay experiments, and SV1.18 evidence-cycle closeout. SV1.18 is complete. BTC/ETH/SOL Hyperliquid research identity is operator-verified by `Tercirafael`, non-trading, and non-strategy-eligible; the intended migrated `money_flow` DB contains `25848` imported public campaign candles; evidence review generated component-scoped packs for `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`; SV1.18 freezes only Hyperliquid ETH `sleeve_1h` current baseline rules as a UAT observation candidate. 15m, 4h, BTC/SOL 1h, lower-RSI variants, market-structure variants, and cross-venue candidates are excluded from current UAT scope. Aster/Binance remain later comparative candidates, OKX/Coinbase are blocked by missing public trade count, and Kraken is blocked by incomplete public REST coverage.
...
- Proposed next phase: `UAT0` safety/security/runtime hardening for plumbing and behavior validation only. Paper trading is not approved. Live trading is not approved. Exchange order submission is not approved. Broader replay may later expand symbols/components/fill-cost sensitivity and exact stop/invalidation replay. Aster/Binance identity verification/import and OKX/Coinbase/Kraken source blockers are broader venue-comparison follow-ups; paper/live trading remains deferred.
