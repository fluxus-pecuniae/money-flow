...
- Phase observed in repo memory: `SV1.13.2` dynamic equity capital simulation.
- Latest implemented scope: Hyperliquid-first public evidence review plus dynamic-equity interpretation. BTC/ETH/SOL Hyperliquid research identity is operator-verified by `Tercirafael`, non-trading, and non-strategy-eligible; the intended migrated `money_flow` DB contains `25848` imported public campaign candles; evidence review generated component-scoped packs for `sleeve_15m`, `sleeve_1h`, and `sleeve_4h`; SV1.13.2 adds `dynamic_equity_pct` so each per-scenario validation run can size new trades from current realized equity. Aster/Binance remain later comparative candidates, OKX/Coinbase are blocked by missing public trade count, and Kraken is blocked by incomplete public REST coverage.
...
- Proposed next phase: manual founder/operator review of the Hyperliquid-only constant-notional and dynamic-equity evidence interpretation, using the dashboard where helpful, before any explicitly scoped paper-trading design phase. Aster/Binance identity verification/import and OKX/Coinbase/Kraken source blockers are broader venue-comparison follow-ups; paper/live trading remains deferred.
