...
- Current operating surface: `Evidence` defaults to the SV2.3 realistic backtest decision layer; `Historical Replay` remains latest public-mainnet chart inspection; `Paper Trading` remains the PT-RT forward-observation surface.
...
- SV2.2 dashboard refocus: the dashboard defaults to Historical Replay and loads `docs/sv2_2_hyperliquid_research_refresh_summary.json` so fresh public-mainnet `1h`/`4h`/`1d` replay rows for the three Week 2 selected strategies can be selected when ignored local chart files exist. Historical Replay now restricts default controls to exact SV2.2 rows for the three Week 2 strategies, hides the locked period selector, excludes `15m` and unsupported fills, uses compact arrows-only markers by default, and does not poll Paper Runtime status while Historical Replay is active.
- SV2.3 Evidence default: Evidence loads `docs/sv2_3_realistic_backtest_summary.json`, defaults to `Latest Evidence / SV2.3 realistic backtest`, and shows next-candle-open-only base/conservative/stress rows for the three Week 2 strategies. Current SV2.3 result: all three strategies are `not_promoted_realistic_gate_failed`.
...
- Next recommended action: keep Paper Trading runtime monitoring separate from SV2.3 Evidence review; use SV2.3 for realistic promotion-facing decisions and SV2.2 Historical Replay for chart inspection.
...
| Historical Replay | SV2.2 latest replay summary plus ignored selected chart/trade JSON for the three Week 2 strategies; older SV2.0.2/SV2.1/SOR/MF-ORIG artifacts remain reference data outside the default selector | Visual candle/indicator/trade review and fresh public-mainnet Week 2 strategy replay context | Do not treat date filters or SV2.2 latest replay payloads as production approval or canonical evidence replacement |
| Evidence | SV2.3 realistic backtest summary by default; SV2.0.2 batch reports and generated replay rows remain reference modes | Realistic next-open strategy review, run ledger, and comparison tables | Do not treat aggregate sums as one-account PnL or failed/passed gates as production approval |
...
- SV2.2 latest replay payloads are research/evidence-style review artifacts for the three Week 2 selected strategies only; they are not production approval, active PT-RT runtime behavior, or canonical SV2.0.2 replacement packs.
- SV2.3 Evidence rows are promotion-facing realistic research rows only; they use next-candle-open fills and execution-cost stress, but still do not approve production/live behavior.
...
- The sticky dashboard top bar carries the logo, title, theme selector, and five primary tabs: Paper Trading, Historical Replay, Evidence, The Lab, Strategy. As of SV2.3, Evidence is the latest realistic decision layer while Historical Replay remains visual inspection. Paper Trading maps to the underlying Paper Observation runtime panel. Manual JSON loading and the visible evidence-pack-loaded status text are intentionally removed from the founder chrome; Codex/repo artifacts manage local report loading.
...
- Local ignored chart-data files must exist for full candle/indicator/trade visualization; SV2.2 selected replay payloads are generated under `reports/strategy_validation/sv2_2_week2_replay_dashboard_chart_data/`.
