...
- STRAT-PRUNE1: remains recommendation-only; the founder overrode its suggested slate for Week 2 in PT-RT1.6.
- Production approval: no strategy is production-approved.
- Live trading: not approved; no real-capital trading is approved.
- Next recommended action: after founder review, start `pt_rt1_6_week2_active`.
...
STRAT-PRUNE1 is recommendation-only and was overridden by the founder for Week 2. PT-RT1.6 implements the founder-selected active slate: baseline control, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. The GOAL-STRAT2 non-existing candidates remain research-only unless a later phase explicitly scopes them.

| strategy_family | lane_or_variant | status | evidence_status | paper_status | testnet_transport_status | production_status | notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Current Money Flow v1.2 | `money_flow_v1_2_baseline` | `pt_rt1_6_week2_active_control`, `production_baseline_logic` | `canonical_sv2_0_2_historical_baseline` | `synthetic_paper_only` | `baseline_only_25_usdc_when_gates_pass` | `not_production_approved`, `not_live_approved` | Week 2 control lane; current derivative implementation with `sleeve_15m`, `sleeve_1h`, `sleeve_4h`, `sleeve_1d`. |
| SOR repair variants | `avoid_low_rolling_range_20` | `pt_rt1_6_week2_active_diagnostic`, `synthetic_paper_only`, `evidence_only` | `SOR-EV3 true-forward replay; promising_control_pocket_risk` | `synthetic_paper_only` | `cannot_send_testnet_orders` | `not_production_approved`, `not_live_approved` | Week 2 diagnostic comparator; synthetic-only. |
...
| MF-ORIG source reconstruction | `mf_orig_1d_stage2_breakout_resistance_full_equity` | `pt_rt1_6_week2_active_source_faithful_candidate`, `synthetic_paper_only`, `reference_only` | `MF-ORIG-EV2 evidence-only` | `synthetic_paper_only` | `cannot_send_testnet_orders` | `not_production_approved`, `not_live_approved` | Week 2 MF-ORIG candidate; synthetic-only and not production approved. |
...
| PT-RT1.6 founder-selected Week 2 slate | baseline + `avoid_low_rolling_range_20` + `mf_orig_1d_stage2_breakout_resistance_full_equity` | `active_week2_default`, `founder_selected` | `mixed evidence; selected for forward paper observation` | `synthetic_paper_only` | `baseline_only_if_gates_pass` | `not_production_approved`, `not_live_approved` | Exact Week 2 runtime default; runtime not started by PT-RT1.6. |
| STRAT-PRUNE1 recommended next slate | baseline + relative-strength rotation + Donchian breakout + `avoid_low_rolling_range_20` | `recommendation_only`, `founder_overridden_for_week2` | `read-only pruning review` | `not_runtime_changed_by_strat_prune1` | `baseline_only_if_future_gates_pass` | `not_production_approved`, `not_live_approved` | Research governance only; not the Week 2 active runtime slate. |
