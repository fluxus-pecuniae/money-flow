...
Strategy Validation is the current priority. No first real canonical Money Flow evidence packs have been generated yet. The intended local `money_flow` DB is migrated/current, but it contains zero persisted candles unless the guarded import is run against operator-provided files. SV1.12 adds the guarded canonical candle bundle import path: intended non-maintenance DB, migrated/current schema, operator-verified research-only/non-trading identity, timezone-explicit files, complete one-to-one requirement-aware preflight, and hardened importer success are all required before candle writes. The immediate next phase should run post-import canonical evidence review with evidence generation only after DB target/schema/identity/data readiness and import status are clean, and keep paper-trading design deferred until founder/operator evidence review justifies it.
...
- SV1.0-SV1.12: Strategy Validation research track. Implemented through intended local DB readiness, unique canonical import requirements, market-identity bootstrap, operator-verified non-trading identity write guard, complete requirement-aware candle-import preflight, and guarded canonical candle bundle import; no first real canonical evidence packs yet.
