...
Current implemented phase is `SV1.16` rejected-signal replay instrumentation. This Phase 8 note is historical/operator-control context; current active work is Strategy Validation research replay. Hyperliquid BTC/ETH/SOL identity is operator-verified as research-only/non-trading, the 9 Hyperliquid public YTD/recent files were imported with `25848` candles, SV1.13 generated component-scoped Hyperliquid-only evidence packs for founder review, and SV1.16 now adds per-candle rejected-signal replay context plus a narrow lower-RSI true replay substrate without changing production rules. It is not routing/SOR expansion or manual-resolution mutation.
