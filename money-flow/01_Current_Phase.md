...
`PT-RT1.1A` Expanded Paper Observation Universe + 10 Strategy Lanes is complete as the current implementation/readiness phase. PT-RT1 remains the forward-observation substrate rather than another backtest: a public-mainnet strategy-truth lane with fully closed candle gating, indicator computation, duplicate synthetic-signal prevention, independent synthetic 10,000 USDC paper ledgers, and a separate Hyperliquid testnet plumbing-probe lane that is disabled/kill-switched by default and never updates strategy paper PnL. PT-RT1.1A expands that substrate before runtime collection to exactly 10 observation lanes, founder-requested scanner symbols, requested/resolved symbol visibility, blocked-symbol reason codes, and dashboard lane/wildcard diagnostics. `PT-RT1.1` dry-run validation is still blocked because the expected ignored 24-hour runtime artifacts under `reports/paper_runtime/pt_rt1_1_24h_dry_run/` are absent; `PT-RT1.1B` may start that probes-disabled runtime collection. PT-RT1.2 cannot proceed until a real probes-disabled run passes. PT-RT1/PT-RT1.1/PT-RT1.1A do not change production Money Flow rules, generate or regenerate historical evidence packs, submit live or testnet orders, approve live trading, approve strategy paper runtime as production behavior, call private/signed/order endpoints from strategy truth, use API keys, use testnet prices as strategy truth, or add SOR/fanout/CBBO/routing behavior.
...
Run `PT-RT1.1B` 24-hour probes-disabled dry run first and retain ignored artifacts under `reports/paper_runtime/pt_rt1_1_24h_dry_run/`. PT-RT1.1 currently records this as blocked because those artifacts are missing; PT-RT1.1A completed the required expanded-lab readiness before the run. If public-mainnet data health, fully closed candle gating, duplicate prevention, indicator availability, and synthetic ledgers remain stable, start the 60-day public-mainnet forward-observation window. Testnet plumbing probes require separate exact approval text, are PT-RT1.2 or later, and remain blocked until the probes-disabled run passes.
