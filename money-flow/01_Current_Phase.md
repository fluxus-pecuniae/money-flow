...
The dashboard now exposes a `Paper Observation` tab that visualizes the committed PT-RT1 summary JSON for founder review: expanded scanner state, requested/resolved symbols, blocked symbols and reason codes, all 10 strategy lanes, lane detail, wildcard diagnostics, public-mainnet data health, a ticking public-mainnet watchlist, selected-pair TradingView chart from public `candleSnapshot`, open/closed synthetic runtime state empty states, drawdown/losing-streak state, and separate testnet plumbing probe status. The tab is display/runtime-observation UI only: dashboard date filters are display-only, testnet probes are plumbing-only, no order controls are present, and no paper/live approval follows from the dashboard.
...
`SV2.1` Broad Hyperliquid 1D Period Evidence is complete as a research-only data/evidence refresh. It used public Hyperliquid mainnet `meta` and `candleSnapshot` only, targeted 183 active public metadata symbols, imported available 1D candles into the intended local strategy-validation DB back to 2024-01-01 where available, and generated 646 ignored period packs across 2024, 2025, YTD, and ALL. This is 1D founder-review evidence breadth only. It does not change production Money Flow rules, supersede the SV2.0.2 multi-timeframe canonical baseline, approve paper/live, submit orders, call private/signed/order endpoints, use API keys, use testnet strategy truth, or add SOR/fanout/CBBO.
