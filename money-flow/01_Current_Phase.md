...
`OB2.0` Obsidian Strategy Brain + Evidence Architecture Refresh is complete as the current documentation/governance phase on top of completed `EV-AUDIT1`. OB2.0 refreshes the vault so founder/agents can distinguish current Money Flow v1.2, Original Money Flow / MF-ORIG, SOR repair variants, STRAT-EV plan-only discovery, canonical evidence methodology, dashboard display-only visualization, UAT sandbox plumbing, and future PT-RT1 paper-observation readiness. It does not change production Money Flow rules, generate evidence packs, regenerate dashboard chart data, submit orders, call exchange endpoints, use API keys, approve strategy paper runtime, approve live trading, or add SOR/fanout/CBBO/routing behavior.
...
`PT-RT1` is the recommended next phase if the founder wants real-time public market-data plus paper observation. It must be separately scoped and is not approved by OB2.0. PT-RT1 must be paper-observation only: trusted public mainnet candles as strategy truth, internal 10,000 USDC paper ledger, real-time signal/arrow/logging visibility, duplicate-signal protection, outage handling, and continued separation from sandbox execution plumbing. No exchange orders, no private/signed endpoints, no API keys, and no live trading. `MF-ORIG-EV3` or `SOR-EV4` may be scoped only after founder review and stricter control-pocket/out-of-sample-style slicing. No production-rule-change phase is approved from EV-AUDIT1, MF-ORIG-EV2, SOR-EV1, SOR-EV2, or SOR-EV3.
