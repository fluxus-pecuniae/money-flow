...
`PT-RT1.4.1` Active Week Runtime Cutover Verification + Daily Review Pack is implemented on top of `PT-RT1.4` Paper Trading Command Center Cleanup and `PT-RT1.3` Candle-Truth Data Health. PT-RT1 remains the forward-observation substrate rather than another backtest: a public-mainnet strategy-truth lane with fully closed candle gating, indicator computation, independent synthetic 10,000 USDC paper ledgers, and a separate Hyperliquid testnet plumbing-probe lane that never updates strategy paper PnL. PT-RT1.3 keeps stale/thin/missing public mids as warning-only when clean fully closed `candleSnapshot` rows are available; candle/indicator readiness remains the strategy gate. PT-RT1.4 makes Paper Trading the weekly command center. Active Week 1 paper review uses `1h`, `4h`, and `1d`; `15m` is `disabled_for_week1_noise_reduction`, excluded from active lane comparison/all-active totals/new synthetic entries, and preserved as paused/legacy data. PT-RT1.4.1 verified runtime truth: the old `reports/paper_runtime/pt_rt1_1c_24h_dry_run/` process continued creating 15m entries after cutover, is now labeled `pre_pt_rt1_4_weekend_burn_in`, and is excluded from active scoring. The restarted active runtime under ignored `reports/paper_runtime/pt_rt1_4_1_active_week/` reported `active_runtime_cutover_verified_after_restart`, `active_timeframes = 1h/4h/1d`, and 0 15m rows in its first artifact cycle. The current founder daily review source is [PT-RT Week 1 Daily Summary](../docs/pt_rt_week1_day_summary.md) plus `docs/pt_rt_week1_day_summary.json`, and the decision is `Week 1 paper observation may continue`. Strategy Lane Comparison defaults to selected timeframe only (`1h`), All active means `1h + 4h + 1d` and is not one combined account, Timeframe Breakdown makes the 15m pause visible, Open Synthetic Positions and Closed Synthetic Trades are founder-readable, Signal Generator is a categorized Signal / Decision Stream, and testnet labels split audit-only shapes from actual testnet order transport. Dashboard-started runs remain audit/order-shape mode for 20 USDC Hyperliquid testnet probes; testnet order transport is disabled in the normal Paper Trading view. Signed testnet transport remains only an explicit gated path behind `--submit-testnet-probes`, exact PT-RT1.2 transport approval, and a configured transport client. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping that process and machine awake/networked for the chosen session duration. PT-RT1/PT-RT1.1/PT-RT1.2/PT-RT1.3/PT-RT1.4/PT-RT1.4.1 do not change production Money Flow rules, approve live trading, approve strategy paper runtime as production behavior, call live endpoints, call private/signed/order endpoints from strategy truth, use API keys, use testnet prices as strategy truth, or add SOR/fanout/CBBO/routing behavior.
...
The dashboard now exposes a `Paper Trading` tab that visualizes PT-RT runtime summaries for founder review: all 10 strategy lanes, lane detail, public-mainnet data health, chart-first selected-pair TradingView candles from public `candleSnapshot`, opened/closed synthetic paper markers, a paginated Signal Generator for recorded synthetic `paper_opened` intended-entry decisions, paginated open/closed synthetic runtime state, drawdown/losing-streak state, separate testnet plumbing probe status, and a local-only Start Run / Stop Run control panel when served through `scripts/run_dashboard_control_server.py`. The underlying implementation still uses the `paper-observation` view id and PT-RT Paper Observation runtime naming, but the current founder-facing tab is Paper Trading. Wildcard Diagnostics now lives in the Strategy tab as observation-only lane context rather than taking space in Paper Trading. The Start Run path always forces `--enable-testnet-probes`, `--founder-approved-testnet-probes-20usdc`, `--testnet-probe-notional-usdc 20`, `--public-mainnet-only`, and compact decision logging; it exposes no order control, no arbitrary command, no private/signed/order endpoint path, no API-key path, and no testnet strategy truth. The tab shows decision-log mode, log size, rows written this cycle, repeated rows suppressed this cycle, and 20 USDC probe audit/order-shape status when runtime summaries provide those stats. The tab is display/runtime-observation UI only: dashboard date filters are display-only, testnet probes are plumbing audit rows only, no order controls are present, and no paper/live approval follows from the dashboard.
...
Continue the restarted PT-RT active-week observation session using active `1h`, `4h`, and `1d` timeframes only, then review daily summaries from `docs/pt_rt_week1_day_summary.md` / `.json` and ignored artifacts under `reports/paper_runtime/pt_rt1_4_1_active_week/`. Keep `15m` paused/legacy unless the founder explicitly re-enables it in a later phase. The review should confirm candle-truth data health, mid-warning rollups, fully closed candle gating, persisted open/flat state, duplicate same-candle open blocking, synthetic `trades.jsonl` close rows when exits occur, compact-log suppression/size stats, 20 USDC probe audit/order-shape rows, and no-order/no-live boundaries. Signed testnet transport should remain off unless the founder explicitly approves the PT-RT1.2 transport wording and a configured client is available.
