...
`SOR-EV4` may be scoped only if the founder selects a narrower hypothesis after reviewing SOR-EV3, and it must use stricter out-of-sample-style slices plus control-pocket preservation before any production-rule-change proposal. No production-rule-change phase is approved from SOR-EV1/SOR-EV2/SOR-EV3. `PT0.1` supervised top-20 paper/sandbox runtime week remains future work and must use trusted market data for strategy truth.
