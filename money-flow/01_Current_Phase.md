...
UAT3.1 may proceed only after explicit founder/operator approval for actual sandbox submission, sandbox runtime submission enablement, executable approval-scope wiring, submit-lease/lifecycle integration verification, risk gate wiring, and persistence-level sandbox artifact labeling. UAT3.0.5 cleared the private read-only live-fed sandbox drawdown verification blocker, but it did not approve actual sandbox order submission. UAT remains plumbing and behavior validation only. It is not paper trading, live trading, unrestricted exchange order submission, routing expansion, or strategy optimization.
