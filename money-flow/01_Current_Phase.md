...
`PT-RT1.3` Candle-Truth Data Health is implemented. PT-RT1 remains the forward-observation substrate rather than another backtest: a public-mainnet strategy-truth lane with fully closed candle gating, indicator computation, independent synthetic 10,000 USDC paper ledgers, and a separate Hyperliquid testnet plumbing-probe lane that never updates strategy paper PnL. PT-RT1.3 builds on PT-RT1.2 runtime state and fixes the latest data-health false positive: stale/thin/missing/nonpositive Hyperliquid public mids are warning-only when clean fully closed `candleSnapshot` rows are available. Candle validity, candle availability, and indicator readiness are the strategy blocking gates. TRUMP is deferred from fresh PT-RT paper-observation scanner runs because it created excessive runtime noise; historical SV2.1 evidence artifacts that already include TRUMP remain historical truth. The dashboard now separates blocking candle rows, non-blocking mid warning rows, lane-expanded `data_unavailable` decisions, open positions, duplicate-open blocks, transport mode, and the synthetic public-mainnet paper PnL source. Signal Generation loads recent ignored PT-RT `decisions.jsonl` rows so prior `paper_opened` rows remain visible even when the latest summary cycle has no new opens. Dashboard-started runs remain audit/order-shape mode for 20 USDC Hyperliquid testnet probes and the UI now separates audit/order-shape rows from signed testnet orders. Signed testnet transport is present only as an explicit gated path behind `--submit-testnet-probes`, exact PT-RT1.2 transport approval, and a configured transport client; no signed/order endpoint is called by default. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping that process and machine awake/networked for the chosen session duration. PT-RT1/PT-RT1.1/PT-RT1.2/PT-RT1.3 do not change production Money Flow rules, approve live trading, approve strategy paper runtime as production behavior, call live endpoints, call private/signed/order endpoints from strategy truth, use API keys, use testnet prices as strategy truth, or add SOR/fanout/CBBO/routing behavior.
