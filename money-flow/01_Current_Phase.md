...
The dashboard now exposes a `Paper Observation` tab that visualizes the committed PT-RT1 summary JSON for founder review: expanded scanner state, requested/resolved symbols, blocked symbols and reason codes, all 10 strategy lanes, lane detail, wildcard diagnostics, public-mainnet data health, public-mainnet chart placeholder, open/closed synthetic runtime state empty states, drawdown/losing-streak state, and separate testnet plumbing probe status. The tab is display/runtime-observation UI only: dashboard date filters are display-only, testnet probes are plumbing-only, no order controls are present, and no paper/live approval follows from the dashboard.
