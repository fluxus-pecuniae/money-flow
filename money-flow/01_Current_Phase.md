...
`PT0.0.2` Historical Strategy Replay Cockpit is complete, including a research-only MACD-removed replay selector beside the OG replay/strategy and RSI/MACD indicator panes inside the same TradingView chart/crosshair timeline for entry/exit review. The follow-up PT0 dashboard chart correctness Playwright hotfix is also complete. PAPER TRADING IS APPROVED. BROADER TOP-20 HYPERLIQUID-SUPPORTED PAPER/SANDBOX TRADING IS APPROVED.
...
`PT0.0.3` historical replay playback controls and market-structure inspector may be scoped next. `PT0.1` supervised top-20 paper/sandbox runtime week remains future work and must use trusted market data for strategy truth.
...
- PT0.0.3 may scope historical replay playback controls and market-structure inspection; it must preserve historical candle strategy truth and no-order/no-live boundaries.
