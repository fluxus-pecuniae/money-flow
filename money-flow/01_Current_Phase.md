...
`SV2.0.2` Hardened Candle DB Import + Canonical SV2 Evidence Pack Generation is complete. Normalized Hyperliquid public mainnet candles were imported through the hardened Strategy Validation candle importer into the intended migrated `money_flow` DB, canonical SV2.0.2 campaign configs were generated for `15m`, `1h`, `4h`, and `1d`, and ignored canonical evidence packs now exist for Money Flow v1.2. Supported canonical evidence symbols are BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, and AVAX; requested SHIB is represented as `kSHIB` but deferred from executable canonical evidence because unit semantics are not clean enough. Dynamic equity uses `10000` USDC per independent scenario, dataset-end open positions are force-closed explicitly, compact replay rows remain noncanonical, and SOR-EV1 may proceed from this baseline. `SV2.0.1` remains the evidence-truth hotfix: compact rows count entry fees at open, normalize Hyperliquid `.999Z` close timestamps to canonical slots, split fetched/normalized/staged data from DB-imported/canonical-evidence-ready data, set runtime sleeve allocations to `0.25` each, canonicalize internal `1d` while displaying `1D`, and treat missing EMA/RSI/MACD inputs as invalid instead of zero. `SV2.0` Money Flow 1D Sleeve + Expanded Historical Data Refresh + Evidence Rebuild is complete. Money Flow v1.2 includes `sleeve_15m`, `sleeve_1h`, `sleeve_4h`, and the real `sleeve_1d`; existing 15m/1h/4h settings remain unchanged. Testnet market data is not strategy truth. PAPER TRADING IS APPROVED. BROADER TOP-20 HYPERLIQUID-SUPPORTED PAPER/SANDBOX TRADING IS APPROVED.
...
SV2.0.2 is complete. SV2.0.1 and SV2.0 are complete.

## Next Proposed Phase

`SOR-EV1` stop-loss / strategy-variant evidence may be scoped next from the DB-backed SV2.0.2 canonical baseline. `PT0.1` supervised top-20 paper/sandbox runtime week remains future work and must use trusted market data for strategy truth.
