...
`PT-RT1` Real-Time Public Market Data Paper Observation + Testnet Plumbing Probes is complete as the current implementation phase. PT-RT1 adds a forward-observation substrate rather than another backtest: a public-mainnet strategy-truth lane with fully closed candle gating, indicator computation, duplicate synthetic-signal prevention, independent synthetic 10,000 USDC paper ledgers, and a separate Hyperliquid testnet plumbing-probe lane that is disabled/kill-switched by default and never updates strategy paper PnL. `PT-RT1.1` dry-run validation is blocked because the expected ignored 24-hour runtime artifacts under `reports/paper_runtime/pt_rt1_1_24h_dry_run/` are absent; PT-RT1.2 cannot proceed until a real probes-disabled run passes. PT-RT1/PT-RT1.1 do not change production Money Flow rules, generate or regenerate historical evidence packs, submit live orders, approve live trading, approve strategy paper runtime as production behavior, call private/signed/order endpoints from strategy truth, use API keys, use testnet prices as strategy truth, or add SOR/fanout/CBBO/routing behavior.
...
Run the PT-RT1 24-hour probes-disabled dry run first and retain ignored artifacts under `reports/paper_runtime/pt_rt1_1_24h_dry_run/`. PT-RT1.1 currently records this as blocked because those artifacts are missing. If public-mainnet data health, fully closed candle gating, duplicate prevention, indicator availability, and synthetic ledgers remain stable, start the 60-day public-mainnet forward-observation window. Testnet plumbing probes require separate exact approval text, are PT-RT1.2 or later, and remain blocked until PT-RT1.1 passes.
