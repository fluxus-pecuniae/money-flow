...
- Research discovery: `STRAT-DISC1` is complete as research-only; 50 local public-mainnet selected replay datasets were accepted, 12 bounded hypotheses were tested, and zero strategies passed the strict founder production-testing review gate.
...
`STRAT-DISC1` Autonomous Strategy Discovery is complete as a separate research-only phase. It adds a local selected-replay JSON data inventory, a bounded curated strategy-discovery harness, strict candidate gates, Markdown/JSON reports, and focused tests. The run tested Money Flow repair, original Money Flow/stage, trend/breakout, volatility expansion, mean reversion, relative strength, and pairs/spread research families. It found no founder production-testing review candidates without overfitting/risk blockers. The closest near misses remain research-only due drawdown, largest-loss, profit-factor, concentration, or market-structure limitations. STRAT-DISC1 changed no production Money Flow rules, mutated no PT-RT runtime artifacts, called no exchange/private/signed/order endpoints, submitted no orders, used no testnet strategy truth, approved no strategy for production, and approved no live trading.
...
Do not promote any STRAT-DISC1 result. A future discovery pass should first improve non-overlapping out-of-sample coverage and control-pocket slicing before widening parameters or strategy families.
