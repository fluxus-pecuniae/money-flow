...
Run `PT-RT1.1D` against the ignored artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`. PT-RT1.1A completed the expanded-lab readiness and PT-RT1.1B completed public-mainnet connector/runtime readiness before the run. PT-RT1.1D should evaluate public-mainnet data health, fully closed candle gating, duplicate prevention, indicator availability, synthetic ledgers, dashboard runtime readability, compact-log suppression/size stats, and no-order/no-live boundaries after completion. If the run passes, start the 60-day public-mainnet forward-observation window using compact logging unless a short diagnostic full-audit run is explicitly requested. Testnet plumbing probes require separate exact approval text, are PT-RT1.2 or later, and remain blocked until the probes-disabled run passes.
