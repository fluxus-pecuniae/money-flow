...
`SV2.0.1` Canonical Evidence Truth + 1D Data/Runtime Hotfix is complete. It fixes SV2 evidence/data/runtime truth without changing Money Flow rules: compact SV2 rows now count entry fees at open, force-close dataset-end open positions with explicit fields, normalize Hyperliquid `.999Z` public candle close timestamps to canonical `(start_at, end_at]` slots, split fetched/normalized/staged data from DB-imported/canonical-evidence-ready data, set runtime sleeve allocations to `0.25` each, canonicalize internal `1d` while displaying `1D`, and treat missing EMA/RSI/MACD inputs as invalid instead of zero. Canonical SV2 evidence packs remain blocked with `canonical_sv2_evidence_packs_missing` because refreshed public candles are staged-only and not DB-imported through the hardened importer. `SV2.0` Money Flow 1D Sleeve + Expanded Historical Data Refresh + Evidence Rebuild is complete. Money Flow v1.2 now includes `sleeve_15m`, `sleeve_1h`, `sleeve_4h`, and the real `sleeve_1d`; existing 15m/1h/4h settings remain unchanged. The new 1D sleeve is an initial non-optimized baseline: 50 bars, RSI 46-72, overbought 78, trim 84, max EMA5 extension 3.0%, and MACD required. SV2.0 resolves BTC/ETH/SOL/XRP/DOGE/HYPE/BNB/SUI/AVAX/SHIB through Hyperliquid public mainnet metadata, explicitly maps SHIB to `kSHIB`, fetches public mainnet `candleSnapshot` rows for 15m/1h/4h/1D where available, and writes compact readiness/evidence summaries. `4h` and `1D` reach the Jan 2025 target from public data; `15m` and `1h` carry `hyperliquid_public_5000_candle_limit`. Testnet market data is not strategy truth. PAPER TRADING IS APPROVED. BROADER TOP-20 HYPERLIQUID-SUPPORTED PAPER/SANDBOX TRADING IS APPROVED.
...
PT0.0.3 added 1D historical replay support and Jan 2025 data-horizon truth as an aggregated replay view only. SV2.0 supersedes that prior limitation by adding `sleeve_1d` as a real Money Flow sleeve and by using direct Hyperliquid public mainnet `1d` candle readiness/evidence where available. The Historical Replay strategy selector still includes OG replay / strategy, MACD removed, and Only close on 5/20 cross; both non-OG strategies are research-only and do not change production Money Flow rules. Historical Replay chart arrows select their linked trade in the Trade Inspector, which is organized around PnL, entry/exit reasons, indicator tiles, costs, and equity movement. The dashboard now shows Money Flow v1.2, expanded symbols, `sleeve_1d`, and SV2.0 readiness/evidence while keeping sandbox execution separate.

SV2.0.1 is complete. SV2.0 is complete.

## Next Proposed Phase

`SV2.x` full evidence-pack generation / DB-backed public candle import may be scoped next if the founder wants full ignored evidence-pack directories beyond the compact SV2.0.1 summary. SOR-EV1 / stop-loss / strategy-variant evidence remains blocked until canonical SV2 evidence packs exist or the team explicitly accepts staged compact rows as noncanonical input. `PT0.1` supervised top-20 paper/sandbox runtime week remains future work and must use trusted market data for strategy truth.
