...
Run `PT-RT1.1C` 24-hour probes-disabled dry run first and retain ignored artifacts under `reports/paper_runtime/pt_rt1_1b_24h_dry_run/`. PT-RT1.1 currently records the first 24-hour validation as blocked because those artifacts are missing; PT-RT1.1A completed the expanded-lab readiness and PT-RT1.1B completed public-mainnet connector/runtime readiness before the run. If public-mainnet data health, fully closed candle gating, duplicate prevention, indicator availability, and synthetic ledgers remain stable, start the 60-day public-mainnet forward-observation window. Testnet plumbing probes require separate exact approval text, are PT-RT1.2 or later, and remain blocked until the probes-disabled run passes.
