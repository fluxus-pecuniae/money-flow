...
`PT-RT1.1C` 24-Hour Runtime Collection artifacts are present locally and pending PT-RT1.1D evaluation. PT-RT1 remains the forward-observation substrate rather than another backtest: a public-mainnet strategy-truth lane with fully closed candle gating, indicator computation, duplicate synthetic-signal prevention, independent synthetic 10,000 USDC paper ledgers, and a separate Hyperliquid testnet plumbing-probe lane that never updates strategy paper PnL. PT-RT1.1A expanded that substrate before runtime collection to exactly 10 observation lanes, founder-requested scanner symbols, requested/resolved symbol visibility, blocked-symbol reason codes, and dashboard lane/wildcard diagnostics. PT-RT1.1B added the public-mainnet connector, runtime command, ignored runtime summary writer, dashboard connection status, and bounded smoke validation: `meta` and `allMids` connected, the 25-symbol requested universe resolved, 23 rows were scanner-eligible, 2 rows were blocked, 80 bounded paper decision events were recorded, and no orders/private/signed/order endpoints/API keys/testnet probes were used. PT-RT1.1C produced ignored artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`; the local artifact set currently contains about 479k decision rows, 0 trade rows, and a latest summary timestamp of `2026-05-15T22:22:12Z`. A later local run showed that full decision logging can grow `decisions.jsonl` to operationally large sizes, so runtime starts now default to `--decision-log-mode compact`: actionable `paper_opened`/`paper_closed` rows and `data_unavailable` rows are preserved, repeated identical non-actionable rows are suppressed across cycles, and summary files expose log size plus suppression stats. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping that process and machine awake/networked for the chosen session duration. The dashboard now has a localhost-only `scripts/run_dashboard_control_server.py` Start Run / Stop Run helper that launches allowlisted public-mainnet runtime sessions through Mac `caffeinate` with compact logging and 20 USDC Hyperliquid testnet probe audit/order-shape rows enabled for synthetic open signals. The probe lane records order-shape/audit rows only; the runtime records `order_endpoint_called=false` and `signed_order_endpoint_called=false`, and testnet fills/prices never update strategy paper PnL. Static dashboard serving still leaves controls unavailable. PT-RT1.1D still needs to evaluate the completed artifacts, compact-log health, and probe audit trail. PT-RT1/PT-RT1.1/PT-RT1.1A/PT-RT1.1B/PT-RT1.1C do not change production Money Flow rules, submit live or testnet orders, approve live trading, approve strategy paper runtime as production behavior, call private/signed/order endpoints from strategy truth, use API keys, use testnet prices as strategy truth, or add SOR/fanout/CBBO/routing behavior.
...
The dashboard now exposes an `Audit Review` tab that visualizes the committed EV-AUDIT1 summary JSON for founder review: audit verdict cards, methodology scorecard, paper-observation readiness, top/worst hypotheses, biggest winning/losing trades, losing streaks, audit issue list, data-integrity rows, and evidence inventory. This is display-only and does not regenerate canonical evidence packs or approve any strategy/paper/live behavior.

The dashboard now exposes a `Paper Observation` tab that visualizes the committed PT-RT1 summary JSON for founder review: expanded scanner state, requested/resolved and blocked-symbol metadata, all 10 strategy lanes, lane detail, wildcard diagnostics, public-mainnet data health, a compact 1-second public-mainnet watchlist with symbol / mid / health, selected-pair TradingView chart from public `candleSnapshot`, a Signal Generation panel for recorded synthetic `paper_opened` intended-entry decisions, open/closed synthetic runtime state empty states, drawdown/losing-streak state, separate testnet plumbing probe status, and a local-only Start Run / Stop Run control panel when served through `scripts/run_dashboard_control_server.py`. Watchlist health is `unhealthy` when the latest market-data tick is missing or stale for more than 2 minutes. The Start Run path always forces `--enable-testnet-probes`, `--founder-approved-testnet-probes-20usdc`, `--testnet-probe-notional-usdc 20`, `--public-mainnet-only`, and compact decision logging; it exposes no order control, no arbitrary command, no private/signed/order endpoint path, no API-key path, and no testnet strategy truth. The tab now shows decision-log mode, log size, rows written this cycle, repeated rows suppressed this cycle, and 20 USDC probe audit/order-shape status when runtime summaries provide those stats. The tab is display/runtime-observation UI only: dashboard date filters are display-only, testnet probes are plumbing audit rows only, no order controls are present, and no paper/live approval follows from the dashboard.
...
Run `PT-RT1.1D` against the ignored artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`. PT-RT1.1A completed the expanded-lab readiness and PT-RT1.1B completed public-mainnet connector/runtime readiness before the run. PT-RT1.1D should evaluate public-mainnet data health, fully closed candle gating, duplicate prevention, indicator availability, synthetic ledgers, dashboard runtime readability, compact-log suppression/size stats, 20 USDC testnet probe audit/order-shape rows, and no-order/no-live boundaries after completion. If the run passes, start the 60-day public-mainnet forward-observation window using compact logging unless a short diagnostic full-audit run is explicitly requested. Any future real signed testnet transport submission remains separately scoped and is not performed by PT-RT1.
