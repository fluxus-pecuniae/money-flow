...
`PT-RT1.2` Runtime State And 20 USDC Testnet Transport Gates are implemented. PT-RT1 remains the forward-observation substrate rather than another backtest: a public-mainnet strategy-truth lane with fully closed candle gating, indicator computation, independent synthetic 10,000 USDC paper ledgers, and a separate Hyperliquid testnet plumbing-probe lane that never updates strategy paper PnL. PT-RT1.2 fixes the latest local-run correctness findings: processed signal keys, open synthetic positions, realized equity by lane, and last processed close state now persist in each output directory's `state.json`; repeated same-candle `paper_opened` attempts become held/blocked rows instead of new opens; synthetic closes append to `trades.jsonl`; and `summary.json` separates public market-data unavailable rows from lane-expanded `data_unavailable` decisions. The dashboard now surfaces open positions, duplicate-open blocks, market-row unavailable counts versus lane-expanded decisions, transport mode, submitted/cancel/reconcile counts, and the synthetic public-mainnet paper PnL source. Dashboard-started runs remain audit/order-shape mode for 20 USDC Hyperliquid testnet probes. Signed testnet transport is present only as an explicit gated path behind `--submit-testnet-probes`, exact PT-RT1.2 transport approval, and a configured transport client; no signed/order endpoint is called by default. PT-RT1 is not an always-on hosted service: new signal generation requires starting the runtime and keeping that process and machine awake/networked for the chosen session duration. PT-RT1/PT-RT1.1/PT-RT1.2 do not change production Money Flow rules, approve live trading, approve strategy paper runtime as production behavior, call live endpoints, call private/signed/order endpoints from strategy truth, use API keys, use testnet prices as strategy truth, or add SOR/fanout/CBBO/routing behavior.
...
Run a fresh `PT-RT1.2` observation session and review the ignored artifacts under the selected `reports/paper_runtime/.../` output directory. The review should confirm public-mainnet data health, fully closed candle gating, persisted open/flat state, duplicate same-candle open blocking, synthetic `trades.jsonl` close rows when exits occur, compact-log suppression/size stats, data-unavailable market-row versus lane-decision rollups, 20 USDC probe audit/order-shape rows, and no-order/no-live boundaries. Signed testnet transport should remain off unless the founder explicitly approves the PT-RT1.2 transport wording and a configured client is available.
