...
- Dashboard state: `DASH-PT1.1` cleans the Paper Trading UI before Week 2. Paper Trading now uses Cockpit / Global Filters, Live Public Candles + Paper Markers, Runtime Control, Watchlist | Testnet Order Lifecycle, Open/Closed Synthetic Positions, Signal / Decision Stream, then lower-priority diagnostics. The visible Audit tab is removed. `LOG-OBS1` adds read-only Runtime Logs metadata and the terminal helper `scripts/watch_pt_rt1_runtime.py`.
