...
`PT-RT1` may be scoped next if the founder wants real-time public market-data plus paper observation. It must be an observation/runtime phase only: trusted public mainnet candles as strategy truth, internal 10,000 USDC paper ledger, real-time signal/arrow/logging visibility, duplicate-signal protection, outage handling, and continued separation from sandbox execution plumbing. `MF-ORIG-EV3` may be scoped only if the founder wants deeper Original Money Flow refinement after reviewing MF-ORIG-EV2 and EV-AUDIT1. `SOR-EV4` may be scoped only if the founder selects a narrower hypothesis after reviewing SOR-EV3/EV-AUDIT1, and it must use stricter out-of-sample-style slices plus control-pocket preservation before any production-rule-change proposal. No production-rule-change phase is approved from EV-AUDIT1, MF-ORIG-EV2, SOR-EV1, SOR-EV2, or SOR-EV3.
