...
`SV2.1` Broad Hyperliquid 1D Period Evidence is complete as a research-only data/evidence refresh and now has Historical Replay artifacts. It used public Hyperliquid mainnet `meta` and `candleSnapshot` only, targeted 183 active public metadata symbols, imported available 1D candles into the intended local strategy-validation DB back to 2024-01-01 where available, and generated 646 ignored baseline period packs across 2024, 2025, YTD, and ALL. The follow-up builder generated 5116 ignored selected Historical Replay chart/trade JSON files under `reports/strategy_validation/sv2_1_broad_1d_dashboard_chart_data/20260514T220500Z` and 1912 ignored evidence-only candidate pack directories for `avoid_low_rolling_range_50`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`. MF-ORIG candidate rows are skipped for 7 symbols with incomplete indicator context; baseline and rolling-range rows remain available where period packs exist. This is 1D founder-review evidence breadth only. It does not change production Money Flow rules, supersede the SV2.0.2 multi-timeframe canonical baseline, approve paper/live, submit orders, call private/signed/order endpoints, use API keys, use testnet strategy truth, or add SOR/fanout/CBBO.
...
Let `PT-RT1.1C` finish the 24-hour probes-disabled dry run and retain ignored artifacts under `reports/paper_runtime/pt_rt1_1c_24h_dry_run/`. PT-RT1.1A completed the expanded-lab readiness and PT-RT1.1B completed public-mainnet connector/runtime readiness before the run. PT-RT1.1D should evaluate public-mainnet data health, fully closed candle gating, duplicate prevention, indicator availability, synthetic ledgers, dashboard runtime readability, and no-order/no-live boundaries after completion. If the run passes, start the 60-day public-mainnet forward-observation window. Testnet plumbing probes require separate exact approval text, are PT-RT1.2 or later, and remain blocked until the probes-disabled run passes.
