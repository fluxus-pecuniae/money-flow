...
- Local sandbox/testnet credential environment variables are missing: `HYPERLIQUID_UAT_SANDBOX_PRIVATE_KEY`, `HYPERLIQUID_UAT_SANDBOX_ACCOUNT`, and `HYPERLIQUID_UAT_SANDBOX_BASE_URL`.
...
- Hyperliquid sandbox account-state payload parsing can build a `sandbox_account` / `not_live_account` drawdown feed from caller-supplied sandbox account truth.
- Because local credentials were missing, no credentials were loaded, no API keys were used, no private endpoints were called, and live-fed sandbox drawdown remains `sandbox_drawdown_feed_missing`.
- UAT3.1 actual sandbox order submission remains blocked.
- No order intent, prepared order, submitted order, executable approval, private endpoint call, order endpoint call, exchange API-key use, paper/live behavior, evidence pack, routing artifact, or Money Flow rule change was created.
...
- UAT3.1 needs live-fed sandbox account drawdown wiring; UAT3.0.5 validated approval but could not verify live-fed drawdown because local sandbox/testnet credential environment variables were missing.
