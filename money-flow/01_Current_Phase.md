...
`PT0.0.3` Historical Data Horizon + 1D Replay Support is complete. Historical Replay now supports 15m/1h/4h/1D selection, loads `docs/pt0_0_3_historical_strategy_replay_summary.json`, shows Jan 2025 target-start readiness truth, and labels 1D as deterministic aggregation from 4h historical replay candles rather than a new production Money Flow 1D sleeve. The current committed local data does not reach `2025-01-01T00:00:00Z`. PAPER TRADING IS APPROVED. BROADER TOP-20 HYPERLIQUID-SUPPORTED PAPER/SANDBOX TRADING IS APPROVED.
...
PT0.0.3 adds 1D historical replay support and Jan 2025 data-horizon truth. Current committed data starts in 2026, so missing Jan 2025 history is reported as data readiness, not strategy failure. 1D is deterministically aggregated from 4h historical replay candles and is not a new production Money Flow sleeve.

SV1.18 is complete.

## Next Proposed Phase

`PT0.0.4` historical data backfill and replay regeneration may be scoped next. `PT0.1` supervised top-20 paper/sandbox runtime week remains future work and must use trusted market data for strategy truth.
