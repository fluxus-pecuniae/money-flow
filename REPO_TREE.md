...
- SV1.16 is not yet wired into the dashboard; the founder-readable replay result is committed under `docs/` and the static dashboard remains visualization-only for local evidence artifacts.
...
- Founder/operator-readable SV1.16 replay-instrumentation report.
- Summarizes per-candle replay context, rejected-signal capture, baseline-vs-lower-RSI true replay metrics for Hyperliquid ETH `sleeve_1h`, and deferred true-replay work while keeping production Money Flow rules, paper/live trading, routing, and execution behavior unchanged.
...
- Includes `scripts/run_money_flow_true_replay.py` for SV1.16 replay instrumentation. It runs baseline and lower-RSI trend-intact research replays over imported candles, writes Markdown/optional JSON, and creates no evidence packs, imports, exchange calls, paper/live artifacts, routing artifacts, or production Money Flow rule changes.
