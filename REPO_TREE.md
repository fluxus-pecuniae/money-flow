...
Last reviewed: `2026-06-12T14:30:00Z`
...
- EXEC-EV1 adds `services/execution_quality/exec_ev1.py` (depth-aware modeled friction model), `scripts/run_exec_ev1_execution_quality.py` (research-only evidence runner), `docs/exec_ev1_execution_quality_evidence.md`, `docs/exec_ev1_execution_quality_evidence_summary.json`, and `tests/test_exec_ev1_execution_quality.py`. It re-scores the three Week 2 lanes under a depth-aware friction layer on top of SV2.3's fee/slippage/adverse-gap terms: per-symbol liquidity-tier half-spread, size-aware square-root market impact (participation = notional / candle-dollar-volume liquidity proxy), and a fill-probability unfilled-chase penalty. EXEC-EV1 cost is always >= the SV2.3 parent cost, so net PnL is always <= SV2.3's (0 violations across 621 rows). Depth is MODELED from historical candle volume — it is NOT real historical order-book depth (which does not exist; Hyperliquid public `l2Book` is a current snapshot only) — and every output is labeled an assumption layer. Also reports a late-entry / entry-timing cost metric (signal candle to fills 0/1/2 candles late) informing the future RT-HISTSEED1 decision. The runner reads SV2.2 candles from disk with no network I/O; the test suite is deterministic and gates the blocking CI lane. Partially addresses K-001 (modeled, not real, depth). No runtime/strategy/order/testnet/live/approval change.
- SEL-EV1 adds `services/strategy_validation/strategy_types.py` (Must 0 strategy-type routing seam: `per_symbol` vs `cross_sectional_selection`, each routed to its own simulator/gate; cross-application raises `StrategyTypeRoutingError`; the three Week 2 lanes are tagged `per_symbol` with behavior/results unchanged), `services/strategy_validation/sel_ev1.py` (point-in-time cross-sectional selection simulator, bounded breakout/relative-strength signals, matched-cadence seeded random-selection benchmark, equal-weight buy-and-hold + naive past-return baselines, rotation/diversity metrics, chronological 70/30 + anchored walk-forward thirds OOS, late-entry sensitivity, selection gate), `scripts/run_sel_ev1_selection_evidence.py` (research-only evidence runner over SV2.2 candles, no network I/O), `tests/test_sel_ev1_selection_evidence.py` (15 deterministic offline tests in the blocking CI lane, incl. a byte-identical per-symbol golden regression at `tests/fixtures/sel_ev1/goal_strat1_per_symbol_golden.json` and a synthetic lookahead leak that must be caught), `docs/sel_ev1_selection_evidence.md`, and `docs/sel_ev1_selection_evidence_summary.json`. SEL-EV1 supersedes the planned GOAL-STRAT3 breadth gate (wrong lens for a strategy designed to concentrate; the breadth-gate idea is deferred) and reframes the ZEC lesson as a rotation/diversity check. Verdict: `no_selection_skill_demonstrated` — the train-chosen config lost out-of-sample post-conservative-friction and beat only 2 of 50 random seeds. Friction is EXEC-EV1 depth-aware (modeled, not real, depth) on every entry/exit/rotation fill. No runtime/strategy/order/testnet/live/approval change.
- DASH-PT2 adds `docs/dash_pt2_prototype.html` (founder visual prototype / design target), `docs/dash_pt2_bolder_exchange_reskin.md`, `docs/dash_pt2_bolder_exchange_reskin_summary.json`, `docs/dash_pt2_screenshots/` (before/after Playwright captures: desktop+mobile dark, desktop light/red-zone), and `scripts/capture_dash_pt2_screenshots.py` (localhost-only screenshot verification tool). It elevates the Paper Trading terminal in place to a bolder, denser, color-coded exchange aesthetic: theme-aware DASH-PT2 tokens in `apps/dashboard/evidence-dashboard.css` (per-lane accents baseline/diagnostic/candidate mapped to current_truth active lanes, --accent-live, --accent-testnet), a dense status strip, lane chips, accent-underlined blotter tabs with dense monospace tables, and an accent-gradient active nav tab. JS changes are display-only markup (paperObservationLaneChip + status-strip state classes); every #paper-observation-* id, behavior, and DASH-QA1 guard is preserved (9/9 green); --color-chart-* tokens untouched; cache-buster bumped on CSS+JS. Dashboard display only — no runtime/strategy/data/order/testnet/approval change.
- DASH-IA1 consolidates the dashboard to two navigation surfaces (Paper Trading default + Research Log) and removes the Historical Replay / The Lab (evidence-lab) / Strategy tabs together with all render/data-loading code and CSS exclusive to those views plus the old Evidence renderers: `apps/dashboard/evidence-dashboard.js` 12,004 -> 6,753 lines (-43.7%), `evidence-dashboard.css` 4,448 -> 3,706, `index.html` 1,088 -> 620. Adds the data-driven Research Log placeholder (committed `docs/*_summary.json` verdict rows; full view in RLOG1), the Must 1b Paper Trading reflow (full-width Global Filters bar, skinnier rails, full-width Testnet Order Transport footer below Daily Review), and the Money Flow OS rename. Adds `docs/dash_ia1_two_tab_consolidation.md`, `docs/dash_ia1_two_tab_consolidation_summary.json`, and `docs/dash_ia1_screenshots/`. DASH-QA1 + `tests/test_dashboard_static_assets.py` updated in lockstep (check #7 relocated to the Paper Trading status strip; retired-tabs-absent checks added). NOTHING deleted from disk — all SOR-EV/MF-ORIG/SV2.x evidence packs, replay JSON, builder scripts, and docs remain as reference; hidden non-nav UAT legacy panels unchanged; the Paper Trading chart/markers untouched. Dashboard display only.
- DOC-LEAN1 adds `CHANGELOG_ARCHIVE.md` (verbatim archive of the 272 older changelog entries, v2026.06.08.001..v2026.04.06.017) and trims `CHANGELOG.md` to the recent rolling window (~20 entries, all new entries still written there; rotate the oldest into the archive when it exceeds ~25 — CI-enforced). Both files together are the complete canonical history; nothing was deleted. AGENTS.md Pre-Task Workflow is now the lean current-state read set (AGENTS.md, CURRENT_TRUTH.md, 01_Current_Phase.md, recent CHANGELOG.md, 05_Agent_Coordination.md, task-relevant component docs) with REPO_TREE/KNOWN_ISSUES/TODO/archive/Command Center/Project Memory consult-on-demand; the post-task update list is unchanged. Adds `docs/doc_lean1_changelog_rotation_and_lean_reading.md` + summary json. Docs/tests only.
- DASH-PT3 adds header critical-state pills (#top-runtime-pill from the existing control-server polling; #top-live-pill LIVE DISABLED · NOT APPROVED), moves the dense status strip to the final reference band after the Testnet footer, makes Global Filters lead the body, widens the rails (watchlist minmax(212px,252px); Runtime Control minmax(300px,344px) + overflow visible so nothing truncates), filters hidden symbols (OKB) out of the watchlist display only (resolver policy untouched), and removes the Daily Review nested scroll. Adds docs/dash_pt3_* + docs/dash_pt3_screenshots/. DASH-QA1 + static-asset guards updated in lockstep. Chart/markers untouched; dashboard display only.
- TSMOM-EV1 adds `services/strategy_validation/tsmom_ev1.py` (vol-targeted time-series momentum: causal trailing-return signal, equal-risk-budget sizing with leverage/weight caps, mark-to-market simulator with EXEC-EV1 friction, buy-hold/always-long/vol-targeted-beta/random benchmarks, buy-hold risk-adjusted gate with honesty qualifiers), a third `strategy_types.py` route (`time_series_momentum`, gate `tsmom_buy_hold_risk_adjusted_gate`), `scripts/run_tsmom_ev1_evidence.py`, `docs/tsmom_ev1_*`, and `tests/test_tsmom_ev1_evidence.py` (blocking CI). Research/evidence only; perp funding unmodeled (documented).
- FUND-EV1 adds `services/strategy_validation/fund_ev1.py` (delta-neutral funding carry: two-leg HL perp+spot simulator with a pending-fill queue so a lagged hedge leg holds REAL one-leg exposure, exact daily funding accrual on the perp leg, causal trailing-funding tilt, EXEC-EV1 friction + fees on both legs, regime/tail analytics, net-OOS+tail gate), a fourth `strategy_types.py` route (`funding_carry`, gate `funding_carry_net_oos_tail_gate`), `scripts/fetch_fund_ev1_funding_snapshot.py` (one-shot public read-only fundingHistory + HL spot candleSnapshot fetch; committed daily funding sums + sha256 provenance in `docs/fund_ev1_funding_data_snapshot_summary.json`; raw hourly/spot payloads stay as documented ignored artifacts under `/tmp/money-flow-fund-ev1/`), `scripts/run_fund_ev1_evidence.py`, `docs/fund_ev1_*`, and `tests/test_fund_ev1_evidence.py` (blocking CI). Research/evidence only; verdict `carry_does_not_survive_costs_and_tail_oos` (authored fail); spot borrow + liquidation mechanics unmodeled (documented).
- FUND-EV2 adds `services/strategy_validation/fund_ev2.py` (the one honest realistic-cost re-test: CITED per-venue/per-asset/per-leg cost models — HL fee docs + one-shot l2Book calibration + Kraken Pro base tier for the cross-venue leg — with a sweep-scale dial, gate v2 wrapper with cost breakpoint + fragility qualifier, both venue constructions), ADDITIVE seams on `fund_ev1.py` (optional `leg_cost_model`, per-config rebalance band, 2x entry-margin selectivity with hold-while-favorable hysteresis; defaults byte-identical), the `fund_ev2_` routing prefix (same funding_carry type/gate), `scripts/fetch_fund_ev2_l2book_calibration.py` (public read-only l2Book snapshot committed as `docs/fund_ev2_l2book_calibration_summary.json`), `scripts/run_fund_ev2_evidence.py`, `docs/fund_ev2_*`, and `tests/test_fund_ev2_evidence.py` (blocking CI). Verdict `carry_does_not_survive_realistic_costs_and_tail_oos` (authored fail): the OOS edge dies at 0.75x the cited cost level — funding carry CLOSED at 10k retail; no FUND-EV3 cost tweak (discipline guard).
- FUND-SCALE1 adds `services/strategy_validation/fund_scale1.py` (the size/fee axis FUND-EV2 sanctioned: cited Hyperliquid T0-T6 + Kraken Pro tier tables, tiered cost models that move only the fee term, own-volume tier achievement — HL 14d weighted with spot double, Kraken 30d — participation-plausibility cap at 10% of candle $ volume, and the COMPUTED viability band), additive seams on `fund_ev1.py` (`starting_equity` + max-fill participation/notional tracking; defaults byte-identical), `scripts/run_fund_scale1_evidence.py` (5 sizes x 5 tiers x 2 constructions sweep with per-rung train-only choice + full gate battery + maker-bound non-gateable line), `docs/fund_scale1_*`, and `tests/test_fund_scale1_evidence.py` (blocking CI). Verdict `carry_does_not_reach_viability_at_credible_scale` (authored fail): the achieved-tier surface is negative at every size and worsens with size; funding carry closed on both sanctioned axes.
- TREND-OVERLAY1 adds `services/strategy_validation/trend_overlay1.py` (read-only drawdown-control signal calculator: reuses the exact TSMOM-EV1 `tsmom_signal`/`realized_vol_annual`/`target_weights` under the train-chosen `tsmom_ev1_lb30_vt20_long_only_1d` config — defaults PINNED BY TEST to the committed evidence summary, re-tuning fails CI; closed-candle-only no-lookahead filter; the drawdown-control-not-alpha disclaimer embedded in every output) and `scripts/run_trend_overlay.py` (CLI: public read-only candleSnapshot fetch of the eight liquid majors or `--input-json` offline replay; prints the signal table and writes the ignored runtime artifact `reports/trend_overlay/current_trend_overlay.json`). Signal only — no orders, no auto-execution, no approval surface; OS panel deliberately skipped (documented; DASH-QA1 untouched). `docs/trend_overlay1_*`, `tests/test_trend_overlay1.py` (blocking CI), `.gitignore` gains `reports/trend_overlay/`.
- DATA1 adds `services/market_data/data1_multi_venue.py` (the multi-venue public read-only data foundation: venue x asset x series catalog with explicit `venue_lacks_market` gaps, paginated per-venue fetchers over an injected transport restricted to a public-endpoint allowlist, strict normalizers — midnight-UTC daily-candle guard so the OKX UTC+8 default bar is refused, per-venue funding-rate fields, FUND-EV1 daily funding aggregation with per-day event counts — union-calendar alignment with explicit `None` holes (no forward-fill, no truncation), and the `load_data1_dataset` loader with sha256 artifact verification + coverage flags), `scripts/fetch_data1_multi_venue_snapshot.py` (one-shot public read-only snapshot across hyperliquid/binance/bybit/okx/coinbase/kraken for BTC ETH SOL XRP DOGE BNB AVAX; raw native payloads -> documented ignored artifacts under `/tmp/money-flow-data1/raw_series/`; committed provenance + sha256 + coverage + funding-interval + gap report in `docs/data1_multi_venue_snapshot_summary.json`), `docs/data1_*`, `tests/test_data1_multi_venue.py` (blocking CI), and `tests/test_data1_live_smoke.py` (env-gated live canary, skipped in CI). Data ingestion only; no strategy logic, no runtime change.
- MF-REPLAY1 adds founder visual backtesting (hypothetical replay, not evidence): `services/paper_runtime/mf_replay1.py` (range-replay engine — runs the committed PT-RT2 lane semantics through `moneyflow_signal1.signal_states`, one code path; pre-registered fresh-start range semantics; Decimal arithmetic; all-time/year/custom ranges; no-lookahead truncation probe; surfaces `max_gross_exposure_x`/`max_concurrent_positions` per K-037 and the committed characterization on every range), `scripts/build_mf_replay1_dashboard_data.py` (ignored `reports/mf_replay1/replay_pack.json` for the dashboard), `scripts/refresh_data1_snapshot.py` (append-only newly-closed candles, idempotent) + `scripts/export_data1_csv.py` (portable per-symbol OHLCV CSVs), the durable DATA1 snapshot move (`var/data1/raw_series/` off `/tmp`; loader anchors repo-relative), the dashboard `Historical Replay` tab (`apps/dashboard/*`: range picker / lane selector / equity curve + result card / candle+markers / live-observation-start separator / characterization note; serves the precomputed pack + the control server's `/api/mf-replay1/range` endpoint), `tests/test_mf_replay1.py` (blocking CI: hand fixture, year boundary, fresh-start, warm-up, no-lookahead, live-ledger equivalence pinned on a non-overlapping sequence), `docs/mf_replay1_*`, and `KNOWN_ISSUES.md` K-037. Replay never writes/backfills the live ledgers.
- PT-RT2 turns the paper runtime over to the source-faithful signal: `services/paper_runtime/pt_rt1.py` gains the PT-RT2 slate anchors (scope `pt_rt2_mf_signal_observation`; active = `mf_source_faithful_baseline` + `mf_source_faithful_regime_gated`; archived = 10 incl. the Week 2 actives; `pt_rt2_lane_testnet_eligible()` -> False for everything and the old week2 seam closed; 1d-only timeframes; 7-major observed universe; committed-verdict constants drift-pinned by test), the lane decision path that consumes `moneyflow_signal1.signal_states` directly (entry/exit/hold + the gated twin's AND semantics with hold-prior-state-on-unavailable), and `build_pt_rt2_regime_context` (per-cycle gate through the committed `resolve_regime_filter()` seam from the runtime's own candles, verdict verbatim, explicit unavailability). `scripts/run_pt_rt1_paper_observation.py` adds `--pt-rt2` (refuses every testnet flag; daily cadence; fresh 10k ledgers in the new ignored scope dir); the dashboard control server pins the PT-RT2 safe command (no testnet pathway); OBS-OS1 daily review + watcher + CURRENT_TRUTH exporter cut over; `tests/test_pt_rt2_mf_signal_slate.py` supersedes the Week 2 slate test at the same strictness.
- MONEYFLOW-SIGNAL1 adds `services/strategy_validation/moneyflow_signal1.py` (the source-faithful Money Flow signal surface — SIGNAL ONLY, NOT ALPHA: rules pinned to the in-repo Gerald Peters PDF with sha256 + page-cited verbatim quotes and recorded interpretation choices; reuses MF-ORIG-EV1 primitives and the production indicator implementations unchanged; per-candle auditable states — stage, p.37 5/20 crossover buy/sell, p.146 Stage-2 confirmed entry, p.150 exits, RSI/MACD warnings, position machines — closed candles only, warm-up never guessed, point-in-time verifiable; regime overlay attached with the honest-FAIL verdict; exposure providers for the characterization books), `scripts/run_moneyflow_signal.py` (read-only CLI: public Binance klines or offline replay; ignored `reports/moneyflow_signal1/current_moneyflow_signal.json`; disclaimer everywhere), `scripts/run_moneyflow_signal1_evidence.py` (fidelity record + honest characterization: EXEC-EV1 friction books vs buy-and-hold/always-long/coin-flip + persistence-matched randoms, 70/30 OOS, thirds texture, MF-alone vs regime-gated with whipsaw cost; two-sided pre-stated labeling — committed label `defensive_trend_mechanic_not_validated_alpha`), `docs/moneyflow_signal1_*`, and `tests/test_moneyflow_signal1.py` (blocking CI). The namesake loop is closed: fidelity + trust delivered; no standalone validated alpha; the trade-level result (`source_faithful_but_underperformed`) stands.
- REGIME2 adds `services/strategy_validation/regime2.py` (the pre-registered objective-aligned re-test of REGIME1: selection on lowest gated TRAIN max drawdown with a whipsaw tie-break — ties within 2.0pp broken by fewest train flips — OOS never seen; gate v2 = REGIME1's bars unchanged + a pre-stated 25pp return-retention tolerance; PRE_REGISTRATION constant committed to git before the selection ran) and `scripts/run_regime2_evidence.py` (same grid/universe/books/folds as REGIME1, unwidened; emits the pre-registration verbatim + the labeled fixed-config fold texture), `docs/regime2_*`, `tests/test_regime2_filter.py` (blocking CI). Verdict: honest FAIL on exactly one pre-registered gate — walk-forward SELECTION-PROCESS stability (the endpoint config cleared every endpoint bar incl. the 30% material drawdown reduction at 33.64%, and reduces drawdown in both fold windows held fixed; at the early-history cutoff the criterion picks a fast filter that worsens the chop fold). The deployed default in `regime1.py` updates to the REGIME2 selection (lb90/br0.6/required) with the honest verdict note on every surface.
- REGIME1 adds `services/strategy_validation/regime1.py` (the market-regime risk-off filter — RISK TOOL, NOT ALPHA: breadth of reused `tsmom_ev1.tsmom_signal` trend signs + BTC bellwether rule -> closed-candle risk_on/risk_off with graded risk_score, warm-up never guessed; whipsaw stats with false-vs-true risk-off spell classification; the risk-tool gate judging MATERIAL OOS drawdown reduction at not-worse Sharpe vs always-long; the importable `build_regime_gate(datasets) -> RegimeGate.is_risk_on(as_of)`), the ADDITIVE `strategy_types.REGIME_FILTER_REF`/`resolve_regime_filter()` seam (no routing/type/gate changes), `scripts/run_regime1_evidence.py` (always-long vs regime-gated equal-weight book on DATA1 Binance perp history, chronological + walk-forward OOS, hindsight texture labeled not-a-verdict), `scripts/run_regime_filter.py` (read-only CLI: public Binance klines or offline replay; ignored `reports/regime1/current_regime_state.json`; disclaimer + committed-verdict note on every surface), `docs/regime1_*`, and `tests/test_regime1_filter.py` (blocking CI). Committed verdict: honest FAIL (`regime_filter_does_not_reduce_drawdown_oos` — missed the 30% material drawdown bar by 0.24pp and worsened drawdown in the chop fold); the tool ships with the failed verdict stated everywhere; defaults pinned by test to the committed train-only choice.
- FUND-VENUES1 adds `services/strategy_validation/fund_venues1.py` (the deep-venue + leverage structural re-open of the closed funding-carry family: binance_single / bybit_single / binance_cross_coinbase constructions over DATA1 data with cited VIP0/non-VIP fee models — taker gateable, maker a non-gateable ceiling — the explicit MarginModel for gross leverage 1x/3x/5x, the DATA1->CarryUniverse adapter with zero-volume refusal + event-count funding-day completeness, venue-fair-window enforcement per K-036, and the v3 gate: FUND-EV2's full bar + every-OOS-regime positivity + zero liquidation events), an additive `margin_model` seam on `fund_ev1.simulate_funding_carry_portfolio` (borrow on the real cash shortfall + account-level intraday liquidation marking every leg at its worst same-day extreme; default None byte-identical, pinned by the existing FUND suites), the `fund_venues1_` prefix on the same funding_carry route in `strategy_types.py`, `scripts/run_fund_venues1_evidence.py`, `docs/fund_venues1_*`, and `tests/test_fund_venues1_evidence.py` (blocking CI). Research/evidence only; verdict honest FAIL in all nine cells: deep venues fixed the cost half (breakpoint 5.0x vs HL 0.75x) but the legged-execution tail breaches the documented limit and leverage liquidates the book in the 2021 mania (-100% at 3x).
- TREND-SUITE1 adds `services/strategy_validation/trend_suite1.py` (the canonical trend suite as causal forward state machines — Donchian 20/55 with channel + chandelier 2.8xATR14 exits, dual-MA crossover, multi-timeframe daily-gated-by-frozen-weekly, the TSMOM carry-over, majority/average ensembles — each cell in vol-targeted AND equal-dollar sizing, simulated through the reused `tsmom_ev1.simulate_tsmom_portfolio` signal-provider/rebalance-timestamps seams; per-config OOS screen + deterministic vol-targeting-effect classifier), a fifth `strategy_types.py` route (`trend_suite`, prefix `trend_suite1_`, deliberately sharing `tsmom_buy_hold_risk_adjusted_gate`), a fractional-strength extension to `tsmom_ev1.target_weights` (integer ±1 path byte-identical, test-pinned), `scripts/run_trend_suite1_evidence.py`, `docs/trend_suite1_*`, and `tests/test_trend_suite1_evidence.py` (blocking CI). Research/evidence only; both headline hypotheses negative (suite adds nothing over TSMOM; removing the vol cap added drawdown without OOS return).
- RLOG1 adds `scripts/build_research_log.py` (read-only deterministic aggregator: parses the fenced yaml `research_log` post-mortem blocks in `money-flow/03_Decision_Log.md`, joins committed `docs/*_summary.json` evidence + `current_truth.json` lanes, emits `docs/research_log.json` with a `--check` CI drift guard), `docs/research_log.json` (13 entries), `docs/research_log_schema.md`, `docs/rlog1_*`, and `tests/test_rlog1_research_log.py`. The dashboard Research Log renders honest post-mortems from that JSON (standing strip, verdict banner, expandable six-facet timeline with per-phase analytics, lessons rail, active-lanes card); the outcome taxonomy (fail/mixed/context/pass) is authored in the Decision Log and never inferred from status strings — non-positive results can never render green. AGENTS.md post-task workflow now requires the block + aggregator run for every research phase. Display + docs/tooling only.
...
- PT-RT1 adds the runtime view now surfaced to the founder as `Paper Trading` and still implemented internally with the `paper-observation` view id. It is backed by `docs/pt_rt1_real_time_paper_observation_and_testnet_plumbing_summary.json`. PT-RT1.1A expands that view before the 24-hour run to show exactly 10 independent synthetic strategy lanes, founder-requested scanner symbols, blocked symbol reason codes, lane detail, wildcard diagnostics, and separate testnet plumbing status. PT-RT1.1B adds public-mainnet connection status, and PT-RT1.1C lets the dashboard prefer the active ignored runtime summary from `reports/paper_runtime/pt_rt1_1c_24h_dry_run/` before the PT-RT1.1B smoke/readiness fallbacks. The Paper Trading browser also polls public mainnet `allMids` and selected-pair `candleSnapshot` for display, while PT-RT1.3 treats mids as warning-only when clean fully closed candles are available. Signal Generation loads recent `paper_opened` rows directly from ignored PT-RT `decisions.jsonl` files when present, so prior signals remain visible even if the latest runtime summary cycle has no new opens. PT-RT1.5 made `pt_rt1_5_week1_active` the active Week 1 scope and added candle-close scheduler/testnet lifecycle status. PT-RT1.5.1 added warm-start gate counts, signed transport configured/missing status, endpoint-called fields, and public-mainnet MTM. PT-RT1.5.2 defaulted the dashboard to `pt_rt1_5_2_week1_active` and proved the transport smoke reached Hyperliquid testnet but rejected on venue size validation. PT-RT1.5.3 adds lifecycle precision columns and a `pt_rt1_5_3_transport_smoke` source so the dashboard shows asset id, `szDecimals`, raw quantity, formatted quantity, and estimated notional for the fixed-25-USDC testnet size hotfix. PT-RT1.6 defaults the Week 2 active paper slate to `money_flow_v1_2_baseline`, `avoid_low_rolling_range_20`, and `mf_orig_1d_stage2_breakout_resistance_full_equity`, archives all other prior PT-RT lanes from default active scoring, and makes `reports/paper_runtime/pt_rt1_6_week2_active/` the preferred runtime scope. PT-RT1.6.2 adds committed operating-review docs over the active ignored Week 2 logs, verifying 0 active 15m rows, baseline-only testnet lifecycle triggers, MTM coverage, closed-trade JSONL truth, and Daily Review `observation_may_continue` status without changing runtime behavior. PT-RT1.6.3 adds a testnet metadata resolver for currently blocked baseline transport symbols and a separate XRP-targeted transport-only smoke scope; it does not restart or mutate the active Week 2 runtime. The view separates public-mainnet strategy truth from Hyperliquid testnet plumbing, keeps candidate/MF-ORIG lanes synthetic-only, keeps date filters display-only, and adds no live trading approval, no production strategy changes, and no testnet strategy-PnL truth.
- `scripts/run_dashboard_control_server.py` serves the dashboard from localhost with a tiny Paper Trading control API. Its Start Run path launches `scripts/run_pt_rt1_paper_observation.py` through Mac `caffeinate` using only allowlisted durations/output directories and now defaults to the PT-RT1.6 Week 2 active scope, `--fresh-signal-only-after-runtime-start`, `--disable-legacy-testnet-probes`, candle-close signal evaluation, fixed 25 USDC baseline-only signed testnet transport gates, `--public-mainnet-only`, and `--decision-log-mode compact`; static dashboard servers leave Start/Stop unavailable. LOG-OBS1 extends its `/api/paper-runtime/status` payload with read-only runtime log-file metadata and copyable tail commands. It adds no arbitrary command execution, no private/signed/order endpoint use from strategy truth, no API-key use for strategy truth, and no live/paper production approval. Candidate lanes remain synthetic-only and testnet fills do not update paper PnL.
...
- Founder/operator report for PT-RT1.6 founder-selected Week 2 paper slate readiness.
- Documents the exact three active lanes, archived/default-inactive lanes, active 1h/4h/1d timeframes, paused 15m status, baseline-only testnet eligibility, dashboard stale/no-active-run handling, and the operator start command without starting the runtime.

`docs/pt_rt1_6_founder_selected_week2_paper_slate_summary.json`
- Compact machine-readable PT-RT1.6 summary.
- Contains active/default lane policy, archived lane policy, timeframe policy, dashboard status, runtime command, and no-order/no-live/no-production boundary flags.

`docs/pt_rt1_6_2_week2_operating_review.md`
- Founder/operator report for PT-RT1.6.2 Week 2 operating review and risk triage.
- Summarizes active ignored runtime log counts, lane/timeframe boundaries, synthetic open/closed trade status, MTM coverage, baseline-only testnet lifecycle behavior, blocked testnet metadata symbols, daily anomaly status, and the recommendation to continue unchanged.

`docs/pt_rt1_6_2_week2_operating_review_summary.json`
- Compact machine-readable PT-RT1.6.2 operating review summary.

`docs/pt_rt1_6_3_testnet_metadata_resolver_hotfix.md`
- Founder/operator report for PT-RT1.6.3 blocked-symbol testnet metadata resolver and XRP transport-only smoke plan.

`docs/pt_rt1_6_3_testnet_metadata_resolver_hotfix_summary.json`
- Compact machine-readable PT-RT1.6.3 metadata resolver hotfix summary.
- Contains runtime scope, decision/trade/testnet lifecycle counts, lane review metrics, boundary flags, daily review status, and next-phase recommendation without embedding raw runtime logs or secrets.
...
- EXEC-EV1 lightweight package (no DB imports). `exec_ev1.py` holds the depth-aware modeled friction model: liquidity-tier half-spreads, candle-volume liquidity proxy, square-root participation-rate market impact, fill-probability unfilled-chase term, the three EXEC-EV1 scenarios (parented to the SV2.3 scenarios), and the late-entry / entry-timing cost measurement. Pure Decimal arithmetic, importable from tests, no network. Models depth from historical candle volume — never real historical order-book depth.
...
- EXEC-EV1 blocking: deterministic offline tests for the depth-aware friction model — market impact scales with size and inversely with the liquidity proxy, square-root participation law, per-tier spread + fill-probability terms applied, EXEC-EV1 net PnL <= SV2.3 on a synthetic fixture that produces real trades, and the late-entry metric monotonic in lateness. No network/runtime/DB.
...
- PT-RT1.6.2 operating-review guardrail checks: verifies the report/summary exist, active runtime scope is `pt_rt1_6_week2_active`, the three founder-selected lanes and `1h`/`4h`/`1d` timeframes are recorded, active `15m` rows are zero, only baseline has testnet lifecycle triggers, candidate-lane triggers are zero, unknown/open testnet state is zero, testnet PnL updates are zero, and no live/production approval is present.

`tests/test_pt_rt1_6_3_testnet_metadata_resolver.py`
- PT-RT1.6.3 metadata-resolver guardrail checks: verifies the exact XRP smoke scope/CLI, blocked-symbol metadata resolved/missing states, XRP-targeted smoke selection without BTC fallback, local fail-closed behavior when XRP metadata is unavailable, no synthetic trade/PnL update from smoke rows, and fixed 25 USDC testnet-only boundaries.
