...
Last reviewed: `2026-05-14T23:09:49Z`
...
- SV2.1 adds `scripts/run_sv21_broad_1d_period_evidence.py`, `scripts/build_sv21_broad_1d_historical_replay.py`, `docs/sv2_1_broad_hyperliquid_1d_period_evidence.md`, and `docs/sv2_1_broad_hyperliquid_1d_period_evidence_summary.json` for broad active-Hyperliquid-public-metadata 1D period evidence plus dashboard Historical Replay artifacts. Generated raw candles and generated period campaign configs remain local under `/tmp/money-flow-sv21-broad-1d/`; generated baseline/candidate evidence packs and selected chart/trade JSON remain ignored under `reports/strategy_validation/`. The first SV2.1 run targeted 183 active public metadata symbols, imported public 1D candles back to 2024-01-01 where available, generated 646 baseline period packs across 2024, 2025, YTD, and ALL, then wrote 5116 selected replay chart JSON files and 1912 evidence-only conservative candidate pack directories for founder review without changing Money Flow rules or approving paper/live.
...
- Includes `scripts/run_pt_rt1_paper_observation.py` for PT-RT1.1B public-mainnet paper-observation runtime readiness and PT-RT1.1C probes-disabled collection. It requires `--disable-testnet-probes` and `--public-mainnet-only`, writes ignored runtime artifacts only under `reports/paper_runtime/`, uses no API keys, calls no private/signed/order endpoints, submits no probes/orders, labels PT-RT1.1C output directories as runtime collection cycles, and supports bounded smoke or 24-hour probes-disabled collection runs.
