...
- SV2.0.2 adds `configs/strategy_validation/campaigns/sv2_0_2/` with canonical DB-imported Money Flow v1.2 evidence configs for `15m`, `1h`, `4h`, and `1d`. These configs cover supported canonical symbols BTC, ETH, SOL, XRP, DOGE, HYPE, BNB, SUI, and AVAX, use `dynamic_equity_pct` with 10000 USDC initial equity per independent scenario, record explicit fee/slippage and no-testnet-strategy-truth boundaries, and exclude/defer SHIB/kSHIB from executable canonical evidence because unit semantics are not clean enough.
...
- SV2.0.2 updates the Historical Replay data-horizon/status surfaces to show canonical evidence-pack status, DB-import truth, evidence-pack count, compact-replay noncanonical truth, and SHIB/kSHIB deferred status from `docs/sv2_0_historical_data_refresh_summary.json`. The dashboard still has no order controls and does not call private/signed/order/live endpoints.
...
- Verifies the intended migrated strategy-validation DB, resolves Hyperliquid public market identity, defers SHIB/kSHIB when unit semantics are unclear, fetches/normalizes public `candleSnapshot` rows, writes temporary import files, imports through the hardened candle importer, generates canonical SV2.0.2 campaign configs, runs existing evidence-pack machinery, and writes committed summary/report files.
