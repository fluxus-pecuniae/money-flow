...
- Includes route-readiness and routing-target-recommendation request/response schemas plus derived routed submitted-order lineage, lifecycle-context, and lifecycle-event response fields so API clients can inspect routed-origin audit metadata without parsing submitted-order raw payload directly, including missing/malformed lineage flags for bounded malformed payload handling.
...
- Phase 6.3 updates it to include explicit operator-triggered accepted recommendation target-choice conversion into exactly one routed child `OrderIntent`, with no preparation, readiness, submission, route executor, fanout, ranking, scoring, CBBO, or auto-submit.
