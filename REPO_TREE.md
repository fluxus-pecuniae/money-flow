...
- Phase 7.1 adds durable operator approval records for explicit same-target automation action gates; Phase 7.1.1 makes reuse lineage-scoped and current-truth-bound, so approvals can be active, revoked, consumed, expired, or stale-lineage and never execute the approved action by themselves.
