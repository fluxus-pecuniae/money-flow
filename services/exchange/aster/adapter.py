...
def _normalize_aster_symbol(raw_symbol: str) -> str:
...
class AsterExchangeAdapter(ReadOnlyVenueAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            resolved_settings.aster_integration,
            resolved_settings,
            transport=transport,
    ...
    async def _ping(self) -> None:
        await self._request("GET", "/fapi/v1/ping", None)

    async def _fetch_symbol_metadata(self) -> Sequence[SymbolMetadata]:
        payload = await self._request("GET", "/fapi/v1/exchangeInfo", None)
        symbols = payload.get("symbols", [])
        metadata: list[SymbolMetadata] = []
        for item in symbols:
            if item.get("contractType") == "PERPETUAL":
                base_asset = str(item.get("baseAsset", _normalize_aster_symbol(item["symbol"]))).upper()
                quote_asset = str(item.get("quoteAsset", "USDT")).upper()
                symbol = _normalize_aster_symbol(str(item["symbol"]))
                metadata.append(
                    SymbolMetadata(
                        ...
                        venue_asset_id=str(item.get("symbol")),
                        market_type=MarketType.PERPETUAL,
                        product_type=ProductType.LINEAR,
                        base_asset=base_asset,
                        quote_asset=quote_asset,
                        settlement_asset=quote_asset,
                        price_tick_size=decimal_or(_filter_value(item, "PRICE_FILTER", "tickSize"), "0.1"),
                        quantity_step_size=decimal_or(_filter_value(item, "LOT_SIZE", "stepSize"), "0.001"),
                        min_order_size=decimal_or(_filter_value(item, "LOT_SIZE", "minQty"), "0.001"),
                        is_active=item.get("status", "TRADING") == "TRADING",
                        asset_id=None,
                        is_perpetual=True,
                        is_builder_deployed=False,
                        is_strategy_eligible=True,
                        is_trading_eligible=False,
                        raw_metadata={"contract_type": item.get("contractType"), "venue": "aster"},
                    )
                )
        return metadata
    ...
    async def get_venue_capabilities(self) -> VenueCapabilities:
        return VenueCapabilities(
            ...
            notes=(
                "Aster perpetual support currently covers catalog sync, top-of-book, preview/preflight, "
                "an account-targeted HMAC submit path, direct private open-order polling, "
                "and persistence-backed fill visibility for the current execution boundary."
            ),
    ...
    async def submit_order(self, intent: OrderIntent):
        if intent is None:
            raise VenueAdapterError(
                "Aster submission requires a concrete child intent.",
        ...
        if preview.preview_status != VenueOrderPreviewStatus.PREPARABLE or preview.payload is None:
            raise VenueAdapterError(
                "Aster order is not venue-preparable.",
        ...
        if not api_key or not api_secret:
            raise VenueAdapterError(
                "Aster submission requires targeted API key and secret material.",
    ...
    async def reconcile_submitted_order(
        ...
        if not api_key or not api_secret:
            raise VenueAdapterError(
                "Aster reconciliation requires targeted API key and secret material.",
                reason_codes=["missing_auth_material"],
            )
        if exchange_symbol is None or submitted_order.exchange_order_id is None:
            return SubmittedOrderLifecycleUpdate(
                ...
                status_message="Aster reconciliation could not resolve the targeted exchange symbol or order id.",
        ...
        if payload.get("code") not in (None, 0, "0"):
            return self._missing_or_rejected_update(submitted_order, payload)
    ...
    async def cancel_order(
        ...
        if not api_key or not api_secret:
            raise VenueAdapterError(
                "Aster cancellation requires targeted API key and secret material.",
                reason_codes=["missing_auth_material"],
            )
        if exchange_symbol is None or submitted_order.exchange_order_id is None:
            return self._cancel_rejected_update(
                ...
                message="Aster cancellation requires the targeted exchange symbol and exchange order id.",
            )
        params = self._signed_private_params(
            api_secret,
            symbol=exchange_symbol,
        ...
        if response.get("code") not in (None, 0, "0"):
            return self._cancel_rejected_update(
                ...
                message=str(response.get("msg") or "Aster rejected cancellation."),
                raw_payload=response,
            )
        return self._cancel_success_update(
            submitted_order=submitted_order,
            message="Aster acknowledged cancellation for the submitted order.",
            raw_payload=response,
        )
        ...
        *,
        ...
        if not api_key or not api_secret or exchange_symbol is None:
            return SubmittedOrderPrivateFillEvidence(
                ...
                fills=[],
                message="Aster private trade evidence was unavailable because auth or symbol context was missing.",
            )
        ...
        if not isinstance(payload, list):
            return SubmittedOrderPrivateFillEvidence(
                ...
                evidence_scope="unavailable",
                fills=[],
                message="Aster private trade query returned no list payload.",
            )
        ...
        for item in payload:
            ...
            fills.append(
                Fill(
                    fill_id=f"fill-aster-{item.get('id') or item.get('orderId') or filled_at.timestamp()}",
        ...
        message = (
            (
                "Aster private trade evidence was matched by exchange order id."
                if fills
                else "Aster private trade query was filtered by exchange order id; no matching fills were returned."
            )
            if scope == "order_scoped"
            else (
                "Aster private trade evidence is submitted-at-bounded same-account/same-symbol only; "
    ...
    def _open_order_from_payload(
        ...
        *,
        ...
        instrument_key = self._instrument_key_for_instrument_ref(instrument_ref_id)
        symbol = symbol_model.symbol if symbol_model is not None else _normalize_aster_symbol(exchange_symbol)
        ...
        average_fill_price = decimal_or(payload.get("avgPrice"), "0")
        average_fill_price_value = average_fill_price if average_fill_price > Decimal("0") else None
        observed_ms = payload.get("updateTime") or payload.get("time")
        observed_at = (
            datetime.fromtimestamp(int(observed_ms) / 1000, tz=UTC)
            if observed_ms not in (None, "", 0, "0")
            else datetime.now(UTC)
        ...
        order_type_raw = str(payload.get("type", "limit")).upper()
        normalized_order_type = "stop" if "STOP" in order_type_raw else order_type_raw.lower()
        client_order_id = (
        ...
        return VenuePrivateOpenOrder(
            ...
            status_message="Aster private open-order query returned a working order snapshot.",
            reason_codes=(
    ...
    async def fetch_candle_snapshot(
        ...
        payload = await self._request(
            ...
            {
                "symbol": exchange_symbol,
                "interval": _timeframe_to_aster(timeframe),
                "startTime": start_time_ms,
                "endTime": end_time_ms,
            },
        )
        ...
        for row in payload:
            ...
            candles.append(
                Candle(
                    ...
                    venue=Venue.ASTER.value,
                    symbol=symbol.upper(),
                    timeframe=Timeframe(_timeframe_to_aster(timeframe)),
                    open_time=open_time,
    ...
    async def _read_account_snapshot_for_context(self, context) -> ExchangeAccountSnapshot | None:
        ...
        return ExchangeAccountSnapshot(
            ...
            account_address=context.account_address or context.account_identifier,
            equity=decimal_or(row.get("balance")),
            available_balance=decimal_or(row.get("availableBalance")),
            margin_used=decimal_or(row.get("crossWalletBalance"), "0"),
            unrealized_pnl=decimal_or(row.get("crossUnPnl"), "0"),
            total_position_notional=decimal_or(row.get("maxWithdrawAmount"), "0"),
            observed_at=datetime.now(UTC),
        )
    ...
    def _missing_or_rejected_update(
        self,
        submitted_order: SubmittedOrder,
        payload: dict[str, Any],
    ) -> SubmittedOrderLifecycleUpdate:
        message = str(payload.get("msg") or "Aster reconciliation did not resolve the submitted order.")
    ...
    def _reconciliation_update_from_order_payload(
        ...
        if original_quantity > Decimal("0") and filled_quantity >= original_quantity:
            return SubmittedOrderLifecycleUpdate(
                ...
                status_message="Aster reconciled the submitted order to fully filled.",
        ...
        if remote_status in {"NEW", "PARTIALLY_FILLED"}:
            if filled_quantity > Decimal("0"):
                return SubmittedOrderLifecycleUpdate(
                    ...
                    status_message="Aster reconciled the submitted order to partially filled.",
            ...
            return SubmittedOrderLifecycleUpdate(
                ...
                status_message="Aster still reports the submitted order as open.",
        ...
        if remote_status in {"CANCELED", "CANCELLED"}:
            return SubmittedOrderLifecycleUpdate(
                ...
                status_message=(
                    "Aster reports the submitted order as canceled after partial execution."
                    if filled_quantity > Decimal("0")
                    else "Aster reports the submitted order as canceled."
                ),
        ...
        if remote_status == "EXPIRED":
            return SubmittedOrderLifecycleUpdate(
                ...
                status_message=(
                    "Aster reports the submitted order as expired after partial execution."
                    if filled_quantity > Decimal("0")
                    else "Aster reports the submitted order as expired."
                ),
        ...
        if remote_status == "REJECTED":
            return self._missing_or_rejected_update(
                submitted_order,
                {"code": payload.get("code", "-1"), "msg": payload.get("msg") or "Aster rejected the order."},
            )
        return SubmittedOrderLifecycleUpdate(
            submitted_order_id=submitted_order.submitted_order_id,
            venue=Venue.ASTER.value,
            ...
            status_message=f"Aster returned an unrecognized order status: {remote_status or 'missing'}",
