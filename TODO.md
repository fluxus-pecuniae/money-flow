...
Last reviewed: `2026-06-12T05:15:00Z`
...
- **REGIME1 (queued)**: condition exposure on explicit regime states (e.g., realized-vol / drawdown / breadth regimes) instead of signal sign — the one trend-family idea TREND-SUITE1 did not test. The OOS bear paid every trend form in avoided drawdown, never in returns; a regime layer would ask whether being LONG-ONLY-IN-SOME-REGIMES (and fully out otherwise) clears the absolute bar. Must route via `strategy_types.py` with its own honest gate; judged vs buy-and-hold risk-adjusted like the trend family.
...
- **REGIME1 is the next research phase** (see queued entry above; unchanged by FUND-VENUES1).
- **MONEYFLOW-SIGNAL1 (parked)**: any original-Money-Flow signal-reconstruction follow-up stays parked until REGIME1 resolves; do not fold it into another phase implicitly.
