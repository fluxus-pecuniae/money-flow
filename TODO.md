...
Last reviewed: `2026-06-12T01:45:00Z`
...
- **DATA1 (queued)**: extend the research candle base beyond the single-venue Hyperliquid window (889 aligned 1d candles, one bull-bear cycle) — multi-venue/longer history would let trend/regime claims be tested across more than one OOS regime. Public read-only data with provenance/sha256 like the FUND-EV1 snapshot discipline; no runtime change.
- **FUND-VENUES1 (queued)**: the funding-carry family is closed at retail and at scale on Hyperliquid+Kraken citations (FUND-EV2/FUND-SCALE1); FUND-VENUES1 would be the structural re-open ONLY if a venue with a materially different cited fee/rebate regime (e.g., maker rebates) is documented — new citations, new phase, never a re-tune of the closed grids.
