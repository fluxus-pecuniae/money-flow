...
Last reviewed: `2026-06-13T01:00:00Z`

## Active

- **MF-REPLAY1 SHIPPED — founder visual backtesting over the committed lanes (replay != evidence)**: the dashboard's re-introduced `Historical Replay` tab replays the two PT-RT2 lanes over the full DATA1 history with selectable ranges (all-time / calendar year / custom dates) and an accurate fresh-start "10k -> end equity" answer; one code path through the committed `moneyflow_signal1.signal_states` surface, no-lookahead + live-ledger-equivalence pinned. A green range is window placement, not alpha (rendered on the surface; the committed verdicts travel). Durable data foundation: snapshot home moved off `/tmp` to ignored `var/data1/raw_series/`; `.venv/bin/python scripts/refresh_data1_snapshot.py` (append-only, idempotent) + `scripts/export_data1_csv.py` (portable CSVs). Rebuild the dashboard pack with `.venv/bin/python scripts/build_mf_replay1_dashboard_data.py`; custom dates use the control server's `/api/mf-replay1/range`. **K-037 founder decision flagged (separately scoped):** full-equity sizing per concurrent symbol position levers the committed PT-RT2 lanes up to ~7.8x gross (~99% all-time drawdown) — is full-equity-per-position the intended paper-lane sizing? Live sizing/trajectories untouched. The open strategy hunts remain characterized-not-started: stat-arb/cointegration (the one untested systematic-alpha avenue); funding carry via institutional sub-candle atomic execution.
- **PT-RT2 OBSERVATION SLATE IS LIVE-READY — the trusted signal under paper observation, verdicts intact**: the active paper surface is now the source-faithful MONEYFLOW-SIGNAL1 signal itself, as a fresh two-lane slate — `mf_source_faithful_baseline` (Control/Baseline) + `mf_source_faithful_regime_gated` (Informational Overlay Observation; REGIME2 verdict verbatim on every payload; gate unavailability holds prior state, never defaults). Week 2 archived not deleted (10 archived lanes, ledgers untouched); NO lane is testnet eligible (paper-only founder decision — the runtime refuses every testnet flag under scope `pt_rt2_mf_signal_observation`); 7-major characterization universe (HYPE/SUI configured-not-traded); 1d only (the committed surface is daily); fresh 10k ledgers, no backfill. Start from the dashboard control surface or: `.venv/bin/python scripts/run_pt_rt1_paper_observation.py --pt-rt2 --duration-hours 24 --output-dir reports/paper_runtime/pt_rt2_mf_signal_observation --fresh-signal-only-after-runtime-start --signal-evaluation-mode candle_close_only --disable-legacy-testnet-probes --public-mainnet-only`. Observation extends trust, never upgrades a verdict. Open hunts unchanged: stat-arb/cointegration; institutional atomic execution for carry.
- **MONEYFLOW-SIGNAL1 SHIPPED — the namesake loop is CLOSED**: the project finally has a trustworthy, source-faithful Money Flow signal surface — rules pinned to the actual Gerald Peters PDF (in-repo at `money-flow/90 Reference/`, sha256-pinned, page-cited; the PDF directly confirms MACD-for-TSI, upgrading MF-ORIG's limitation), MF-ORIG-EV1 primitives reused unchanged, every intermediate term auditable on closed candles, warm-up never guessed, no-lookahead verified. `.venv/bin/python scripts/run_moneyflow_signal.py` prints the per-asset state (stage, 5/20 cross buy/sell, Stage-2 confirmed entry, documented exits, RSI/MACD warnings) with the regime overlay attached carrying its honest-FAIL verdict (live 2026-06-12: 7/7 majors stage_4_markdown, flat, RISK_OFF). The honest characterization re-confirmed there is nothing to hunt: the exposure book's OOS green vs buy-and-hold (Sharpe 0.71 vs 0.05) is the KNOWN defensive trend mechanic (cuts the bear dd 27% vs 72%, gives up the rising third +404% vs +1135%) — `defensive_trend_mechanic_not_validated_alpha`, consistent with TSMOM-EV1/TREND-SUITE1; the trade-level namesake result (`source_faithful_but_underperformed`) stands; the regime overlay trims a further ~6-8% of OOS dd at ~6pp return cost (informational, not a validated control). Re-run evidence with `.venv/bin/python scripts/run_moneyflow_signal1_evidence.py`. The live strategy-hunt threads remain OPEN and characterized, NOT started: (1) stat-arb/cointegration — the one untested systematic-alpha avenue; (2) funding carry reopened only by institutional sub-candle atomic-execution evidence. Any alpha claim from the Money Flow/trend family needs a dedicated pre-registered confirmatory phase.
- **TREND IS CLOSED ON BOTH SANCTIONED AXES (signal form + sizing)**: TREND-SUITE1 tested the canonical suite TSMOM-EV1 never tried — Donchian 20/55 (channel + chandelier exits), MA crossover 3x3, multi-timeframe confirmation, the TSMOM carry-over, majority/average ensembles — every cell under vol-targeted AND equal-dollar sizing (46 configs, train-only choice, same buy-and-hold risk-adjusted gate). Both headline hypotheses negative: (1) the suite adds nothing — the train-only choice picked the EV1 signal again (digit-exact reproduction, test-pinned); no form is profitable OOS in absolute terms (hindsight-best still −4.1%); (2) vol-targeting was NOT the cap — all 23 vt→eq pairs added drawdown without more OOS return (16/23 raised bull-window returns and gave it all back). Trend's defensive value is consistent (29/46 pass the relative screen); TREND-OVERLAY1 stays deployed unchanged. Authored mixed. Reopening trend requires REGIME1 or DATA1 below, not a re-tune. Re-run with `.venv/bin/python scripts/run_trend_suite1_evidence.py`.
- **REGIME1 SHIPPED — honest fail on its value gate; the tool exists with the verdict stated**: the breadth-based risk-off filter (fraction of majors trend-up + BTC bellwether, closed candles, no-lookahead-verified) had to EARN its use on an equal-weight long book (DATA1 Binance 7 majors, 2020-09->2026-06, always-long vs regime-gated, conservative friction). Verdict `regime_filter_does_not_reduce_drawdown_oos`: the train-chosen lb30 config cut OOS max drawdown 29.76% vs the pre-committed 30% material bar (missed by 0.24pp) and WORSENED drawdown in the 2022-24 chop fold (58 OOS flips — whipsaw is the cost of a fast filter), despite clear defensive texture (OOS dd 46.2% vs 65.7%, Sharpe 0.53 vs 0.13, return +26% vs -19.6%). Hindsight (NOT a verdict): slower/stricter configs (lb60/br0.6) cut dd ~50% — the criterion gap is the finding; nothing was re-decided post-OOS. Ships anyway per scope, failed verdict on every surface: `.venv/bin/python scripts/run_regime_filter.py` (live sample 2026-06-12: RISK_OFF, 0/7 majors up) + the importable `strategy_types.resolve_regime_filter()` -> `RegimeGate.is_risk_on(as_of)` seam for suppressing longs in market-wide downtrends (informational risk context, not a validated control). Re-run with `.venv/bin/python scripts/run_regime1_evidence.py`. A REGIME2 re-test with a drawdown-purposed train criterion (chosen up front) would be a NEW phase, never a re-tune of this grid.
- **DATA1 SHIPPED — the HL-only data limitation is fixed**: `services/market_data/data1_multi_venue.py` + `scripts/fetch_data1_multi_venue_snapshot.py` ingest perp funding history, perp daily candles, and spot daily candles for BTC/ETH/SOL/XRP/DOGE/BNB/AVAX across hyperliquid/binance/bybit/okx/coinbase/kraken — public read-only only, provenance + sha256 committed (`docs/data1_multi_venue_snapshot_summary.json`; 101/101 expected series ok, 25 venue gaps recorded as coverage, 0 fetch failures). Longest public history obtained: Coinbase BTC spot from 2015-07 (~3979 daily candles) and Binance spot from 2017-08 vs the old 889-candle HL window; Binance funding from 2019-09. Honest venue limits recorded (consult before designing a test): OKX funding history is only ~3 months deep; Kraken Futures funding ~1y; Kraken spot capped at 720 days; HL perp candles before ~2023-02/03 are zero-volume venue backfill (counted per series — not market history); Coinbase XRP keeps its real 904-day delisting hole. Consume via `load_data1_dataset()` (coverage flags + sha256 verification; missing artifacts say rerun-the-fetch, never fabricate). Refresh with `.venv/bin/python scripts/fetch_data1_multi_venue_snapshot.py`.
- **FUNDING IS NOW CLOSED ON DEEP VENUES AND AT LEVERAGE TOO (the family's third sanctioned axis)**: FUND-VENUES1 ran the structural re-open — Binance perp+spot and Bybit perp+spot (6-7y DATA1 funding history, cited VIP0/non-VIP fees at the tier a 10k account's own flow earns), the cross-venue Binance+Coinbase variant, and gross leverage 1x/3x/5x with explicit borrow financing + account-level intraday liquidation — verdict `carry_does_not_survive_realistic_costs_and_tail_oos` in ALL NINE (construction x leverage) cells. What it settled: deep venues DID fix the cost half of the HL fail (binance_single 1x: OOS +179 USDC / Sharpe 3.5 / maxDD 0.08%, every fold, leave-one-out, OOS regime, and cycle segment positive, cost breakpoint 5.0x cited costs vs HL's 0.75x, zero liquidations) but it fails the pre-committed legged-execution tail stress (9.68% vs the 8% account limit) and the OOS capture is economically thin (~0.76%/yr at 1x); leverage is CATASTROPHIC, not a capture multiplier — the 2021 alt-mania liquidates the 3x book four times and wipes the account to -100% (5x ends negative; even calm-window Bybit liquidates once at 5x with stressed DD ~90%); a nominal 1x book transiently needs ~58% of equity in financing during violent rallies (discrete rebalancing — the margin model priced it). Maker fills and the always-on book are reported as non-gateable bounds only. Only sub-candle atomic-execution evidence (closing the legging gap the stress prices) could reopen carry — a new phase with new evidence, never a re-tune. Re-run with `.venv/bin/python scripts/run_fund_venues1_evidence.py`.
- **REGIME2 SHIPPED — the criterion fix, pre-registered; honest fail on process stability**: selection on train drawdown (whipsaw tie-break) chose lb90/br0.6/required, which cleared EVERY endpoint bar (OOS dd reduction 33.64% >= the held 30% bar; Sharpe 0.88 vs 0.13; return +60.9% vs -19.6%; 35 OOS flips vs REGIME1's 58; reduces drawdown in BOTH fold windows held fixed — chop 28.2% vs 39.4%) but the pre-registered fold gate judges the SELECTION PROCESS (per-fold train-only choice, REGIME1's method unchanged) and at the early-history cutoff the criterion picks a fast filter that worsens the chop fold (45.6% vs 39.4%) — verdict `regime_filter_does_not_reduce_drawdown_oos`, the bars held, fixed-config fold texture surfaced as labeled NOT-A-VERDICT. The deployed gate/CLI now pin the REGIME2 selection with the honest note (informational risk context, not validated control). What a REGIME3 would need (new phase, pre-registered): a selection rule stable on short history (e.g., always-pick-slowest, or a minimum-train-length guard before trusting the criterion) — never a post-hoc re-read of this fold gate. Re-run with `.venv/bin/python scripts/run_regime2_evidence.py`.
- (MONEYFLOW-SIGNAL1 shipped — see the top entry; it imports the REGIME2-pinned gate exactly as REGIME2 left it: endpoint-strong, process-unstable, informational risk context with the verdict embedded.)
- TREND-OVERLAY1 deployed the TSMOM-EV1 defensive finding as a read-only signal tool: `.venv/bin/python scripts/run_trend_overlay.py` prints the current vol-targeted target exposure for the eight liquid majors on the latest closed public-mainnet candles and writes the ignored `reports/trend_overlay/current_trend_overlay.json`. Drawdown control, NOT alpha (disclaimer on every surface; TSMOM-EV1 authored mixed — the overlay lost 12.2% absolute OOS while cutting the bear drawdown 66%→17%). Signal only — no orders, no auto-execution. Defaults are pinned by test to the TSMOM-EV1 train-chosen config; re-tuning without new evidence fails CI. The optional OS panel was skipped (DASH-QA1 lockstep surgery not justified for a CLI-first tool) — scope a display-only panel phase if the founder wants it on the OS surface. Live sample at deployment: all 8 majors in downtrend → target fully flat.

- **FUNDING IS CLOSED AT EVERY CREDIBLE SCALE**: FUND-SCALE1 mapped the one axis FUND-EV2 sanctioned (account size x cited fee tier; 10k→5M x HL T0-T4 / Kraken VIP rungs) — verdict `carry_does_not_reach_viability_at_credible_scale` (authored fail). The achieved-tier surface is negative at EVERY size and the % loss GROWS with size (impact rises; the strategy's own flow never earns the tiers — $1.8M weighted 14d volume at a $5M account vs $5M needed for even HL tier 1). The lone positive stripe (Kraken 10 bps VIP at ≥50k) needs ~30x the strategy's own volume AND fails the 10%-participation plausibility cap. Even the optimistic non-gateable maker bound ceilings at ~0.26% OOS (~0.8%/yr). Funding carry is closed on BOTH sanctioned axes (cost realism + scale/fee tiers); only structural changes (market-maker flow, a venue fee regime change, sub-candle atomic execution evidence) could reopen it — each a NEW phase with new citations. Re-run with `.venv/bin/python scripts/run_fund_scale1_evidence.py`.
- **FUNDING IS CLOSED AT RETAIL**: FUND-EV2 was the one honest re-test of FUND-EV1's fail with CITED realistic costs (HL fee docs + live l2Book calibration + Kraken Pro retail tier), selectivity (2x round-trip entry margin), and longer holds — verdict `carry_does_not_survive_realistic_costs_and_tail_oos` (authored fail). The cost-sensitivity sweep shows the OOS edge dies at 0.75x the cited cost level (positive only at an implausible 0.25-0.5x); cross-venue spot is closed by retail fees alone (115 bps round trips vs 33 single-venue). FUND-EV1's spot-leg conservatism was ours (~15x measured spreads) but correcting it does not flip the verdict. **No FUND-EV3 cost tweak (discipline guard).** A materially larger account (fee tiers/maker fills) or a venue fee change would be NEW evidence with new citations, not a re-tune. Re-run with `.venv/bin/python scripts/run_fund_ev2_evidence.py`.
- FUND-EV1 tested delta-neutral funding carry (short HL perp + long HL spot equal notional; BTC/ETH/SOL via Unit + native HYPE) and the verdict is `carry_does_not_survive_costs_and_tail_oos` (authored fail): train +4.2% in the bull (funding 8-14%/yr), OOS −0.3% in the funding-compressed 2026 bear, costs ate 30% of gross at 10k size, walk-forward fold C negative, every leave-one-out drop negative. FUND-EV2 re-tested this at cited realistic costs (see above) — same conclusion. Re-run with `.venv/bin/python scripts/run_fund_ev1_evidence.py` (data refresh: `.venv/bin/python scripts/fetch_fund_ev1_funding_snapshot.py`, public read-only).
- **TREND-CARRY (synthesis hypothesis — recorded, not built)**: TSMOM-EV1 showed the trend short side defends drawdown in bears but loses money; funding could pay that short side (a short perp collects while funding is positive). FUND-EV1/FUND-EV2 set the constraints that decide the test design: (1) funding COMPRESSES OR INVERTS exactly when the bear arrives (SOL −6%/yr annualized in the 2026 bear), so the combined book must be judged on net carry through the SAME regime windows — never bull-window funding stapled to bear-window trend; (2) any funding credited to the trend short side must be priced with the FUND-EV2 CITED cost model (`fund_ev2.hl_single_cost_model`), not gross funding — carry cannot pay for entries it could not afford standalone (FUND-EV2 showed standalone carry is closed at retail); (3) the legged-fill gap risk (up to ~48% one-leg exposure = a modeled 11.3% gap loss at the window's worst candle) needs an execution answer before any size matters. Scope as its own phase with the `funding_carry` gate discipline (net-of-costs OOS + tail), routed via `strategy_types.py`.
...
- TSMOM-EV1 tested vol-targeted trend on the liquid majors: relative risk-adjusted pass vs buy-and-hold OOS (with absolute-performance qualifiers - the strategy still lost 12.2% OOS in a bear; defensive value only; authored mixed). The queued FUNDING phase shipped as FUND-EV1: funding history is now a committed data input (`docs/fund_ev1_funding_data_snapshot_summary.json`), so re-pricing the TSMOM long/short upper-bound rows with real funding is now possible; the combined test is the TREND-CARRY item below.
- RLOG1 shipped the real Research Log: honest post-mortems auto-joined from the Decision Log `research_log` blocks (schema `docs/research_log_schema.md`) + committed summaries into `docs/research_log.json` (rebuild with `.venv/bin/python scripts/build_research_log.py`; CI runs `--check`). Every future research phase must author its block and rerun the aggregator (AGENTS.md post-task step). Outcomes are authored fail/mixed/context/pass — never inferred from status strings.
- DASH-PT3 refined the Paper Trading layout: runtime/live pills in the header, status strip as the bottom reference band, Global Filters leading the body, wider untruncated Runtime Control rail, wider watchlist, OKB hidden from the watchlist display (resolver untouched), Daily Review nested scroll removed. Hard refresh if the old layout shows — cache-buster `dash-pt3-header-pills-20260611`.
- DOC-LEAN1 rotated the changelog (CHANGELOG.md = recent ~20-entry rolling window; CHANGELOG_ARCHIVE.md = older 272 entries verbatim; both canonical, nothing lost) and made the AGENTS.md pre-task reads the lean current-state set (AGENTS.md, CURRENT_TRUTH.md, 01_Current_Phase.md, recent CHANGELOG.md, 05_Agent_Coordination.md, task-relevant component docs); the heavy logs are consult-on-demand and the post-task update list is unchanged. When CHANGELOG.md exceeds ~25 entries, roll the oldest into the archive (the operational-docs guard enforces this at 25).
...
- After the current 24h runtime window, generate the Week 2 daily review, run exactly one PT-RT1.6.3 XRP transport-only smoke if still desired, inspect `testnet_order_lifecycle.jsonl`, and then restart Week 2 unchanged only if the smoke accepted/reconciled, safely rejected, or safely blocked locally.
...
- CI-CLEAN1 closed the CI enforcement loop and made installs reproducible: promoted `dashboard-qa` to blocking, split the informational lane into independent `typecheck` and `full-tests` jobs (so mypy can no longer hide the full pytest signal), and added `requirements-dev.lock` (226 pinned lines) generated by `pip-compile`. KNOWN_ISSUES K-031 tracks the strict-mypy debt with an explicit promotion criterion. See `docs/ci_clean1_ci_enforcement_close_and_locked_deps.md`. No runtime, strategy, order, testnet, slate, approval, dashboard-behavior, or test-logic changes.
